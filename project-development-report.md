...
## 2026-09-06 历史版本回退纠正计划

目标：按全部分支及工作目录审计证据纠正 dfe434d 混入的旧资源和断开的第四章功能，保留其后已确认的新玩法及本地电量/捕获修复。参考版本 e07a72e 是整合前已提交功能清单，不能整工作树覆盖。

1. 主角与配电箱：将八张侧走 PNG 恢复到 53e3121 的确切文件，保持新站立与既有帧加载器；恢复 e07a72e 的五边形配电布局及对应 CSS，仅替换配电箱样式规则。
2. Scene 任务（独立实现）：恢复 ChapterFourTemporalMazeScene 中手动调时面板、六教室调查入口、三层记录/推理、维修诊断发起、面包房动态传送带及交互、204 修复后展示；保留当前四关 Three 楼梯接入、独立追逐楼梯间、保安捕获与导航、独立请求编号、明确关门、原版灯外景与分阶段加载。拥有该 Scene 文件及一个新增针对性检查脚本。
3. 主任务接线：恢复 Controller、Host、Quest、SaveStore、Room204Model 和必要内容/类型/CSS 中对应功能的完整提交、反馈、存档与展示链。保持唯一 TypeScript 进度权威；六小游戏按已提交版本恢复，楼梯入口仍只认 a3_reference_observed。
4. 验证：对照 e07a72e 的功能入口清单，检查 typed imports、模型规则和存档往返；浏览器重点重走侧向行走、五边形配电与面包房，完成场景任务后分别进行规格和代码质量审查。
5. 原工作树中未提交的早期实验不自动合入；宣传录制和无尽模式继续独立。已授权的本地电量玩笑和保安生命周期修复不得丢失。未经新指令不 stage/commit/push。

## 2026-09-06 低电量倒计时玩笑

用户确认：电量已为 1% 时，再次执行现有耗电操作触发提示音与 10 秒警告；成功充电取消警告及后续文案，否则显示“吓吓你的”，保持 1% 并继续游戏。设计采用 App 生命周期的临时呈现状态，不写入存档，不增加实际关机路径。同一轮低电量只触发一次，充电后重新允许触发。

实现顺序：先检查既有耗电与充电事件；添加并运行失败的倒计时契约检查；实现共享计时模型与通知层；接入 App 的手机和 RPG 分支；验证 10 秒边界、防重复与充电取消。计时检查和 TypeScript 通过。浏览器分别确认未充电时正常显示结尾且电量保持 1%，以及实际充电控制器返回 charged 后电量恢复为 45%、超过原截止时间也不再显示玩笑。当前修改仅在本地。

## 2026-09-05 三处墙面局部淡化与独立分支交付

用户确认“全量源码与资源＋离线 Release，暂不合并 main”，随后标记面包坊北墙、104 教室北墙和 104/105 横墙，要求人物经过墙沿后局部淡化，并可继续向下走到合理的实体边界。本轮仍在隔离交付 worktree 操作，原始开发 checkout 保持不动。

### 实现与验证

- 原第四章人物固定在深度 9900，普通前景裁片在 4000 加各自 baseline，因此既有墙裁片无法遮挡人物。对三处获准墙面增加 `playerRevealAlpha=0.22` 的可选原图字段；场景逐帧按脚部底边决定前后关系，以当前时间态同源裁片覆盖重叠身体区域。未遮挡区域保留原人物显示，不修改人物贴图、比例、碰撞体、存档或剧情事实；主入口门的既有专用遮挡保持。
- 三处实体阻挡从墙顶移到原图底边 `y=264/248/584`，横向范围及 source annotation 保留；104/105 新增一片原图墙面前景。顶部可通过、底部不可穿越，原肖像墙两条通道与教室门洞继续畅通。
- 既有无框架地图验证器更新墙面/底边关系与可走采样，5097 项断言通过；没有删除原其他楼层、门洞、NPC、追逐和剧情检查，没有安装测试依赖。项目规则记录此次获准的显示及边界要求。
- 标准 `develop-web-game` 客户端在开发版、最终生产版运行按键并检查截图/状态。另用真实浏览器按键逐段走到墙前、局部遮挡、全身淡化、底边阻挡和前侧恢复位置：Chromium/Firefox 1063×739 与 WebKit 390×844。首次 WebKit 路径脚本未抵达目标，修正等候逻辑后重走，不把失败路径当成遮挡验证。
- 最终生产 WebKit 与 `file://demo/index.html` Chromium 再次经过三面墙。实测脚部底边分别为 264、248、584；绕教室门洞到前侧后不再淡化。两者页面/控制台错误及文档溢出均为 0。以上是专项路径，未声称本轮连续人工通关全游戏。
- `text:export` / `text:check` 同步全项目 7140 条文本索引；最后 `validate:release` **34/34，199487ms**，包含 TypeScript、生产构建、单文件和三条浏览器冒烟。类型与构建结果针对本次最终代码，未手工修改 `demo/index.html`。
- 最终单文件 **234383011 字节**，SHA-256 `f1718c141da897c5595c4d8f00ee6ef17059ec8bd45324377772e4b45a00ce28`。临时 QA 图片目检后清理，不进入源码或 Release。

### 上传范围与边界

- 用户已授权此前全部源码、资源及本次墙面修订。发布前工作区核对为 70 个已跟踪修改、32 个新增文件；另更新本次 README 与 ASSETS 日期索引。新增文件包括 23 个无损 WebP、其清单/说明及 7 个实现或生成器文件。
- `fetch origin` 后原 HEAD 相对 main 为本地独有 0、远端独有 1，远端仅为 PR #44 合并记录，树内容相同。创建 `codex/release-20260905` 从当前 `origin/main` 承接现有修改；不会推送或合并 main。
- 发布目标为 `demo-20260905` 预发布：离线 HTML、全量已跟踪源码/资源 ZIP、各自 SHA-256 文件。README、ASSETS、附件和 ZIP 内根目录均使用 `20260905`。具体上传完成与远端资产验证以 Release 页面及本次交付回复为准，不将本地检查描述为远端 CI。

## 2026-09-05 A 版文案扩展：全物品详情与第三至四章

用户选择 A 版适度引导，要求语言更荒诞、带冷嘲讽，直接修改；随后明确范围不能局限于此前示例，还包括各种物品介绍。本轮在既有隔离交付 worktree 实施，原始开发 checkout 保持不动，未暂存、提交或上传。上一节完成的节奏钓鱼重写保留。

### 实施范围与文字边界

- 全部 **64 件物品**的简介重写，包含第一、二章取得的材料、第三章餐品/票根/湖区工具及第四章钟表部件；同时修改详情用途备注和纸本文书。与 HEAD 逐项核对，64 项 id、顺序、sourceScene 和 target 完全相同。
- 发现 `items.config.json` 与 `PixelIcon.ITEM_META` 原有两套物品简介，实际界面消费后者。已让 `ITEM_META` 直接投影配置，手机和 RPG 详情共用同一份名称、简介；移除详情表中未被渲染的重复 intro 字段。64 个 ItemId 唯一且齐全。统一来源规则写入 AGENTS.md，避免只更新文档却未改变游戏。
- 票据正文覆盖离座留言、馆藏单、识别/认证报告、取餐小票、湿节目单及签到记录纸等；保留证据字段、编号、时间与道具消耗关系。用途备注主要描述接口、材质和可观察反应，不提前列出尚未取得的整套材料。
- 第三章修改食堂回收/调饮/点单/取餐、剧场验票/拼票/节目单/道具、湖区调查与失败反馈；3.5 章修改离湖时间、记录核对、任务与调查图文字；第四章修改任务提示、面包坊、值班登记、维修诊断、追击和终章确认。共享调查图改为“调查记录 / 已查记录 / 已查齐”，不再展示“并行调查环 / 汇合”等实现术语。
- A 版继续提供当前问题、可查来源、必要输入和失败纠正。讽刺集中在登记、盖章、补材料、办事条件与局部对白，避免每句强加笑点。校史两问及事实资料保留原有语气，没有改写历史人物言论。
- 第四章回答保存后只显示“回答已保存”，移除该状态三条内部播放验证提示；40 项任务键保留，提示数从 117 调整为 114。相关原有验证器更新获准文案期望，时间、楼层、物品事务、追逐与完成证明断言未删除。
- 剧场三句系统对白及保安上楼一句改变了语义，同步修改中文台词与英文朗读稿，通过既有音频生成器增量生成对应 **4 个 MP3**；沿用原角色、音频键、通道和触发事件。音频校验通过，未静默取消配音。

### 验证与交付

- `validate:quick` 4/4；文案期望同步后的 `validate:critical` 23/23；最后一次 `validate:release` **34/34，71396ms**，包含类型、文本导出、剧情/地图/音频合同、生产构建、单文件和浏览器冒烟。`git diff --check` 通过。
- 标准 develop-web-game 客户端运行剧场入口按键；读取状态及截图。另通过 Chromium 实际步行接近检票员、触发对白，检查新行显示。
- Chromium、Firefox、WebKit 实际双击 RPG 油渍纸巾详情、返回手机并用键盘打开同一物品详情、查看分针和签到纸正文、进入记录核对页、回答终章两问并等待完成。没有页面/控制台错误或文档横向溢出；手机保持 1:2。覆盖 1063×739 桌面与 WebKit 390×844 小屏。
- 小屏签到纸正文可滚动到底部，实测 dialog scrollTop=111、clientHeight=692、scrollHeight=803，页尾可见。长文案没有靠裁去正文解决空间限制。
- 最后共享调查图文字调整后重跑完整发布套件；最终生产 HTTP 的 WebKit 与 `file://` 单文件 Chromium 再次检查详情、记录页和两问到点灯完成。以上为专项路径验证，不声称全游戏连续人工通关。
- 文本索引重新生成于 `docs/game-text-by-chapter.md`，共 7138 条，这是全项目导出数，不能作为本轮改写数。临时浏览器截图在目检后清理，不进入交付。
- 最终 `demo/index.html` 为 **234381755 字节**，SHA-256 `6575565f2ca964d6f53db0036855a1d74974cbdd7ef337f27083723e52fa9d8d`，由构建生成，无手工修改。

试玩：`http://127.0.0.1:4180/?recording=1&devCheckpoint=c3-theater-entry&previewFix=20260905-copy-a-expanded`。仍使用 4180 生产资源服务，无需换到开发端口。

## 2026-09-05 节奏钓鱼裁切故障与三列界面重写

用户要求优先修复长期不显示方块、疑似遮挡的问题，允许重做。暂停第三至四章整体文案方案，未实施未选择的文字改稿；继续在现有隔离交付 worktree 工作，未暂存、提交或上传。

### 根因与实现

- 在原生产版 `c3-qizhen-rhythm-fish` 真实启动钓鱼，运行态 `running / elapsed=0.816 / judged=0`，截图中的 S 方块右侧被截断。原音符使用独立 GeometryMask，遮罩通过 `scene.make.graphics` 创建且不在显示列表；共享 `RpgRenderResolution` 只为显示列表中的屏幕对象应用逻辑分辨率补偿，因此音符和遮罩的坐标变换不同。原判定验证通过，无法证明真实像素完整。
- 重写 `QizhenFishingRhythmVisual`，新增纯布局 `QizhenFishingRhythmLayout`：整个节奏 UI 共用单一屏幕根容器，删除模板遮罩，先计算可见矩形再绘制。三列为 A/S/D；短块点按、长条持续按住，长条头部固定在线上、尾部继续下降。下一枚音符始终可见并持续下落，原谱面目标时刻不变。
- 固定键区与音符区分离，音符字母位于反馈线之上。保留世界层的浮标与钓线，复用少量图形和文字对象，没有新增图像资源或依赖。
- 浏览器发现第二处遮挡：竖屏顶部任务条覆盖缩小后的节奏标题。宿主改为会话期间卸载任务条，继续隐藏库存、模式与普通字幕；小屏横向触控按钮对齐固定键区，离开下落区域，最小高度 44px。
- 原模型、谱面、判定窗口、失败辅助、音频时钟、道具前置检查和单次结算事务均保留。只增加按需调试的几何快照，没有新存档字段。
- 现有无框架 fishing validator 增加四谱面几何采样：到线时刻、左右边缘、有限裁切、键区分离、长条收缩、单根容器与无独立遮罩。真实浏览器像素检查仍为独立验收环节。

### 实际验证

- 修改前后分别运行标准 `develop-web-game` 客户端；最终生产版也重跑并读取状态和截图，S 方块完整可见，未生成浏览器错误文件。
- 四个目标通过真实浏览器按键完成：Chromium 网框与最终纸条、Firefox 钥匙、WebKit 手机小鲤鱼。前三者分别获得对应道具，纸条进入 `swan_chase`；保留原 8/4/1/26 音符数量。
- Firefox 在 DPR 2 下从 `1063×739` 调整到 `1000×700`，当轮继续且 D 列完整。最初跨越 1100px 桌面分栏阈值触发原有场景重建，正确取消当轮并保留物品；此项不作为无缝跨分栏验证。
- WebKit `390×844` 真正触屏点按完成小鲤鱼；`844×390` 检查长条和三键区域。Chromium `844×390` 通过浏览器 TouchStart/TouchEnd 完成网框长按，D 按钮与方块没有被截断。控制台和页面错误均为 0。
- 减弱动态模式下，两轮不输入失败均保留所有道具，第三轮启用辅助并取得小鲤鱼。中途点击返回手机、重新加载后没有恢复半张谱面，仍只存在一个画布。无头浏览器切换标签未改变 visibility，因此不声称本轮实测了后台隐藏取消；原隐藏事件处理未改动。
- 最终生产 HTTP 与 `file://demo/index.html` 各自再次用按键完整钓取网框，长按和右侧方块截图已检查，实际获得 `brokenNetFrame`，无页面/控制台错误。
- `validate:quick` 4/4、`validate:critical` 23/23 通过。调整最后的字母/判定线层级后重新导出文本并完整重跑 `validate:release`，最终 34/34（107909ms），包含 TypeScript、生产构建、单文件、剧情/地图/音频合同与浏览器冒烟。`git diff --check` 通过。
- 单文件为 `234232894` 字节，SHA-256 `abb6f05e07dbc97c0ae7c6fe81f48154c2c8c3a350826e8b51c2a6b5da7ca482`。仅保留最终回复附带的一张生产版截图，其余本轮临时截图/脚本清理，不进入游戏交付。

试玩：`http://127.0.0.1:4180/?recording=1&devCheckpoint=c3-qizhen-rhythm-net&previewFix=20260905-rhythm-rebuilt`。本次结论限节奏钓鱼与相关退出/恢复路径，未宣称连续通关整个游戏。

## 2026-09-05 全章节交互辅助框与道具栏简化

用户要求：道具栏删除拖动、距离与场景可用性说明；从第一章到终章隐藏鼠标靠近、悬停或拖拽物品时出现的交互框。本轮继续修改隔离交付 worktree，保留此前性能与剧场入口功能，未修改原始开发 checkout，未执行 Git 暂存、提交或推送。

### 实现边界

- 手机共享 `InventoryBar` 删除目标候选框与悬停框的 class 注入和 CSS，保留 `[data-drop-target]`、实际命中判定、合成和道具事件。持有或拖动时仅展示道具名称，不再显示拖动、距离、剧情校验提示；失败说明也不再引用已删除的高亮区域。
- 共享 `RpgInventoryDock` 仅展示“道具”、图标和名称。删除拖拽指南、可用性说明、结果扩展面板和鼠标停留原生长提示；详情查看、键盘输入、指针捕获、拾取动画及松手后的 controller 检查保留。实际使用结果通过现有 `rpg_subtitle` 短暂显示。
- 删除剧场各设备的鼠标悬停框，以及剧场、图书馆、启真湖的道具投放范围绘制对象和每帧同步逻辑；关闭食堂饮品投放框。图书馆前台辅助框和入口小屏额外悬停描边、寝室物件的矩形闪框、签到热区的鼠标悬停描边同步移除。
- 审核第一章手机页到第四章终章的独立悬停/绘框入口。第四章普通发布态已无热区边框，保留正常物体外观、观测线索、鱼群水纹、谜题必需图形和键盘焦点提示。没有改动碰撞、范围数值、地图数据、模式门槛或剧情状态权威。
- 新展示边界写入 `AGENTS.md` 的 Quest And Inventory Feedback，替换原先强制高亮投放区域的旧要求。

### 实际验证

- 真实浏览器：第一章签到页面截图检查，最终生产版在 `430×860` 点击“收集本周缺勤次数零”取得 `d1=0`；第二章微信页拖天气水滴到导师头像，候选/悬停 class 数量均为 0，水滴消耗并取得导师竖线。
- Chromium `1063×739`、Firefox `1440×900`、WebKit `390×844` 均实际键盘沿剧场通道移动、悬停海报、拖入纸巾，最终 `posterCleaned=true`、纸巾消耗、票根 A 获得；道具栏没有说明面板，console/page errors 均为 0。Chromium 直接打开生成的 `file://` 单文件完成同一路径。
- 图书馆前台、食堂饮料区、湖区钓网框和第四章大厅检查点逐项拖动物品并检查截图。真实终章两问依次选择后，经过点灯达到 `chapter4.completed=true`，没有新增浏览器错误。
- QA 初次移动脚本撞到柜体/门柱，曾在海报可用距离边界外投放；依据现有碰撞与目标边缘调整实际行走路径，独立重跑通过。没有为使 QA 通过而修改碰撞或距离条件。
- 标准 `develop-web-game` 客户端分别执行剧场键盘移动与手机页面检查，截图和状态均已阅读，无错误产物。最后移除入口小屏两行悬停描边导致文本清单行号过期，重新生成清单后最终 `validate:release` 34/34 通过（86967ms），包含类型检查、文本清单、原有剧情/地图/音频合同、生产构建、单文件构建与浏览器冒烟。
- 最终 `demo/index.html` 为 `234254654` 字节，SHA-256 `37b225bd1aeeb4913563378176a2ad77d771e7dea3b6ffc516c58cac66a1f465`。由正式构建生成，临时截图已清理，没有手工编辑生成页面。
- 以上是源码范围审核与跨章节专项浏览器回归，未声称逐个连续通关所有章节、所有可选交互。临时截图和 QA 脚本不进入交付。

试玩：`http://127.0.0.1:4180/?recording=1&previewFix=20260905-clean-interactions&devCheckpoint=c3-theater-entry`。离线交付仍为生成的 `demo/index.html`。

## 2026-09-05 剧场入口感应门、现场充电与票务文案

用户本轮标记四处：正门靠近开合、取票提示过于直接、左侧柜体改为现场充电站、完整票拼接对白。继续在隔离交付 worktree 修改，原始开发 checkout 未改动，未暂存、提交或上传 GitHub。

### 实现

- 正门沿用当前同源闭门双扇素材与开门底图，增加近距离打开/离开关闭的不同距离区间，避免临界位置反复开合。感应动画不签发门票，不改变验票和剧情离场条件。
- 左侧原柜体保留位置和碰撞，以原生 Phaser 图形加装固定充电标识、电池指示、两个接口、短接线和接入手机状态。靠近设备按 Space 或触控交互，在浅色模式完成约 2.2 秒补电后恢复到 45%；走远、换模式、离开现场或隐藏页面取消未完成补电。充电过程保留设备进度和邻近状态，不弹字幕遮住设备。
- `PhoneChargingStation.ts` 统一现场坐标、距离和模式检查；`PhoneBatteryController` 二次检查现场/位置后写入存档电量。彻底删除控制中心的充电操作与原地图级远程充电方法，保留省电模式、耗电规则、1% 主线保底。
- 初始目标改为“找到进入观众席的办法”，检票员和取票机只透露临时退票/校园论坛留言。已接单和抢中后的取票进度仍提供当前操作反馈。
- 票根说明、打印说明、拼合对白和检票对白保持同一逻辑：场次/票号一致、断口吻合、验票区域恢复。打印提示结束后才进行拼合和对白，按文本长度计算阅读时间。返回现场重建时清除临时对话/面板锁，不清除票务事实。

### 验证记录

- 修改前真实 Chromium：出生点 `(836,820)` 的门一直 closed，任务直接显示去 CC98；源代码确认控制中心只凭 rpgScene 就可充电。
- 三内核实测：Chromium 1000×740、Firefox 1440×900、WebKit 390×844，键盘走到设备后门关闭、回到入口重新打开；17% → 45% 只补一次；走远取消、深色不充电、重复交互不增加次数，页面无报错。相关截图已逐项检查。
- 新增依赖轻量的 `theater:validate-lobby`，接入 critical/release：实际运行通过 9 类拒绝分支、补电幂等/高电量不降低/存档恢复/1% 保底、两种先后顺序的票根生产与一次性拼合。未新增测试框架或依赖。
- `validate:quick` 4/4、`validate:critical` 23/23 已通过；最终 `validate:release` 34/34（76585ms），包含类型检查、生产/离线构建和三条浏览器冒烟。标准 web-game 客户端已执行键盘移动并检查截图。
- 最终生产版 Chromium 真实操作通过：返回手机检查无充电按钮 → CC98 接单 → 校园网第一波失败 → 切移动数据第二波成功 → 切回校园网，从浙大钉地图返回剧场 → 拖纸巾擦海报 → 取票机输入回执 → 打印/拼票对白 → 拖完整票验票 → 进入 `program_search`。两半票消耗、完整票保留；未通过脚本改写游戏状态。
- 最终 WebKit 390×844 点按触屏交互键启动充电；最终 Chromium `file://` 单文件再次通过门开合、17%→45%、重复不补、深色拒绝和走远取消，均无 page/console error。最终字幕已去除重复的“玩家：”前缀。
- 单文件 `demo/index.html` 为 234269833B，SHA-256 `cf213075503ee18fb8bcd6156476e2c516ffa98488fbdb86a43e1d47b4edb8f1`。本轮覆盖剧场入口专项，未重新完整通关四章。临时截图、导航脚本与日志检查后清理；未添加测试依赖。

试玩：`http://127.0.0.1:4180/?recording=1&devCheckpoint=c3-theater-entry&previewFix=20260905-theater-lobby`。

### 可复用检查

- 现场服务同时检查场景、人物位置和当前模式；手机只显示状态与地点说明，不拥有现场动作。
- 临时演出/对话先检查生命周期与排队，避免生产道具成功后被并发字幕遮挡。以上在本轮剧场三内核和两种票根顺序中验证，其他场景尚未扩展。

## 2026-09-05 可见加载等待优化

用户补充：降低负载的同时尽量减少玩家肉眼可见的加载时间。本轮仍使用隔离交付 worktree，原始开发 checkout 不改动，不提交或上传 GitHub。

### 已实现与原因

- CC98 校园网访问已通过时取消固定 700ms 验证等待；未通过时的 1600ms 验证、620ms 退出和票务例外条件保持。浙大体艺闪退时序未更改。
- CC98、录音、记录恢复共享可复用模块导入 Promise，主页停留后只空闲预取已开放应用；指针进入/按下/聚焦可提前开始。未开放入口与新游戏闹钟不预取，预取不修改剧情、库存或电量。
- RPG 场景预热从逐张串行变为有上限并发：后台 2、明确请求 4，受限网络/低内存 1。修复手机子页面切换导致相同目标预热反复取消。
- 新增限容量、限时的已解码图片缓存，与正常 Phaser 加载结果、预热结果共用源图。寝室、图书馆、食堂、剧场、启真湖、第四章阶段资源以及已有共享地图/角色加载入口接入。保留原画质、精灵表尺寸和游戏规则。
- 缓存 64 MiB（低内存 16 MiB），闲置 60 秒过期，后台释放引用。缓存失效/超限时正常加载，不把缓存命中作为剧情事实。上限只描述缓存，不代表全游戏内存。
- 真实断网注入发现重试后加载提示停留：Phaser shutdown 清掉 loader 监听，但旧 WeakSet 阻止重装。已清除 shutdown 标记并在完全缓存的 create 完成时报告就绪；实测三次失败后恢复资源，再次点击重试能进入和往返地图。

### 可见等待测量

同机 Chromium、1000×720、干净浏览器上下文、相同 Python 静态服务器类型（无 gzip），每项前后各三次。使用真实点击和 `render_game_to_text`；进入地图后另行验证键盘位移。手机测量等待两次动画帧后取可见耗时。模拟网络只用于本机对照，不代表公网、真实手机或所有章节。

| 场景 | 条件 | 前三次 ms | 后三次 ms | 中位数变化 |
| --- | --- | --- | --- | --- |
| 首次打开 CC98 | 1 MiB/s、80ms 延迟，主页停留 3s，悬停 250ms | 1349.8 / 1361.8 / 1331.0 | 100.0 / 99.3 / 95.8 | 1349.8 → 99.3，减少约 92.6% |
| 手机返回启真湖 | 5 MiB/s、80ms 延迟，手机停留 2.5s | 236.2 / 223.2 / 235.4 | 80.6 / 91.1 / 78.8 | 235.4 → 80.6，减少约 65.8% |
| 直接冷启动启真湖检查点 | 5 MiB/s、80ms 延迟，没有此前预热 | 4149 / 4029 / 4054 | 4131 / 4045 / 4046 | 约 4.05s，未见明显改善 |

返回启真湖的图片加载条目由 33 降到 0；此前多为 HTTP 缓存命中的 XHR，不能当作省下同等公网流量。第一次冷进入仍加载场景完整资源，不能以热进入结果声称所有地图即时打开。闹钟启动仍只有 JS、CSS、字体三个资源；未压缩同口径主脚本由 1,367,689B 到 1,372,081B（增加 4,392B），图片缓存 0B。未重新引入启动大地图预载。

### 验证

- 最终 `npm run validate:release` 33/33，通过 TypeScript、生产构建、离线构建/校验、相关音频/剧情/地图合同、三条浏览器冒烟。首轮两处失败为文案源位置清单过期和旧正则固定要求反向 if 语法；重新导出文案并明确检查权限、时间线排除和稳定 effect 依赖后复验通过。
- 有界缓存临时执行验证 13/13：失效图拒绝、容量/LRU、超大图、到期、低内存上限、图片/精灵表复用、幂等与未命中回退。没有新增测试框架或测试依赖。
- Chromium 人为阻断湖区主图三次，失败提示可见；恢复网络并点击重试成功，库存不变。再往返三次，每次恰好一个画布，缓存均为 33 张、32,957,984B，没有逐次增长。
- Chromium、Firefox、WebKit 定向检查三种延迟手机页、两问到终章完成、湖区移动和返回手机，均无 page/console error，手机页无文档溢出。标准 web-game 客户端执行地图移动，状态和截图已检查。
- Chromium file:// 生成单文件复测手机页、终章和湖区链。单文件 234,262,293B，SHA-256 `35ce859538159786bdaea817a6e57ed4597bedd1dd894138bdadfeb832754bb9`。
- 最终三内核额外实测主页预取→回到湖区→打开CC98，电量和库存未因预取变化、每次一个画布、页面可见且无溢出/错误；离线最终构建也完成上述专项场景复测。
- 对比原始 JSON 结果位于 `/private/tmp/7-55-entry-latency-1WQ0hg`，仅作为本机调查材料；关键条件和原始数值已保存在本报告。临时诊断脚本与截图检查后删除。

### 复利回溯

- 复利回溯：统一预取、源图复用和实际就绪信号；做得好：加入真实失败重试，发现了常规正常路径不会出现的持续等待。
- 成功模式压缩：分别测冷进入、提前准备后的进入和失败恢复。类似场景：手机应用入口与 RPG 场景切换。验收口径已写入 `docs/client-compatibility.md`。
- 遗憾检查：直达未缓存大地图仍约 4 秒；真实低端设备长时间内存和公网下载未完成测量。以上收益限于本机各三次对照，未推广到全游戏所有入口。

试玩：`http://127.0.0.1:4180/?recording=1&previewFix=20260905-fast-entry`。

## 2026-09-05 四分钟实机预告片

- 用户要求由 Computer Use 实际操作并录制，包含签到提醒、体艺闪退、食堂纸条、启真湖划船钓鱼、灿若星辰灯。已在独立媒体目录 `/Users/zhuhangcheng/Movies/7-55-trailer-20260905` 完成首版，未修改游戏逻辑或原始开发 checkout，未执行 Git 提交/上传。
- 通过既有录制模式和检查点操作游戏，屏幕录像不采集麦克风。首次部分素材受到桌面切换干扰；用户确认可连续录制后重新取得闹钟、微信签到、两问及点灯的干净录像。无关桌面不进入成片。
- 成片 `final/7-55_实机预告片_4分钟_1080p.mp4`：240.000 秒，1920×1080、30 fps、7200 帧，H.264/yuv420p，AAC 双声道 48 kHz，32,209,962 字节。31 个剪辑段，6 个容器章节；字幕、淡入淡出、局部特写、适度慢放及末尾 7 秒真实点灯帧定格均在后期完成。
- 现有游戏配乐及效果音后期编排，无额外 AI 游戏画面。钓鱼镜头是实际抛线及节奏会话开局，未伪造成功钓获或补画音符；保安与天鹅为追逐动作特写，未展示通关结果。
- 已实际运行 `edit/render_trailer.py` 与 `edit/verify_trailer.py`，全片音视频完整解码无错误；62 个分镜抽帧逐页检查，修改了字幕与湖区提示重叠、钓鱼按键特写裁切。最终音频峰值 -1.5 dBFS，均值 -18.7 dBFS；尾段检查确认片尾停留在点亮帧，没有切回手机主页。
- SHA-256：`534519211357369b58d29447c406eb617d8ecf60e2b3f42068a0741e372e18c9`。素材、选用音频、时间线、字幕层、剪辑和验证脚本保留供改版；临时抽帧在检查后清理。本次视觉验证是分镜抽帧检查，未声称另行实时观看完整四分钟。

## 2026-09-05 游戏性能优化

用户请求：尝试优化整个游戏的性能，使其降低负载与快速加载。

本次修改在 `/private/tmp/7-55-chapter4-delivery-tZmEdm` 完成。原始开发 checkout 未改动，本次没有暂存、提交、推送或合并。

### 已实现

1. 新游戏手机阶段禁止提前预热尚未开放的校园地图。`App` 通过既有 `fullCampusMap` 权限选择器判断；地图开放后的空闲预热保留。
2. CC98、录音、时间线恢复三个较重手机页面改为进入时加载代码；局部 Suspense 保留手机外框，离线构建继续内嵌全部代码。
3. 校园、寝室、图书馆、食堂、剧场、启真湖、第四章主场景的调试快照改为读取时生成。正常帧不再构造大量诊断数组；诊断调用仍取得最新状态，场景清理释放闭包，不修改剧情、时钟、碰撞或存档。
4. 两张校园图、九张室内/湖区图、十二张第四章楼层/时间图生成无损 WebP 派生文件。原始 PNG 保留为源图；构建插件验证源图和派生图哈希后才替换打包资源。生成命令 `npm run assets:lossless`；编码器 `cwebp 1.6.0 -lossless -exact -m 4`。正常构建不依赖编码工具。

### 同机单次测量

Chrome，1280×720，生产 HTTP，干净浏览器上下文。页面 load 后等待 8 秒，再测量 5 秒。资源量为 ResourceTiming 的 encodedBodySize 求和，含期间预加载或重复加载条目，未包含 HTML 文档，不能直接视为实际网络流量。CPU 数值为该 5 秒窗口内 ScriptDuration，受系统负载和场景时序影响；当前只有一组前后样本，性能比例属于本机观察，未推广至所有设备。

| 指标 | 优化前 | 优化后 |
| --- | ---: | ---: |
| 新游戏启动资源条目 | 30 | 3 |
| 新游戏启动资源体积 | 26,736,068 B | 1,098,654 B |
| 新游戏 JS 堆快照 | 19,521,780 B | 5,458,998 B |
| 终章检查点资源体积 | 42,666,009 B | 33,507,196 B |
| 终章 5 秒脚本耗时 | 0.6252 s | 0.5192 s |
| 追逐检查点资源体积 | 86,851,674 B | 67,466,788 B |
| 追逐 5 秒脚本耗时 | 0.4836 s | 0.4104 s |
| 23 张地图合计 | 98,680,130 B | 72,494,984 B |
| 离线单文件 | 269,164,832 B | 234,255,410 B |

### 验证与边界

- 23 张原图与派生图使用 Pillow 解码为 RGBA 后逐字节比较，23/23 完全一致，尺寸和透明像素均保持。
- 调试快照执行验证通过：登记时不立即计算、连续读取实时更新、即时快照覆盖旧场景提供者、清理后返回 null。
- `npm run validate:release` 33/33 通过，包含 TypeScript、生产构建、单文件构建与校验、音频/地图/剧情合同及三条浏览器启动检查。首次执行中旧校验器固定要求原插件列表，已改为明确验证包含新地图插件且保留原视频嵌入顺序，未删除原 H3 合同检查。
- 标准 develop-web-game Playwright 客户端执行第四章 opening 检查点，画面与诊断状态已检查，无错误文件。
- 生产版 Chromium 1280×720、Firefox 1440×900、WebKit 390×844 均实测三个延迟加载手机页面、两问到终章完成、湖区移动、返回手机。九次手机页面检查无文档溢出；三条终章链均 complete；三种浏览器 console/page error 为 0。此为定向兼容验证，未宣称全部浏览器版本或所有尺寸组合通过。
- Chromium 直接通过 file:// 打开生成单文件，复测相同五处场景及返回手机，全部完成且无 console/page error。地图移动前后位置变化，回到手机并完成卸载后诊断状态清空。
- 单文件 SHA-256：`9a88286b8806a740033bfa23af898b6f6527869a6f25f53d819da87715c280fb`。
- 本次不降低分辨率、不减帧、不裁剪剧情。无损压缩不减少同尺寸纹理解码后的内存；较大地图与完整视频仍使离线单文件偏大。在线试玩优先使用拆分式生产版。
- 未完成的后续方向：真实低端手机长时间游玩的内存曲线、已离开区域的纹理生命周期、慢网下各章节入口就绪时间。需要对应数据后再决定优化，当前未更改这些机制。

本地生产预览：`http://127.0.0.1:4180/?recording=1&previewFix=20260905-performance`。既有 4178 开发页仍运行；其地图继续使用源 PNG，完整压缩效果请从生产预览检查。
...
- `scripts/verify-rpg-character-sprite-integrity.mjs` is the dependency-free Node authoring validator for all four roles. It checks source component counts, head/foot padding, source/output silhouette overlap, aspect ratio, per-animation scale stability, and the recovered protagonist mapping without relying on Python or Pillow.
...
# 2026-08-28｜启真湖雨天落水与寝室吹风机流程

## 1. 剧情与状态闭环

启真湖雨天首次尝试上船时，值班老师只说明当前不能下水。第二次尝试触发强行下水、落水、救援和回寝室，控制器把湖区阶段写为 `rain_recovery`。玩家在自己的书桌拾取吹风机后，才可进入天气页的风向校准。完成低、中、高三层云带的六步校准会消耗吹风机、停止降雨，并把湖区恢复到上船教学阶段。

## 2. 视觉与交互

- 吹风机使用独立 `128×128 RGBA` 像素素材，机身、电线和插头完整；寝室以 source-pixel 锚点把它缩放到书桌尺度，并使用 nearest filtering。
- 道具只在救援完成、尚未拾取且天气尚未放行时出现。拾取成功后，场景对象、可交互目标和道具栏状态在同一 controller 结果后更新。
- 天气页复用同一素材作为校准设备。三个云带各有当前位置与目标槽；所有云带对齐以前，最终送风按钮保持不可提交。
- 任务栏只给当前目标：回寝室时提示寻找可用设备，拾取后提示处理天气记录，不提前暴露最终天气答案。

## 3. 存档与兼容

存档版本为 `30`。v29 中已经提出天气处理、但仍停留在旧 `boarding_tutorial` 的存档会迁移到 `rain_recovery`，补齐救援、天气请求和吹风机，以保证恢复后存在可完成路线。已经完成雨天放行的存档保持吹风机已消耗。

## 4. 验证证据

- 启真湖雨天专项：`67` 项断言通过。
- TypeScript：`npm run typecheck` 通过。
- 生产构建：普通 `npm run build` 通过；本轮未构建单文件。
- Chromium：寝室书桌显示、近距离拾取、拾取后消失、任务切换、天气起始页和校准面板均通过；console/page error 为 `0`。
...
| 目标 | 档位 | 时长与输入 | 主线作用 |
|---|---|---|---|
| 生锈的柜门钥匙 | 完整教学 | `10s / 8` 次 | 依次教学 `A / S / D`、张力与失败恢复 |
| 破损网框 | 短谱 | `7.5s / 4` 次 | 保留一次长按，只检查玩家是否掌握收线 |
| 小鲤鱼 | 一次判定 | `5s / 1` 次 | 水纹收紧时按 `S` 提竿，不重复完整谱面 |
| 纸条本体 | 完整高难 | `20s / 26` 次 | 两次长按、错拍节点和逐段增强的节拍，完成后直接进入追逐前状态 |

四档仍使用同一 `QizhenFishingRhythmModel`、同一两阶段事务和同一控制器结算。谱面数据通过 `experience` 和 `instruction` 声明体验档位，场景提示、模型时长和 validator 从同一来源读取。

## 2. 场景与音频闭环

- 大湖补齐小鲤鱼的实际浅色钓点；持有 `fishFeedPellets` 且尚未捕获时才激活。失败不会消耗饲料，成功后由原控制器一次转换为 `smallCarp`。
- 破损网框继续位于返航河道。对应 DEV 检查点切换到 `qizhen_channel / channel_entry`，避免在大湖母图生成无法接近的目标。
- 钓取开始使用正式节奏音乐与湖面环境层。普通三次完成后恢复湖畔音乐；最终纸条开始时提升音乐强度并增加低频场景声，完成后不恢复平静音乐，只播放纸条脱离声并交给后续追逐。
- 通用 `qizhen_fishing_completed` 只负责清理 UI。宿主根据目标继续发 `qizhen_fishing_catch_completed` 或 `qizhen_fishing_paper_completed`，音频时间线据此区分普通收获和最终捕纸。

## 3. 验证证据

- `npm run qizhen:validate-fishing`：四档谱面、判定边界、网框长按、小鲤鱼单次提竿、纸条密度与错拍、取消和失败恢复、音频分流、场景入口与 DEV 检查点全部通过。
- `npm run qizhen:validate-tool-branches`：三条道具分支六种排列、`114` 项断言通过。
- `npm run typecheck`、第三章音频合同、`npm run text:export`、`npm run text:check` 和普通 `npm run build` 通过。导出文本总数为 `6305`。
- Chromium `1280×720` 实际启动四档节奏会话，状态快照确认 `totalNotes=8 / 4 / 1 / 26`，计时采用 `audio_context`。四张运行画面已目视检查且没有 console/page error。
- 小鲤鱼故意零输入失败后，鱼饲料仍在、鱼未捕获、失败次数只增加一次，验证失败事务不会提前消耗剧情物品。

## 4. 交付边界

本轮只生成普通 Vite 生产构建，没有构建离线单文件，没有编辑 `demo/index.html`，也没有执行 Git 写操作或上传。

# 2026-08-28｜常规横屏室内门机制

## 1. 统一运行时

新增 `RpgInteriorDoorRuntime`，将门的视觉运动、关闭阻挡、开启后通行、拒绝反馈和人物经过门框时的同源前景遮挡收敛为一个场景组件。场景只声明门洞坐标、尺寸、运动方式、门扇偏移、阻挡线和前景裁片；门状态通过统一调试快照对外提供。

## 2. 场景接入

- 寝室正中下方出口使用单扇侧滑；开门达到通行阈值后再移动人物，过门时门框裁片位于人物上层。
- 图书馆沿用已有玻璃门滑动动画和门障碍，补入统一前景遮挡与调试字段。
- 食堂东南出口使用双扇侧滑，删除悬浮“出门”按钮。旧碰撞曾用一整段墙覆盖可见门洞，现只保留东侧实体墙；西侧门框作为前景通道，人物经过时被门框遮挡。
- 剧场正中下方主门使用双扇侧滑；门开启后从同一实体出口返回校园。

第四章电梯、202 教室门和食堂实时守出口仍属于剧情专用门，不由常规门组件替换。

## 3. 验证证据

- `npm run verify:rpg-interior-doors`：`28` 项断言通过。
- `npm run typecheck`：通过。
- `npm run build`：普通 Vite 生产构建通过，仅有既有大 chunk 警告。
- Chromium `1280×720`：四个场景的关门位置均完成目视检查；寝室完成单扇开启、通行延迟与过门遮挡；食堂和剧场完成双扇开启与通行检查，食堂成功返回校园；console/page error 为 `0`。

## 4. 交付边界

本轮没有构建离线单文件，没有编辑 `demo/index.html`，没有执行 Git 写操作或上传。浏览器临时截图和状态证据在记录结论后删除。

# Cross-Chapter Reality Residual Decay Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL — use superpowers:executing-plans to implement task-by-task.

**Goal:** 让食堂、剧院、启真湖和第四章共享“每条线索独立衰减”的残影运行时，同时保持浅色操作与深色观察的任意调查顺序。

**Architecture:** 永久观察事实继续由各章节控制器和 `SaveStore` 管理；残影剩余时长、透明度、消散阶段和暂停状态由不持久化的共享运行时模块计算。共享视觉合同固定深蓝环境、荧光蓝残影、原场景暖色与金色操作点；`RpgGameHost` 提供统一暂停上下文，四个 Phaser 场景把已有残影对象绑定到快照，不创建场景私有倒计时和私有色板。

**Tech Stack:** React 18、TypeScript、Phaser 3、现有 RPG bridge、现有 `GameState`／controller／`SaveStore`。

**范围限制：** 本计划不创建测试文件、测试依赖或专用 validator；不手工编辑 `demo/index.html`；不执行 Git 暂存、提交、合并或推送。
...
**Files:**
- Create: `src/scenes/rpg/RpgResidualMemory.ts`
- Create: `src/scenes/rpg/RpgRealityVisualContract.ts`
- Modify: `src/scenes/rpg/RpgInteractionContract.ts`

**Step 1: 定义目标规格与快照**

写入 `RpgResidualSpec`、`RpgResidualSnapshot`、`RpgResidualStage` 和统一配置表；配置表覆盖食堂、剧院、启真湖、104／105、电梯、导视板、Room 204 三分区、维修和配电目标。

**Step 2: 实现纯运行时记录模块**

提供 `observe(id)`、`tick(deltaMs, context)`、`pause(reason)`、`resume(reason)`、`clearScene(sceneId)` 和 `snapshot(id)`。`tick` 仅在浅色、场景可控、没有暂停原因时扣减时间。

**Step 3: 实现消散参数计算**

按 `stable → weakening → dissolving → expired` 输出 Alpha、像素缺口比例和消散格尺寸；`reducedMotion` 输出静态三段值。

**Step 4: 固定共享色板与形状合同**

在 `RpgRealityVisualContract.ts` 定义深色 `#020B16 / #06142D`、荧光蓝 `#67DDFF / #9AEAFF / #3BC5EF`、浅色暖层 `#FFE2A6` 与金色 `#FFD36F / #FFE8AE`。深色观察点统一为圆形，浅色操作点统一为菱形；消散过程中不改变荧光蓝色相。

**Step 5: 静态检查**

Run: `npm run typecheck`

Expected: 新模块和目标 ID 类型通过；`GameState` 与 `SaveStore` 没有新增残影计时字段。

### Task 2: 接入宿主暂停与 bridge 快照

**Files:**
- Modify: `src/scenes/rpg/RpgBridge.ts`
- Modify: `src/scenes/rpg/RpgGameHost.tsx`
- Modify: `src/scenes/rpg/RpgRuntimeDebug.ts`

**Step 1: 由宿主持有一个记录模块实例**

在当前 RPG 宿主生命周期内创建记录模块，通过 bridge 暴露只读快照、观察刷新和场景清理入口。禁止场景直接修改 `GameStore` 的残影时间。

**Step 2: 连接暂停条件**

任务抽屉、物品栏、对话、手机覆盖层、页面隐藏、场景暂停、加载和不可控演出分别注册命名暂停原因；多个原因重叠时只有全部解除才恢复计时。

**Step 3: 提供调试快照**

开发调试状态显示目标 ID、阶段和是否暂停；正式 HUD 不显示剩余毫秒。

**Step 4: 静态检查**

Run: `npm run typecheck`

Expected: bridge 类型一致，退出 RPG 后无残留计时器和订阅。

### Task 3: 统一食堂与剧院残影

**Files:**
- Modify: `src/scenes/rpg/CanteenInteriorScene.ts`
- Modify: `src/scenes/rpg/TheaterInteriorScene.ts`
- Modify: `src/modules/ChapterThreeCanteenController.ts`
- Modify: `src/modules/ChapterThreeTheaterController.ts`
- Modify: `src/data/chapter3-canteen.content.json`
- Modify: `src/data/chapter3-theater.content.json`

**Step 1: 绑定食堂目标**

菜单、三号窗口、三个出口推车和单车锁分别刷新自己的记录；浅色场景逐帧读取共享 Alpha、像素遮罩和荧光蓝颜色曲线。食堂可保留已有 `#071127` 深蓝底色。

**Step 2: 绑定剧院目标**

票务机、节目单顺序、道具箱／舞台经理和纸条路径使用独立记录；聚合视觉只组合渲染，不合并计时。剧院可保留已有 `#090C24` 深蓝底色，线索色改从共享合同读取。

**Step 3: 保留任意调查顺序**

控制器仅验证当前模式与实体动作自身条件；操作完成后观察入口仍能刷新临时残影并幂等写入永久事实。

**Step 4: 更新反馈文案**

删除隐含固定顺序的“观察后再操作”，改为说明当前模式职责；残影进入末段时最多显示一次“残影正在减弱”。

**Step 5: 静态检查**

Run: `npm run typecheck`

Expected: 食堂和剧院控制器、内容表与场景类型通过。

### Task 4: 统一启真湖残影

**Files:**
- Modify: `src/scenes/rpg/QizhenLakeScene.ts`
- Modify: `src/modules/ChapterThreeQizhenLakeController.ts`
- Modify: `src/data/chapter3-qizhen-lake.content.json`

**Step 1: 为各钓点建立独立记录**

倒影、钥匙、网框、饲料、鱼群、黑天鹅交换和最终捕纸目标分别刷新，不把整张湖面作为一个计时对象。

**Step 2: 绑定水面消散表现**

水纹与目标残影使用荧光蓝像素断层和透明度快照；启真湖保留已有 `#06142D` 深蓝覆盖。真实水面、船体、石块、浮标和碰撞保持原状。

**Step 3: 保护操作先行路径**

正确坐标、物品、距离和浅色模式满足时允许直接操作；事后仍可深色观察对应历史位置。

**Step 4: 静态检查**

Run: `npm run typecheck`

Expected: 湖区模式、钓取事务和追逐状态不持有残影倒计时。
...
- Modify: `src/modules/ChapterFourTemporalMazeController.ts`
- Modify: `src/components/temporal-maze/ElevatorTrackSyncGame.tsx`
- Modify: `src/components/temporal-maze/WayfindingBoardGame.tsx`
- Modify: `src/data/chapter4-temporal-maze.content.json`
- Modify: `src/styles/rpg.css`

**Step 1: 绑定 104／105、电梯与导视板**

三个目标使用 `60s`、`65s`、`60s` 配置；React 小游戏使用与 Phaser 相同的快照和暂停上下文。

**Step 2: 拆分 Room 204**

把十二个残影家具归入东、中、西三个运行分区，各自 `90s`；永久事实仍保持控制器兼容，正确家具继续锁定。

**Step 3: 绑定维修与配电**

故障诊断残影使用 `50s`，五区亮灯痕迹使用 `65s`；物理维修与面板操作维持浅色职责。

**Step 4: 接入分阶段视觉消散**

模式切换为 `420ms`；深色使用深蓝环境与荧光蓝圆形观察点，浅色恢复原场景暖色并显示金色菱形操作点。普通模式使用保持蓝色相的像素遮罩，`prefers-reduced-motion` 使用静态透明度台阶；残影层保持无碰撞、低于玩家。

**Step 5: 静态检查**

Run: `npm run typecheck`

Expected: 第四章控制器事实、React 小游戏和 Phaser 场景共享同一运行时类型。

### Task 6: 现有工程门禁与真实浏览器检查

**Files:**
- Modify: `progress.md`
- Generated by command only: `demo/index.html`
...
Expected: 生成可直接打开的 `demo/index.html`，构建无错误。

**Step 2: 手动检查食堂、剧院和启真湖**

通过现有 DEV 检查点检查观察先行、操作先行、单条刷新、暂停恢复、过期后重看、荧光蓝色相保持、暖色浅色场景与 `prefers-reduced-motion`。

**Step 3: 手动检查第四章**

依次检查 104／105、电梯、导视板、Room 204 三分区、维修和配电；确认数字倒计时没有进入玩家 HUD，深色圆形观察点、浅色金色菱形操作点和 `420ms` 过渡一致，人物、门、家具和碰撞不受残影层影响。

**Step 4: 记录证据并清理临时图像**

把命令结果、检查点、浏览器和视觉结论写入 `progress.md`；删除本轮生成的临时截图。

**Step 5: 保持 Git 交付边界**

在任何 Git 写操作前重新 fetch，并向用户呈现工作树、相对远端的本地提交、远端未包含提交三视图；等待用户确认精确提交范围。

# 2026-08-29｜第四章空白区域因果谜题实施计划

> **执行要求：** 使用现有 `selected-phaser-ui-20260721` 工作区分批实施；保留所有现有关卡，只向原本缺少有效交互的区域插入新谜题。不得新建测试文件或专用 validator，不得手改 `demo/index.html`。

**Goal:** 在 A1 前台、A3 301／302、A2 201／203／开放自习区插入六个可实际完成的谜题，使第四章的 A1 → A3 → 错位楼梯 → A2 时间阶段具有新的因果连接，同时保持每个阶段内部与浅／深模式的任意调查顺序。

**Architecture:** TypeScript 第四章 controller 与 `SaveStore` 继续持有唯一进度权威；Phaser 负责在原母图中显示生成素材、精确热区和接近交互；React 覆盖层负责拖拽、滑块、连线与路线排序。六个谜题共享一个数据模型和一套打开／提交事件，不建立第二套存档。素材由 image2 生成透明底原图，裁切、等比缩小后通过单一 placement manifest 放入场景。

**Tech Stack:** React、TypeScript、Phaser 3、现有 RPG bridge／EventBus、CSS、内置 image2、现有 Vite 单文件构建。
...
- Modify: `src/core/types.ts`
- Modify: `src/core/SaveStore.ts`
- Create: `src/modules/ChapterFourInsertedPuzzleModel.ts`
- Modify: `src/modules/ChapterFourTemporalMazeController.ts`

**Steps:**

1. 增加六个事实 ID、六个谜题 ID、每个谜题的输入合同与纯答案校验。
2. 增加打开谜题和提交结果两类 controller intent；错误结果只返回当前缺失关系，不清空局部界面状态。
3. 把值班牌、影像对齐、定位板校准、拓扑恢复和疏散路线接入现有阶段门槛。
4. 对已越过阶段的旧存档补齐兼容事实，保持当前位置、物品与章节完成状态。
5. 运行 `npm run typecheck`。

## Task 2｜生成、裁切与登记透明底像素素材

**Files:**
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_front_desk_duty_board_v01.png`
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_a3_archive_film_v01.png`
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_a3_alignment_scanner_v01.png`
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_a2_calibration_jig_v01.png`
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_a2_circuit_terminal_v01.png`
- Create: `src/assets/rpg/interiors/finale/chapter4-755/props/chapter4_a2_evacuation_map_v01.png`
- Create: `src/scenes/rpg/ChapterFourInsertedPuzzleAssets.ts`
- Modify: `src/scenes/rpg/FinaleEnvironmentTextures.ts`
- Modify: `src/scenes/rpg/ChapterFourWarmupAssets.ts`

**Steps:**

1. 分别以 A1、A2、A3 母图为风格参考生成六张独立透明底像素道具。
2. 检查 Alpha、视角、光向与缩小可读性；裁去透明空白并使用最近邻等比缩放输出正式文件。
3. 用单一 manifest 记录纹理 key、源文件、世界坐标、显示宽高、基线深度、实际热区和所属谜题。
4. 把素材加入现有阶段预热；没有达到对应阶段时不提前解码整组六图。
5. 运行 `npm run typecheck`。

## Task 3｜React 统一谜题覆盖层

**Files:**
- Create: `src/components/temporal-maze/ChapterFourInsertedPuzzleGame.tsx`
- Modify: `src/scenes/rpg/RpgGameHost.tsx`
- Modify: `src/styles/rpg.css`

**Steps:**

1. 建立统一覆盖层外框，维持 `960×540` RPG 壳内布局、关闭路径、键盘和指针输入隔离。
2. 值班牌使用三卡排序；档案索引使用三维筛选；影像对齐使用横移、纵移和旋转；定位板使用三轴校准；拓扑恢复使用五区连线；疏散路线使用四段排序。
3. 覆盖层显示相应生成素材的局部图，深色观察使用荧光蓝，浅色操作使用暖色与金色，且不显示答案或数字倒计时。
4. controller 接受后才关闭并写事实；错误结果保留当前输入。
5. 运行 `npm run typecheck`。

## Task 4｜场景放置、精确热区与因果入口

**Files:**
- Modify: `src/data/chapter4-three-floor-maze.layout.json`
- Modify: `src/data/ChapterFourInteractionContent.ts`
- Modify: `src/scenes/rpg/ChapterFourTemporalMazeScene.ts`
- Modify: `src/data/chapter4-temporal-maze.content.json`

**Steps:**

1. 在 A1 前台、A3 301／302、A2 201／203／开放自习区增加可见素材和与轮廓一致的热区。
2. 把六个原环境说明点升级为谜题入口；目标可任意调查，只有阶段收束检查因果事实。
3. 场景只提交 engine-neutral intent，并根据 controller cue 打开覆盖层；不在 Phaser 中写事实或复制答案。
4. 更新任务栏渐进提示和本地反馈，避免把正确顺序或答案直接写进目标文字。
5. 运行 `npm run typecheck`。

## Task 5｜现有门禁、真实导航与单文件验证
...
1. 运行 `npm run typecheck` 与 `npm run build:single`，不新增自动化测试或专用 validator。
2. 真实浏览器从第四章 A1 完成值班牌，进入 A3 完成 301／302，经过错位楼梯进入 A2，再以不同调查顺序完成 201／203／开放自习区。
3. 在浅色先行和深色先行两条路径中各抽查至少一个新增谜题；确认人物层、门、碰撞和原谜题均未被素材覆盖。
4. 在 `1280×720` 与一个非 16:9 视口检查素材缩放、热区、覆盖层和任务提示；再直接打开生成的单文件验证同一入口。
5. 删除临时浏览器截图，把实际命令、导航路径和发现写入 `progress.md`。
6. Git 仍停在交付门前：重新 fetch 并呈现工作树、本地相对远端提交、远端相对本地提交三视图，等待用户确认范围后再写 Git。

## 2026-08-29｜第四章正式“灿若星辰灯”收束接入

### 输入与保留范围

- 用户提供的压缩包只保留五张 `1024 × 1536 RGBA` 分层 PNG；旧 Godot、Three.js、Next.js 工程、依赖缓存、构建产物、Git 历史、预览和重复副本不进入运行时。
- 原始压缩包在同目录备份区和恢复区各保留一份完整副本；原路径改为含五张正式素材与清理清单的精简包。

### 运行时闭包

- `ChapterFourStarLampClosure` 复用正式分层素材和原 `6200ms` 亮灯顺序，作为现有 React/TypeScript 宿主中的表现层，不创建新的游戏引擎或第二套章节状态。
- `ChapterFourClosureSessionRegistry` 为一次实际播放签发 runtime-only proof；`ChapterFourTemporalMazeController` 校验素材 ID、序列 ID、consumer 模块、session 与 completion event 后，才把 `exterior_closure` 原子推进到 `complete`。
- SaveStore v32 只恢复字段闭合的正式完成态。v31 及更早版本、裸完成布尔值或缺少灯光／签到／最后一分钟事实的记录会回落到可重播的楼外安全点。

### 验收结果

- TypeScript、章节文本、离线单文件构建、单文件结构验证和 diff whitespace 检查通过。
- 本地 HTTP 浏览器实测显示正式球形灯体、轮廓 LED、灯芯和光晕分层；完整播放后自动回到手机主页，任务栏显示第四章 `章节完成 1/1`，浏览器没有 console/page error。
- 离线单文件：`258649544` 字节，SHA-256 `3ff9e53b71b65a5f2125ef0fc28ff8160440415b7bbaa7e9b9794ad708f1e355`。
