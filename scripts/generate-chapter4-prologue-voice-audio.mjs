...
const contentPath = join(root, "src/data/chapter4-prologue-voice.audio.content.json");
const generatedPath = join(root, "src/data/chapter4-prologue-voice.audio.generated.json");
const checkpointPath = join(
  root,
  "node_modules/.cache/seven-fifty-five/chapter4-prologue-voice.audio.checkpoint.json"
);
const audioRoot = join(root, "src/assets/audio");
const voiceDir = join(audioRoot, "chapter4/prologue");
...
const tempDir = mkdtempSync(join(tmpdir(), "seven-fifty-five-chapter4-prologue-voice-"));
...
function probeAudio(path) {
  if (!existsSync(path)) throw new Error(`Missing Chapter 4 prologue voice asset: ${path}`);
  ...
  if (!stream || stream.codec_name !== "mp3") {
    throw new Error(`Chapter 4 prologue voice asset must be mp3: ${path}`);
  }
  if (!Number.isFinite(durationMs) || durationMs < 250 || durationMs > 30000) {
    throw new Error(`Unexpected Chapter 4 prologue voice duration ${durationMs}ms: ${path}`);
  }
  const sampleRate = Number(stream.sample_rate);
  const channels = Number(stream.channels);
  if (sampleRate !== 32000 || channels !== 1) {
    throw new Error(`Chapter 4 prologue voice must be 32000Hz mono: ${path}`);
  }
...
function validateContent() {
  if (content.version !== 1 || !Array.isArray(content.lines) || content.lines.length === 0) {
    throw new Error("Invalid Chapter 4 prologue voice-line catalog.");
  }
  const keys = new Set();
  ...
  for (const line of content.lines) {
    if (
      ...
      || !LINE_KEY_PATTERN.test(line.key)
    ...
    if (keys.has(line.key)) throw new Error(`Duplicate Chapter 4 prologue voice key: ${line.key}`);
    if (subtitles.has(line.subtitleZh)) throw new Error(`Duplicate Chapter 4 prologue subtitle: ${line.subtitleZh}`);
    keys.add(line.key);
...
function main() {
  ...
  if (Object.keys(assets).length !== content.lines.length) {
    throw new Error(`Chapter 4 prologue voice manifest is incomplete: ${Object.keys(assets).length}/${content.lines.length}`);
  }
