...
try {
  ...
  for (const [label, state, expectedFacts, expectedHeld, expectedPhase] of [
    ...
    ["installed", installedState, bakeryFactOrder, false, "room204_restore"]
  ]) {
  ...
  assert(
    sameJson(repairedZhuAnswers.chapter4.zhuQuestionAnswers, {
      purpose: null,
      person: null
    }) && !repairedZhuAnswers.chapter4.factIds.includes("zhu_two_questions_answered"),
    "missing Zhu answers must clear forged completion without inventing selections"
  );
  ...
  assert(
    sameJson(clearedUncommittedZhuAnswers.chapter4.zhuQuestionAnswers, { purpose: "serve_public", person: "clear_minded" }) && !clearedUncommittedZhuAnswers.chapter4.factIds.includes("zhu_two_questions_answered"),
    "valid uncommitted answer drafts must remain drafts without fabricating completion"
  );
  ...
  assert(diagnosedState.items.shortPryBar && !diagnosedState.items.universalLubricatingOil, "diagnosis grants the pry bar; oil requires opening the wheel cover");
  ...
  for (const legacyTargetId of ["a1_bakery_back_pry_bar", "a1_cleaning_cart_oil_bottle", "a1_hall_clock_gear"]) {
    ...
    assertZeroWriteRejection(
      maintenanceStore.getState(),
      fixture.intent,
      legacyTargetId === "a1_hall_clock_gear" ? "wrong_item" : "locked",
      fixture.target,
  ...
  assert(diagnosedLoaded.items.shortPryBar && !diagnosedLoaded.items.universalLubricatingOil, "diagnosed save must preserve the pry bar without fabricating covered oil");
  ...
  assert(
    selectQuestViewModel(blackoutState).objective === "点亮追赶所需的通路"
      && !/(?:0b|mask|掩码|01101|1101|东走廊.*教室.*面包坊)/i.test(selectQuestViewModel(blackoutState).objective),
    "Task11 blackout must state the route objective without publishing the solution mask or switch sequence"
  );
  ...
  const panelPreviewStore = createGameStore(blackoutWithoutProjectionComposite);
  const panelPreviewController = new ChapterFourTemporalMazeController(panelPreviewStore, new EventBus());
  const previewSnapshot = snapshot(panelPreviewStore.getState());
  const previewResult = panelPreviewController.resolve755Intent({
    type: "open_power_panel", targetId: "a1_power_panel", spatial: validSpatial
  }, powerPanelRuntimeTarget);
  assert(previewResult.accepted && !previewResult.changed, "panel inspection is read-only even before deferred evidence is complete");
  assert(snapshot(panelPreviewStore.getState()) === previewSnapshot, "previewing a locked progression panel must write no state");
  for (const requiredFact of ["room204_projection_composite_completed", "room202_endpoint_inferred", "a2_power_topology_recovered", "a2_evacuation_route_confirmed"]) {
    const missingEvidenceState = {
      ...blackoutState,
      chapter4: {
        ...blackoutState.chapter4,
        lightGrid: { mask: 13, locked: false },
        factIds: blackoutState.chapter4.factIds.filter(fact => fact !== requiredFact)
      }
    };
    assertZeroWriteRejection(missingEvidenceState, {
      type: "lock_light_grid", targetId: "a1_power_panel", spatial: validSpatial
    }, "locked", powerPanelRuntimeTarget, "solved panel missing " + requiredFact);
  }
  ...
  assert(
    selectQuestViewModel(finalChaseState).objective === "追进 202，关好门",
    "Task11 chase must expose the approved Room202 door goal, with successful entry verified below through the controller"
  );
