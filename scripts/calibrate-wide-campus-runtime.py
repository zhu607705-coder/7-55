...
# The canteen gate is also part of the one shared campus coordinate system.
# It lands on the visible forecourt, inside the walkability polygon and clear
# of the billboard, vending machine, bins, and utility cabinet.
CANTEEN_GATE = {"x": 756, "y": 756, "radius": 88}
CANTEEN_APPROACH = {"x": 756, "y": 756}

# The theater entrance sits behind a narrow gap in the planted frontage.  Its
# route follows the visible paving from the promenade to the stair landing.
# The strip deliberately excludes the surrounding facade, lawn, fence, and
# flower beds.
THEATER_CORRIDOR = {
    "left": 7672,
    "right": 7788,
...
# Qizhen Lake begins from the open roadside pavement west of the theater.  The
# existing promenade already covers these points; keeping the gate in runtime
# metadata makes the campus transition independently verifiable.
...
def build_walkability_grid() -> np.ndarray:
    ...
    grid_rect(
        grid,
        left=THEATER_CORRIDOR["left"],
        right=THEATER_CORRIDOR["right"],
        top=THEATER_CORRIDOR["top"],
        bottom=THEATER_CORRIDOR["bottom"],
        walkable=True,
    )
...
def main() -> None:
    ...
    runtime["canteenGate"] = CANTEEN_GATE
    ...
    runtime["walkability"] = {
        ...
        "canteenApproach": CANTEEN_APPROACH,
    ...
    print(
        ...
        f"canteenGate={CANTEEN_GATE['x']},{CANTEEN_GATE['y']} "
