...
function validateTask7RuntimeSources(errors) {
  ...
  if (new Set(suppressedHandshakeIntents).size !== suppressedHandshakeIntents.length || !sameArray([...suppressedHandshakeIntents].sort(), [
  ...
  if (!sameArray(task9ActionableIds, [
    ...
    errors.push("Task 9 Scene actionable target allowlist must include the front desk, A2 safety officer, A3 teacher, both classroom gates, A3 reference, A2 residual, positioning plate, and the four grouped Room204 targets");
  }
  ...
  const realityAtmosphereBlock = scene.match(
    /private syncRealityModeVisuals\([\s\S]*?private destroyInsertedPuzzleProps\(\)/
  )?.[0] ?? "";
  ...
  const room204GroupTargets = interaction.match(/const ROOM204_GROUP_TARGETS[\s\S]*?(?=\nconst |$)/)?.[0] ?? interaction;
  if (!/room204_group_available/.test(room204GroupTargets)
    || !/requiredMode: "light"/.test(room204GroupTargets)
    || !/case "place_room204_group"[\s\S]*?mode !== "light"[\s\S]*?reject\("wrong_mode"\)/.test(controller)) {
    errors.push("Room204 grouped physical placement must reject dark observation through the shared controller");
  }
  ...
  const room204PersistedFactOrder = [...room204FactOrderBlock.matchAll(/"([^"]+)"/g)]
    .map((match) => match[1]);
  if (!sameArray(room204PersistedFactOrder, EXPECTED_ROOM204_FACTS.filter(fact => !["elevator_a2_call_record_observed", "elevator_a3_arrival_record_observed", "elevator_stop_chain_reconstructed", "a1_duty_board_reconstructed", "a3_archive_film_retrieved", "a3_media_alignment_completed", "a2_positioning_plate_calibrated", "a2_power_topology_recovered", "a2_evacuation_route_confirmed"].includes(fact)))) {
  ...
  if (!/case "inspect_cart_wheel"[\s\S]*?chapter4_maintenance_diagnosis_requested[\s\S]*?acceptReadOnly/.test(maintenanceControllerBlock)
    ...
    || !/case "complete_maintenance_diagnosis"[\s\S]*?appendFact\(chapter, "cart_wheel_inspected"\)[\s\S]*?shortPryBar:\s*true/.test(maintenanceControllerBlock)
  ...
  if (!/const cartWheelCoverOpened\s*=\s*hasFact\("cart_wheel_cover_opened"\)/.test(maintenanceItemBlock)
  ...
  const maintenancePushBlock = maintenanceCreationBlock.match(
    /private ensureMaintenancePushCharacter[\s\S]*?private createMaintenanceGuardRuntime/
  )?.[0] ?? "";
  if (!/this\.maintenancePushCompleted\s*\|\|\s*this\.maintenancePushTween/.test(maintenancePushBlock)
    ...
    || !/const push = MAINTENANCE_RUNTIME\.repairedPush[\s\S]*?push\.animationId/.test(maintenancePushBlock)
  ...
  if (!/private isStoryInputLocked\(\)[\s\S]*?finalClockDragActive/.test(scene)
    ...
    || !/chapter4OverlayBlocked\s*=\s*chapter4InvestigationOpen\s*\|\|\s*chapter4InteractionBlocked\s*\|\|\s*chapter4PowerPanelOpen/.test(host)
  ...
  if (!/allowScenePointer[\s\S]*?this\.alumniPanel !== null/.test(scene)
    ...
    || !/(?:closeButton|actionButton)[\s\S]*?setInteractive[\s\S]*?closeAlumniPanel/.test(scene)
    || !/ChapterFourStarLampClosure/.test(host)
    || !/ChapterFourExteriorQuestions/.test(fs.readFileSync(new URL("../src/components/temporal-maze/ChapterFourStarLampClosure.tsx", import.meta.url), "utf8"))
  ...
  if (!/"arming"[\s\S]*?"running"[\s\S]*?"portal_transfer"[\s\S]*?"finish_pending"[\s\S]*?"failure_pending"[\s\S]*?"complete"/.test(finalChaseModel)
    ...
    || !/startGraceMs:\s*2000/.test(finalChaseModel)
    || !/playerSpeed:\s*208/.test(finalChaseModel)
    || !/guardSpeed:\s*174/.test(finalChaseModel)
  ...
  if (!/case "reach_202_threshold"[\s\S]*?\["expectedAttempt"\]/.test(controller)
    || !/case "fail_chase"[\s\S]*?hasExactKeys\(value, value\.failureFloor === undefined \? \["type", "expectedAttempt"\] : \["type", "expectedAttempt", "failureFloor"\]\)/.test(controller)
  ...
  if (!/phase === "final_chase"[\s\S]*?savedFloor === "A2" && stairwellComplete[\s\S]*?floor: "A2", roomId: "a2_corridor"[\s\S]*?floor: "A1", roomId: "a1_lobby"/.test(saveStore)
...
function validate(content) {
  ...
  if (!tasks) {
    ...
    if (Object.keys(tasks).length !== 41 || activeTaskEntries.length !== 40) {
      errors.push("tasks must contain 40 active tasks plus chapter_complete");
    }
    ...
    if (activeHintCount !== 117) {
      errors.push("tasks must expose 117 hints; saved answers and completion must not invent another objective");
    }
  ...
  if (!isRecord(lightGrid)) {
    ...
    if (lightGrid.initialMask !== 6) errors.push("lightGrid.initialMask must use the revised arrangement mask 6");
    ...
    const verifiedSolution = Array.isArray(lightGrid.verifiedSolutionZoneIds)
      ...
      : [];
    if (!sameArray(verifiedSolution, ["east_corridor", "classroom_zone", "bakery_back_area"])) {
      errors.push("lightGrid.verifiedSolutionZoneIds must match the revised three-toggle solution");
    }
  ...
  if (!isRecord(guard)) {
    ...
    if (!isRecord(guard.finalChase)
      ...
      || guard.finalChase.guardSpeed !== 174
      || guard.finalChase.startGraceMs !== 2000
