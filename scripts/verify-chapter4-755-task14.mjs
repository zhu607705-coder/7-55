...
assert(taskEntries.length === 41 && activeTaskEntries.length === 40, "Task 14 must define 40 active tasks including the added chase stairwell plus chapter_complete");
...
assert(activeHints.length === 117, "Task 14 must expose 117 hints including the chase stairwell, without receipt hints");
assert(
...
assert(
  ["a1_guard_chase_body", "a2_guard_chase_body", "a2_room204_disordered_furniture", "a2_room202_recovery_barrier"].every(id =>
    (sceneSource.match(/const RUNTIME_MANAGED_DYNAMIC_COLLISION_IDS[\s\S]*?new Set\(\[([\s\S]*?)\]\)/)?.[1] ?? "").includes(`"${id}"`))
...
try {
  ...
  const visibleC4Ids = DEVELOPER_CHECKPOINTS
    ...
    .map((entry) => entry.id);
  assert(new Set(visibleC4Ids).size === visibleC4Ids.length && sameJson([...visibleC4Ids].sort(), [...GAMEPLAY_IDS].sort()), "visible Chapter 4 DEV entries must contain each canonical gameplay ID exactly once");
