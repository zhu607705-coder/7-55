> 2026-09-06 当前运行规则更新：以下历史音乐与三列谱面设计保留作参考；当前剧情使用 lake-rhythm-v4 四次递进谱面，实际参数以 src/data/chapter3-qizhen-fishing.charts.json 为准。

## 四次剧情钓鱼难度递进

| 顺序 | 捕获物 | 基础速度 | 四拍时值比例 | 节奏 | 判定窗口倍率 | 鱼影运动倍率 |
|---|---|---|---|---|---|---|
| 1 | 柜门钥匙 | 80 BPM | 1 / 1 / 1 / 1 | 均匀慢拍 | 1.20 | 0.80 |
| 2 | 破损网框 | 96 BPM | 1.15 / 0.85 / 1.15 / 0.85 | 长短交替 | 1.05 | 0.95 |
| 3 | 小鲤鱼 | 112 BPM | 1.10 / 0.90 / 0.85 / 1.15 | 变拍追鱼 | 0.95 | 1.05 |
| 4 | 纸条 | 128 BPM | 0.90 / 0.80 / 0.80 / 1.50 | 短拍连奏 | 0.80 | 1.15 |

保持稳、放、收、提四步和每次八次起鱼。预备小节仍有四个均匀拍，进入正式遛鱼后采用各自长短节拍。节拍音、装饰音、光标、鱼的猛拽窗口和起鱼判定共用同一时值映射。奖励验证从可信剧情谱面按真实输入重放，不能借用第一关的宽松参数取得后续奖励。失败和取消保留物品，不涉及无尽模式。
