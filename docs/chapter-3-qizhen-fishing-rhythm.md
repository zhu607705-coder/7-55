...
**判定时钟原则**：生成音乐不承担判定时钟。判定、固定节奏轨道、浮标水纹与输入统一引用同一个单调时钟 `now()`（场景注入 Web Audio `AudioContext.currentTime`，不可用时回退 `performance.now()/1000`）：`t_n = t_0 + 0.625 × b_n`。禁止用 `setTimeout` 累积计时。
...
- 固定节奏轨道使用 `scrollFactor(0)`，逻辑视口范围为 `x=170..790`、`y=295..357`；高对比判定线固定在 `x=288`，并显示文字标签「判定线」。
- 音符提前 2 拍（`leadSeconds`）从轨道右侧 `x=760` 出现，向左移动并在目标时刻抵达判定线。超过目标时刻但仍处于 good 窗口时，音符继续移到判定线左侧，使「稍早 / 稍晚」具有空间方向。
- 浮标世界坐标仍是抛竿落点和场景反馈锚点。半径 72 px 到 14 px 的水纹环保留为低透明度辅助提示，不得成为唯一可见判定参照。
- 图标语言：left = 柳树枝左桨图标 + `A`；hook = 鱼钩图标 + `S`；right = 三角牌右桨图标 + `D`。触屏按钮与键盘保持相同的左中右顺序。
- 长按音符命中后锁定在判定线上，判定线右侧显示水平持续时长进度条，松开或完成时立即清除。
...
- `prefers-reduced-motion`：取消连续缩放和判定线脉冲；轨道音符按离散位置向判定线移动，水纹环按 `72→48→32→14` 离散半径变化，保留图标与整数拍数字倒计时。
...
- `src/scenes/rpg/QizhenFishingRhythmVisual.ts` — 固定节奏轨道/判定线/音符图标/持续时长条/辅助水纹/状态栏/钓线（§9.2）
...
8. 音频被浏览器阻止时，固定判定线、轨道音符、辅助水纹与低音量程序化点击仍允许完成主线。
9. `prefers-reduced-motion` 下取消大幅缩放与脉冲，保留固定判定线、离散音符位置、离散水纹半径、图标与数字倒计时。
