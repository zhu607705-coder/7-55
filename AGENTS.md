...
- Kayak input is stroke-based. Desktop uses `A` or left arrow for the left paddle and `D` or right arrow for the right paddle; coarse-pointer layouts render explicit left-paddle and right-paddle controls. Alternating strokes generate forward speed and recover balance. Repeated same-side strokes rotate the hull and increase roll. Exceeding the balance limit causes a visible capsize, increments the persisted capsize count, and restores the latest safe zone checkpoint.
