...
- The formal Chapter 4 room-204 route requires the elevator and the approved Three.js stair puzzle as causal progression gates. Elevator-history observation and the six-second passenger-trace alignment may be completed in either order; the successful calibration result still gates formal floor travel. A3 reference observation, A2 residual observation, and visible furniture placement remain an unordered completion set. The approved misaligned-stair levels still gate relocation to the A2 corridor, while echo observation and structure adjustment may be attempted in either order. A generic floor selector must never bypass or replace these gates.
...
- `深色观察` records reflection locations and item silhouettes. `浅色操作` performs boarding, casting, pickup, combination, feeding, and capture. Observation facts provide coordinates, outlines, timing assistance, and explanation; they never gate whether a visible light-mode target can be attempted. A correct light-mode cast may succeed without an earlier observation when the coordinate, item, distance, zone, and story phase are valid. Casting directly at the paper remains a readable failure and never advances the puzzle.
...
- Every item-use attempt reports accepted, missed target, wrong item, too far, wrong mode, story lock, or direct-paper failure. An unobserved target may omit observation assistance, but lack of an observation fact alone cannot reject a valid physical attempt. Items disappear immediately after their final successful use, including stale selected or dragged state.
...
- The global mode rule is fixed: `深色观察` reads clues, residual images, routes, rhythms, and abnormal state; it never performs physical pickup, drag, cleaning, payment, pushing, scanning, or device activation. `浅色操作` performs those physical actions whenever the item, distance, target, room, and story-phase requirements are valid. No light-mode action may require an earlier dark-mode fact solely to become available, and no dark-mode observation may require an earlier light-mode action solely to become available.
- Reality-mode targets remain present in both modes whenever their room, phase, and visibility conditions are satisfied. Both modes route the attempt through the controller and return a visible result. A mode with no world-changing effect returns mode-specific feedback without consuming an item or clearing a drag selection.
- Observation facts such as `*Observed`, `*Read`, and `*DarkClueRead` are hint and evidence facts by default. A final puzzle may require an unordered set of observation and operation facts, but intermediate interactions cannot impose a fixed `dark → light` or `light → dark` order. Entering a scene, completing a round, failing a round, or returning to a checkpoint must not overwrite the player's selected reality mode.
...
- Chapter 4 reality modes require a scene-level visual distinction in addition to the toggle label. `深色观察` uses a restrained cool-blue wash, edge vignette, scan texture, and cyan circular clue anchors; `浅色操作` uses a restrained warm wash and gold diamond action anchors. The atmosphere and anchor layers remain below the player and shared HUD, inactive-mode anchors stay dim but readable, and mode changes crossfade without blocking control.
...
- Run `npm run validate:quick` after localized source or content changes, `npm run validate:critical` after controller, save, bridge, shared input, Phaser behavior, or progression changes, and `npm run validate:release` before delivery. The release suite owns the single TypeScript pass, production build, Chromium smoke route, offline single-file build, and artifact verification.
- `.github/workflows/web-ci.yml` is the canonical repository CI and calls the same `npm run validate:release` entry used locally. Retired Godot sources and exports remain outside the active delivery gate.
- Keep validators dependency-light and executable through Node and existing build tools. Python files under `scripts/` are reserved for deterministic map or art generation and calibration; do not add Python test cases. Third-party test frameworks and test-only dependencies remain excluded unless the user explicitly asks to restore them.
