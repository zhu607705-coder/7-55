...
- Campus exploration currently uses the approved `13668px × 1084px` side-view loop panorama at `src/assets/rpg/campus/zijingang_campus_loop_panorama.png`. It restores the pseudo-`2.5D` depth curve, a source-pixel foot box, and one connected foreground road. The active Godot migration must reproduce this visual, collision, input, checkpoint, and story contract before replacing the Phaser fallback.
- `src/assets/rpg/campus/zijingang_campus_plate.png` and `src/assets/rpg/campus/source/topdown/` remain reference inputs for the future Godot campus rebuild. They are not loaded by the current Phaser campus scene and must not overwrite the loop panorama or its runtime manifest.
- The loop panorama inserts a `1924px` Qizhen Lake segment at old panorama `x=8400`, immediately east of the first theater. Coordinates at or east of the split move right by `1924px`. The theater stays at its original coordinate; Qizhen Lake owns the inserted segment; the Basic Library and later eastern landmarks use shifted coordinates.
- `npm run map:zijingang:rebuild` rebuilds the Qizhen insertion from the checked-in legacy panorama source and the approved Qizhen plate, then regenerates the compressed collision mask and runtime hashes. `npm run map:zijingang:walkability` only recalibrates collision and metadata from the already generated loop panorama. Square-map builders and the retained top-down calibration command must not overwrite the active loop asset.
- The runtime manifest owns world size, insertion bounds, shifted story gates, collision data, depth curve, and bidirectional wrap points. The player may wrap only on the continuous foreground road; a short camera fade hides the map boundary and preserves checkpoint, inventory, quest, and save state.
- The campus camera follows the player, supports bounded pointer panning and discrete zoom, and never stretches the map. Nominal default zoom is `1.1`; the campus backing-store scale multiplies the actual camera value without changing the visible world extent. The actor uses the shared strong depth curve and grows uniformly toward the foreground. The overview minimap stays removed.
...
- Every active panorama asset uses the same side-view pixel density and lighting direction. Dynamic actors may change uniform scale only through the shared depth curve; unequal `scaleX` and `scaleY`, mixed viewpoints inside one rendered segment, and scene-local perspective constants are prohibited.
- Campus walkability comes from the compressed `4px` cell mask embedded in `src/data/maps/zijingang-campus-runtime.json`. Its dimensions must match the loop panorama. The full foreground road is connected, visible entrance corridors are explicitly opened, foreground props remain solid, and the player foot box must fit every seam and both wrap arrivals.
- Side-view building facades remain background art. Collision comes from the calibrated ground mask and measured foreground props; top-down landmark rectangles and former top-down occlusion polygons must not be attached to the loop panorama.
- The first completed theater flow exits at `campus_theater_junction` and runs one Qizhen approach cinematic while `qizhenLake.locationBriefingSeen` is false. The wet paper and residual trail move east toward the inserted lakeside segment, stop before revealing the final location answer, and then hand control to the existing three-source phone investigation. Reload after the acknowledgement skips this one-time cinematic; reload before acknowledgement replays it from the safe theater checkpoint.
- Future campus-art replacements must preserve the loop topology, Qizhen-after-theater sequence, source-pixel gate calibration, road wrap, strong depth curve, and engine-neutral TypeScript state authority. MiniMax remains audio-only and must not generate campus artwork.
