...
- Fishing interactions whose visible target is a water ripple use kayak-to-ripple distance only. Kayak heading remains a movement and chase value and must not become a hidden prerequisite for casting, baiting, or starting the rhythm session. Entity interactions with a readable front, such as the dock locker and black swan, may still require facing.
...
- Inventory drag feedback remains owned by the shared React dock. Phaser scenes report exact world-space drop bounds, accepted item ids, distance rules, facing rules, and rejection reasons so the dock can display accepted, missed target, wrong item, too far, wrong facing, and locked states consistently.
...
- `src/scenes/rpg/RpgInteractionContract.ts` is the shared authority for reality-mode labels, item drop bounds, distance limits, facing rules, accepted item ids, required modes, target ordering, and drop rejection classes. Every Phaser RPG scene consumes the same authored target data.
