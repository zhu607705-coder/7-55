...
- Old `reflection_hunt`, `sign_alignment`, `decoy_setup`, `mist_timing`, and `chase_ready` saves migrate into safe kayak checkpoints. Completed old lake progress must not regress to location search. `qizhen_lake` remains outside `GODOT_ACCEPTED_SCENES` until all four zones, kayak input, item drag, save/reload, chase, and Blink/Gecko/WebKit validation pass.
