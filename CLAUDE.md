...
- Starting exercise must immediately point to the new `方向校准` home-screen notice. Collecting its triangle must then advance the current objective and feedback to Weather, where the water drop becomes available. Keep these as two sequential cues instead of exposing later movement-puzzle steps at exercise start. The active Tiyi exercise layer must fully cover the baked blue start button at every supported phone scale.
