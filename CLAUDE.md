...
- The user-selected repository panorama is the campus spatial and visual source of truth. It is one `5016px × 5016px` PNG stored at `src/assets/rpg/campus/zijingang_campus_plate.png`; runtime code must never position or scale source tiles independently.
- Rebuild it from `~/Downloads/大地图/` only with `npm run map:zijingang:rebuild`. Source order is row-major `(1,1)` through `(4,4)`. Twelve tiles are already `1254px` square; the four non-square tiles are normalized to `1254px` before the three vertical and three horizontal joins are replaced by continuous connector roads.
...
- The campus camera follows the player in both axes, supports bounded pointer panning and discrete zoom, and never stretches the map. Default zoom is `0.375`, exposing a `2560px × 1440px` world area in the logical `960px × 540px` canvas while the canonical player stays `48px × 64px`. A minimap may show the complete north-up plate, but it cannot replace the playable world.
...
- Campus walkability comes from the compressed `4px` cell mask embedded in `src/data/maps/zijingang-campus-runtime.json`. The mask is rebuilt from the final `5016 × 5016` plate, and runtime collision must keep the player foot box on its connected road region. Alternate-size masks must never be attached to this panorama.
