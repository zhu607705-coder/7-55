...
- Every current and future controllable RPG player sprite must use the helpers in `src/scenes/rpg/RpgPlayerTextures.ts`. Runtime frames are `96px × 128px`; that shared module alone owns the compensating interior display scale, the campus `2×` visual baseline, the campus perspective curve, the fixed world-space foot collision box, the four-phase walk cycle, and the dynamic name-label offset. Scene files must not define their own player scale, body geometry, or frame cadence.
...
- Campus exploration uses the approved wide pixel-art panorama with a controlled `2.5D` depth treatment. The player moves in four directions on the generated road network; lower foreground positions render larger and upper distant positions render smaller, while Phaser owns the player, collisions, depth, labels, hotspots, quest markers, camera, and state transitions.
...
- Campus walkability comes from the compressed `4px` cell mask embedded in `src/data/maps/zijingang-campus-runtime.json`. Its dimensions must match the active wide plate. The continuous foreground road and sidewalk starts at source `y=864`, every visible entrance approach is source-pixel calibrated, and lane markings, drains, shadows, or scene joins must never split the connected promenade. Buildings, fences, flower beds, water, and measured foreground props remain blocked. Alternate-size or texture-classified masks must never be attached to this panorama.
- The foundation-library entrance follows the visible L-shaped paving: the right-side gap rises from the promenade, then turns west across the door forecourt. Its interaction gate is `(9000,770)`, its safe checkpoint is `(9070,770)`, and the central and east flower beds remain blocked. Three other angled public paths preserve their visible source-pixel outlines instead of using broad rectangular openings.
- A campus building entrance must be calibrated against the visible source pixels: its landmark anchor belongs on the matching building, its interaction marker belongs on the visible entrance path, and its checkpoint must fit the player foot box on a connected walkable approach. A successful route transition alone does not validate entrance placement.
- Future GPT Image replacements must preserve the approved wide-panorama viewpoint, active coordinate system, seam alignment, and landmark positions. MiniMax remains audio-only and must not generate campus artwork.
...
- The top of the Phaser canvas is reserved for the shared task bar. RPG feedback subtitles are rendered only by `RpgSubtitleLayer` in the shell-level lower safe row defined by `src/scenes/rpg/RpgHudLayout.ts`; Phaser scenes emit `rpg_subtitle` and must not create their own feedback text box. The interaction hint remains in the final canvas row. Scene files must not restore local top-positioned feedback coordinates or render two subtitle surfaces at once.
...
- Every spoken voice asset ships in English. Every on-screen voice subtitle uses Chinese and has exactly one owner: a scene dialogue surface when that scene already renders the line, or the shared `ToastLayer` for voice-only cues. Set `subtitleSurface: "scene"` for scene-owned lines and never emit the same text through both surfaces. All subtitle owners use the bottom-center safe row, `game-subtitle-frame` box contract, shared stepped entry/exit motion, and tone-only accent changes; audio-derived display duration remains unchanged. Presentation cards and functional form feedback are separate UI categories. TTS source text, voice language, generated manifests, and runtime audio must use English.
...
- After the first geolocation failure advances the story beyond the prologue, the `checkin` scene is inaccessible. The baked check-in card inside `浙大钉 → 学在浙大` remains visible as static content with no hotspot, button semantics, focus target, or denial toast. Reopening this scene requires a future explicit story gate.
...
- Starting exercise opens both the `方向校准` home-screen notice and the collectible Weather drop. Triangle and weather-water collection are independent and may be completed in either order; the task model points to whichever source remains. The active Tiyi exercise layer must fully cover the baked blue start button at every supported phone scale.
...
- The `022` receipt gap accepts the retained right arrow on the player's first physical visit to the seat. Receipt recovery does not depend on backpack inspection, CC98 investigation, or a later library phase, and it preserves the current phase.
- Library world interactions animate on their physical object: shelf motion, backpack motion, paper movement, machine scan lines, and terminal indicators. Do not add full-object rectangular highlight panels, generic target flashes, or full-screen presentation cards when an object-local animation exists. Bottom-safe-zone dialogue and feedback, physical device labels, and development-only collision overlays remain allowed. Collision and interaction availability must follow the visible motion; re-entry restores completed object positions and automatically reopens acquired unread evidence without replaying finished motion.
- Administrative certification in the library must use a readable physical device. The lost-and-found identity target is a pixel stamp machine with an input tray, scan state, mechanical stamp head and lever, stamped paper output, and object-local feedback; a standalone status rectangle cannot represent the interaction.
- Required evidence chain: reservation note → CC98 `23`-floor investigation → catalog number `755` → archived leave rule; dimmed photo → item report → stamped non-person proof; persistent right arrow → `022` receipt; Tiyi form `7 / 47 / 3` → presence proof; four CC98 uploads → four-digit BD heat password → top-ten rank `01`; three library recovery slots → eviction PASS; PASS on the RPG backpack → restored seat and `022` dialogue.
- The CC98 BD step must define `bd` as `帮顶` before interaction. After all four evidence items are uploaded, show eight numbered reply posts. Selecting a post's `bd` appends its digit to a persistent four-slot password; the source order is archived-rule requirement count, stamped identity pass count, seat-number final digit, and arrival minutes. Controller validation owns selection, attempts, and the single `04 → 01` rank transition. The retired A/C/E reply filter must not appear as a parallel solution.
- The Tiyi presence audit must show the collected evidence content, not generic source-ready badges: the two entrance timestamps for subtraction, the CC98 floor-23 author edit containing public notice number `47`, and the three archived-rule requirements for counting. Each input labels its source and operation; missing sources identify the exact place to inspect without printing an unavailable answer.
...
- CC98 keeps exactly five optional `ac01` joke replies in the new `23`-floor investigation post. They do not gate progress. After all four evidence uploads are validated, eight new numbered replies open the BD password step; only the controller-validated four-post sequence advances rank `04 → 01`.
