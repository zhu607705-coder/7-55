...
- Every current and future controllable RPG player sprite must use the helpers in `src/scenes/rpg/RpgPlayerTextures.ts`. The source frames remain `48px × 64px`; that shared module alone owns the interior display scale, the campus `2×` visual baseline, the campus perspective curve, the fixed world-space foot collision box, and the dynamic name-label offset. Scene files must not define their own player scale or body geometry.
...
- The foundation-library south approach is the measured stone corridor `x=9072..9172`, extending from the road to the building front. Its interaction gate is `(9120,780)`; adjacent flower beds and the east lamp remain blocked.
