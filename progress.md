...
## 2026-09-07 校友像空格交互与一楼旧钟动态机制（本地）

- 用户要求校友像只允许靠近后按空格打开，并开始构建一楼旧钟实际转动动画。
- 删除肖像点击/悬停 Zone；附近资料入口只接受真实空格键，并在 openAlumniPanel 核对楼层与 72 源像素距离。
- 新增 ChapterFourClockMotion，使用原空白钟面覆盖静态指针，独立绘制时分秒针和反向转动齿轮。故障状态周期性回弹，缺时针状态隐藏时针，修复后按真实秒率运动；手动拨钟时隐藏动画分针。保留原状态精灵供交互范围校准使用，动画不写 GameState。
- 延续用户不测试的要求，未新增或运行测试、未构建、未进行浏览器验收。修改只在 4183 对应的集成工作树，未提交或上传。

## 2026-09-07 签到纸拾取呈现重做（本地）

- 用户要求完全放弃此前参照，重新设计第四章拾取签到纸后的界面，并明确不需要测试。
- 新增 ChapterFourPaperPickupPresentation：暖色撕边纸张入场、22:45 现场时间与 07:55:23 手机冻结时间并置、红色“时间不符”印章落下、纸张缩小收起。替换旧的蓝色边框及手机线框对照面板，布局抵消地图相机缩放。
- 原拾取事实、七秒演出结束请求及后续旧钟流程继续由现有场景和控制器管理。销毁时取消呈现补间和布局监听。
- 按用户要求未新增或运行测试，未进行浏览器验收；仅修改实现并同步文字索引。尚未提交或上传。

## 2026-09-06 被抓后进入楼梯间崩溃修复（本地）

复现了“被抓一次、复位后走到楼梯”出现 `Cannot read properties of null (reading 'resume')`。保安对白完成后仍保留 shutdown 回调；场景切换时 ArcadePhysics 先清空 world，旧回调随后调用 resume 导致崩溃。现在对白正常结束会移除退出监听，shutdown/destroy 只取消计时且禁止旧重试回调；正常恢复前检查场景仍活动且物理世界存在。原生键盘复走同一路径已进入楼梯间且无页面错误，生命周期回归检查与 TypeScript 通过。2 秒起步宽限及原有对白时长保持不变。此修复尚未推送。

## 2026-09-05 墙后行走与全量预发布准备

- 用户授权全部源码/资源＋离线 Release，不合并 main；新增三处墙面标记已实现：面包坊北墙、104 北墙、104/105 隔墙。墙顶允许经过，脚部在源图 y=264/248/584 停止；重叠身体部分保留 22% 可见度，前側恢复正常。
- 同源前景随时间态切换，不改人物贴图或共享身体，不新增剧情状态。原地图验证器5097断言通过；最终 release 34/34（199487ms），单文件234383011B，SHA-256 `f1718c141da897c5595c4d8f00ee6ef17059ec8bd45324377772e4b45a00ce28`。
- 三内核实际步行与截图检查，最终生产WebKit和离线Chromium重走三面墙，零页面/控制台错误、零文档溢出。标准游戏客户端另运行桌面按键。验证为专项路径，非全游戏连续通关。
- 从同内容远端main基线创建 codex/release-20260905；README/ASSETS及预计附件统一20260905。源码/资源全部包含，临时截图、dist、node_modules不进入归档；demo仅通过Release交付。具体远端结果以Release和交付回复为准。原始开发checkout未修改。

## 2026-09-05 A 版文案实施与全物品范围扩展

- 用户已选择 A，并要求更荒诞、嘲讽，范围扩到各类物品介绍。改写全部 64 件物品简介、详情用途备注、票据正文及第三章/3.5章/第四章任务、场景对白和反馈；物品 ID、来源路由、用途目标与证据数值不变。
- 修复物品简介双源：PixelIcon 从 items.config.json 提取名称与简介，手机和 RPG 共用；移除未渲染的重复 intro，统一来源写入 AGENTS.md。共享调查图去掉“并行调查环 / 汇合”等术语。
- 对应语义变化的三句剧场系统语音和一句保安语音增量重生成，共 4 个 MP3，英文朗读与中文字幕同步，原音频键/触发通道不变。
- quick 4/4、critical 23/23、最终 release 34/34（71396ms）通过。保存终章回答后的多余三条技术提示删除，对应提示总数 117→114；原有状态/因果验证保留。
- 三内核检查 RPG/手机详情、纸本文书、记录页与终章完成；WebKit 390×844 的长票据滚动到底可读。最终生产 WebKit 和离线 Chromium 复验，无页面/控制台错误。专项验证，不声称全程连续通关。
- 单文件234381755B，SHA-256 `6575565f2ca964d6f53db0036855a1d74974cbdd7ef337f27083723e52fa9d8d`。文本总索引7138条是全项目数量，非本轮改写数量。完整记录见 project-development-report.md 顶部。
- 未提交、上传或改动原开发 checkout；4180 生产预览已更新，上一轮节奏钓鱼修复保持。临时截图目检后清理。

## 2026-09-05 节奏钓鱼重新实现

- 实际复现 S 方块被截断；根因为独立 GeometryMask 未接受屏幕逻辑坐标补偿，和音符变换不一致。重写三列屏幕根容器与算术裁切，短块/长条/固定键位统一，保留原谱面与控制器事务。
- 修复竖屏任务条遮标题，横屏触控按钮对齐键区；增加只读几何调试快照和既有 fishing validator 的几何断言，无新测试框架或依赖。
- 三内核、四种钓取均实际成功，D 列与长按截图检查；Chromium 真触控长按、WebKit 真触屏点按成功。两轮失败保留道具、辅助重试成功，退出与重载取消未完成谱面。最终 HTTP 和 file:// 另完整钓取网框。
- quick 4/4、critical 23/23、最终 release 34/34（107909ms）通过。单文件 234232894B，SHA-256 `abb6f05e07dbc97c0ae7c6fe81f48154c2c8c3a350826e8b51c2a6b5da7ca482`。
- 详细证据与验证限制见 project-development-report.md 顶部。保留一张最终回复截图，其余临时 QA 清理；前述文案改稿未应用，未上传或提交 Git。

## 2026-09-05 剧场入口四处修订

- 正门新增靠近打开、离开关闭的双距离区间感应，复用同源木门，不改变验票与离场事实。
- 左侧柜体改为现场充电站：固定标识/电池条/接线和手机状态；接近浅色操作补电至45%，走远/隐藏/退出取消。手机控制中心移除充电动作，控制器检查现场与距离，保留1%主线保底。
- 初始任务与取票机提供退票/论坛留言线索，不直接要求去CC98接单；打印→拼票→检票对白说明匹配依据，串行播放并按文本长度留出阅读时间。
- quick 4/4、critical 23/23、最终 release 34/34通过；三内核实测感应门、充电完成/取消/拒绝与重复调用，WebKit另用触屏交互键，最终离线单文件完成相同专项验证。
- 最终 Chromium 从手机接单/两波抢票回到剧场，真实拖拽擦海报、输入回执、拼票、验票后进入 program_search；两张半票消耗、完整票保留。对白按顺序显示且不重复角色前缀；充电进度不被字幕遮住。临时截图检查后清理。
- 生成单文件234269833B，SHA-256 `cf213075503ee18fb8bcd6156476e2c516ffa98488fbdb86a43e1d47b4edb8f1`。未提交或上传，原始开发checkout未改动；详见 project-development-report.md 顶部。

## 2026-09-05 减少玩家可见加载等待

- 已开放手机应用按意图/空闲预取；CC98 正常网络取消固定 700ms 等待，失败与体艺闪退剧情不变。RPG 预热有限并发、同一目标不因手机子页切换取消。
- 已解码图片通过 64MiB / 低内存16MiB、闲置60秒缓存直接交给 Phaser，覆盖地图和人物精灵表；缓存失效继续正常加载。修复 shutdown 后重试监听丢失及全缓存就绪通知。
- 同类型静态服务器、同机各三次：CC98 点击可见中位数 1349.8→99.3ms；启真湖回到现场 235.4→80.6ms，图片加载条目33→0；首次冷进入约4.05s未明显改善。正式报告记录限速、停留条件与原始样本，不外推全部场景。
- 最终 release 33/33；缓存执行验证13/13；主图加载失败三次后重试、三次地图往返通过，库存不变、画布单实例、缓存未增长。三内核和 file:// 定向验证完成；截图检查后清理。未提交/上传，原始开发 checkout 未改动。
- 单文件 SHA-256 `35ce859538159786bdaea817a6e57ed4597bedd1dd894138bdadfeb832754bb9`；网页版仍用4180。详细记录见 `project-development-report.md` 顶部。
...
- 调整：首轮把 Phaser 共享人物与第四章错位楼梯 Three.js 人物统一调整为侧向每帧 `110ms`。用户查看单文件后要求再加快一点，最终值收紧为 `100ms`，即 `10 FPS`、十二帧完整周期 `1200ms`。上下八帧仍是 `880ms`，人物位移速度、碰撞、尺寸和转身时长均未改变。
- 平均过渡帧：以内置 ImageGen 参考现有 `player_side_2/3/8/9`，生成透明的左右腿收拢经过姿势，保存为 `source/player_side_passing_close_v1.png`。既有素材生成器把它规范化到 `96×128`、轮廓高 `101px`，只替换运行时 `player_side_3.png`；其余 `27` 张运行时人物帧内容未变化。
- 素材目检：新第 `3` 帧保持同一人物、服装、背包、朝向和完整鞋底，双腿间距小于相邻第 `2/4` 帧，放入 `0–5` 半步序列后形成“大步—收拢—近并步—再展开”的连续变化。生成源图为真实 RGBA，透明度范围覆盖 `0–1`。
- 面包房营业员诊断：其脚底锚点被写在柜台图形内部的 `(260,340)`，同时运行时将人物深度设置为 `a1_foreground_018` 柜台前景基线再加一，使裁切后的上半身绘制在柜台裁片上方，因此视觉上位于台面。该问题本轮按用户提问只给出原因，没有修改营业员位置或层级。
- 单文件接入：新 `player_side_3.png` 已由 Phaser 共享人物和第四章错位楼梯 Three.js 人物共同导入。以最终 `100ms` 侧向节奏重新运行项目保留的 `build:single:unchecked` 入口，生成的 `demo/index.html` 为 `268864617` 字节，SHA-256 为 `7dd581580f47c584816048cb0db677deedf556ce07a9ff0ec3758e225aa89835`；新帧的 Base64 前缀可在成品内精确命中。
- 定向校验：`npm run verify:single` 通过，确认成品只包含一个 HTML、两个内联脚本和一个内联样式。正式 `npm run build:single` 仍被当前 worktree 内既有的全局 TypeScript 错误阻断，错误集中在第四章兼容阶段类型、SaveStore 时间契约、食堂边界字面量、第四章地图布局与启真湖碰撞编辑器；本次修改的 `RpgPlayerTextures.ts` 和 `playerSprite.ts` 均未出现在错误列表。
- 范围：本轮没有运行 Playwright、跨内核矩阵或整套测试，没有处理上述无关类型错误；没有执行 Git 暂存、提交、合并、推送或上传。

## 2026-09-03 东区食堂东南门同源校准

- 根因确认：食堂正式底图中的东南双扇门位于源图 `{left:1305,top:795,right:1393,bottom:900}`，合计 `88×105`、中心 `(1349,847.5)`；旧动态门却用程序色块绘成 `122×126` 并放在 `(1380,858)`，因此造型、比例、横向位置和实际门洞一起偏离底图。
- 动态门改为直接裁取 `canteen_interior.png` 的左右门扇，各为 `44×105` 源像素。关闭态与底图原门逐像素重合；开门时两扇以门框外侧为轴收合，底层只显示限定在原门洞内的暗部，不再出现额外的青绿色大门。
- 同一份源坐标同时校准东侧墙体碰撞、左右门框遮挡、出口交互中心、纸条逃逸锚点和人物离场轨迹。门洞有效横向范围统一为 `x=1305–1393`，画面与判定不再分别沿用两套坐标。
- 标准 web-game Playwright 客户端在 `c3-canteen-entry` 运行两轮：关闭态门框与底图对齐，人物从 `(1194,834)` 向右移动到 `(1211,834)`，运行时门保持 `closed`，东侧墙体碰撞从 `x=1393` 开始，未产生错误文件。另在真实 Chromium 中查看完全打开态，并从 `(1349,835)` 按住 `Space` 走完实际离场链，事件依次包含 `rpg_canteen_leave_requested` 和 `canteen_returned_to_campus`，最终到达 `campus_bootstrap / campus_canteen_gate`，console error 为 `0`。
- 遵照当前范围，本轮没有生成单文件，没有运行 typecheck、项目校验脚本、跨内核矩阵或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 CC98 114 板公开快照与“郁闷小屋”本地内容补充

- 公开来源：读取 `https://www.cc98.org/board/114` 的公开页面，并核实字段归属。该页在 `2026-09-03` 快照中显示“今日贴数 `292`”和“总主题数 `132481`”。统计仅作为来源快照写入数据文件，游戏运行时不会请求外部论坛。
- 内容边界：新增“郁闷小屋”版面与 `12` 条本地原创、去标识化讨论。新帖只吸收近期公开讨论中可泛化的校园议题类型，如求职等待、学习卡顿、宿舍沟通、协作催办和低门槛互助；没有复制真实用户昵称、原帖标题、正文、回复、浏览数或个人经历。
- 数量呈现：版面目录将“本地收录 `12` 帖”与“公开快照 · 2026-09-03 · 今日 `292` · 总主题 `132481`”并列呈现，并在无障碍标签中保留相同语义，避免将游戏内置内容数量标为真实论坛总量。
- 目标页复核：在独立 Vite 预览的真实浏览器中进入 CC98 → 版面，目录显示 `13` 个版面；“郁闷小屋”卡片和语义标签均显示上述两类数量。进入该版面后，页头显示 `12 帖`，DOM 中恰有 `12` 条 `cc98-post`，页面无横向溢出。
- 交付边界：遵照此前“暂不生成单文件、暂不做相关测试检查”的要求，本轮没有运行 `build:single`、`verify:single`、类型检查或全量验证；没有执行 Git 暂存、提交、合并、推送或上传。

## 2026-09-03 CC98 135 板公开快照与“开怀一笑”内容补充

- 公开来源：读取 `https://www.cc98.org/board/135` 的公开页面，确认该板名称为“开怀一笑”，页面快照显示“今日贴数 `128`”与“总主题数 `139709`”。快照写入同一份本地数据，游戏运行时不访问外部论坛。
- 内容边界：在既有 `3` 条“开怀一笑”帖子基础上补入 `12` 条原创校园轻松话题，使游戏内该板合计为 `15` 帖。主题包括误入次日课程、校园导航入口、群聊歪楼、食堂观察、门禁与校园卡误判、雨伞辨认和课堂投影桌面等；没有复制真实昵称、原帖标题、正文、回复或具体经历。
- 数量呈现：版面目录继续区分本地 `15 帖` 与“公开快照 · 2026-09-03 · 今日 `128` · 总主题 `139709`”。`boardId` 现进入快照结构，`114` 和 `135` 分别可追溯到两块外部公开版面。
- 目标页复核：独立浏览器预览中，目录的“开怀一笑”卡片与无障碍标签均为本地 `15 帖`、今日 `128`、总主题 `139709`；进入版面后页头为 `15 帖`，实际渲染 `15` 条 `cc98-post`，页面无横向溢出。
- 交付边界：继续不生成单文件、不运行类型检查和全量验证，不执行 Git 暂存、提交、合并、推送或上传。

## 2026-08-31 寝室人物相对比例与取餐演出暗框修正

- 比例拆分：寝室人物继续直接调用 `configureRpgPlayerSprite()`，与图书馆、食堂和剧院共用 `RPG_PLAYER_DISPLAY_SCALE = 0.65`，没有新增寝室专属人物倍率。寝室原始 `941×1672` 场景只缩小一档至 `0.5`，与共享人物本身相对原寝室人物更大的变化共同形成所需相对比例；没有把地图缩到四分之一。
- 同步映射：`DormHubModel` 保留全部原图源像素坐标，并通过唯一的 `0.5` 变换同步生成地图、静态碰撞、交互热点、校园卡、吹风机、出生点和门的运行时坐标。室内动画装饰放入同一缩放层；门前景裁切扩展为“源裁切 + 运行时显示倍率”，避免缩图后遮挡错位。移动速度统一为其他室内量级的 `160px/s`。
- 取餐演出：按用户补充说明，问题并非菜单面板描边，而是正确取餐后 `animatePaperBurst()` 演出叠加的 `70px` 半透明黑色描边。现已删除这层四周暗框及其淡入、淡出和销毁引用，窗口辉光、推镜、纸条爆开、贴镜与拉远镜头保持不变；误删的菜单面板原有 `4px` 描边已经恢复。
- 浏览器验证：真实 Chrome 在 `1280×720` 和 `390×844` 目检寝室缩放，地图、人物、HUD 和道具栏无重叠或拉伸；中央通道按键 `650ms` 实测移动 `104px`，门框碰撞停在映射后的实体边缘。另验证校园卡可拾取、寝室门可完整过渡到 `campus_bootstrap / campus_spawn`，并从点餐、3号取餐窗口进入纸条演出检查暗框；控制台错误为 `0`。
- 防回归与自动验证：RPG 室内门验证器新增共享人物倍率、寝室半尺度全层映射、缩放门裁切和取餐演出禁止四周暗框合同，共 `41` 项断言通过。Task 14 合同同步到当前的统一 Host 任务栏所有权，要求桌面 RPG 只由 `RpgGameHost` 挂载任务栏且 `App` 不重复渲染。
- 发布门：临时补齐 `ffprobe/ffmpeg` 后 `npm run validate:release` 为 `31/31`，覆盖类型检查、文本清单、第三章音频、第四章合同、校园地图、生产构建、离线单文件及三条 Chrome 冒烟路线。离线单文件为 `268974378` 字节；其最终内联载荷不再执行高内存收益很低的 esbuild 压缩，但仍按 Chrome/Edge 90、Firefox 91、Safari 15 目标转译。
- 交付范围：按用户确认，本次提交全部已跟踪修改；未跟踪的 `godot/`、奇珍湖素材与生成脚本、`第三章文案.md` 继续排除，临时媒体工具、Chrome 配置、截图与 QA 脚本不进入仓库。

## 2026-08-31 食堂场景重进卡死与追踪入口演出修复

- 双重根因：Phaser 停止后会复用同一个 `CanteenInteriorScene` 实例，但食堂原先没有在下一次 `create()` 前清理 `paperBusy`、`dialogueLocked`、`entryPaperTriggered`、出口锁、弹窗引用和运行时对象集合。演出中途切换 DEV 节点后，全局、Scene 和键盘输入虽然都已恢复，旧的 `paperBusy=true` 仍会把人物速度归零，而旧的 `entryPaperTriggered=true` 会永久挡住下一轮入场演出。另一个独立问题是 DEV“东区大食堂追踪”节点错误写入 `entryPaperEscaped=true`，导致从校园真实大门进入时控制器直接认为演出已经完成。
- 生命周期修复：食堂 Scene 新增统一的重启状态清理，在每次建图前和 `shutdown` 收尾时归零输入、移动锁、演出锁、出口状态、弹窗、队列、遮挡、NPC、追逐运行时和对象缓存；关停时同时移除键盘、指针、自动开演、纸条帧和追逐计时器。食堂调试状态现公开三层输入开关、入场演出状态、纸条距离、移动许可及每个具体阻塞项。
- 入口修复：`canteen-hunt` 与 `c3-canteen-entry` 两个 DEV 节点都保留 `entryPaperEscaped=false`。只要正式状态仍待播放，食堂首帧稳定后会自动开始纸条发现演出，不再要求玩家绕过北侧墙体进入一个不可见的 `360px` 半径；原距离触发仍保留为后备，演出完成事实仍只由第三章食堂控制器写入。
- 防回归：既有 DEV 关卡验证器新增追踪节点演出事实合同；既有 RPG 室内门验证器新增食堂 Scene 重启清理、自动开演、输入解绑和移动阻塞可观察性合同。没有新增测试文件或测试依赖。
- 真实浏览器：真实 Chrome 完成两条回归。第一条在纸条演出进行中再次应用“进入食堂”节点，第二轮演出完整结束，所有阻塞项归零，方向键令人物向上移动 `118px`。第二条从“东区大食堂追踪”校园场景把人物移到正式食堂门点并通过真实入口事件进入；开演时人物与纸条相距 `563.5px`，高于旧半径 `360px`，仍自动触发，结束后方向键同样移动 `118px`。两条路径控制台错误均为 `0`。
- 自动验证：刷新文本清单后 `validate:quick` 为 `4/4`；`validate:critical` 为 `18/20`；`validate:release` 为 `27/31`。类型检查、第三章链路、RPG 合同、第四章合同、生产构建、`261029113` 字节离线单文件、结构验证和 Chrome 三场景烟测均通过。剩余追逐音频、Task 14 音频时长、第三章音频和 3.5 章语音备忘仍仅因本机缺少 `ffprobe`。
- 交付边界：临时 CDP 脚本、Chrome 用户目录和开发服务器均已清理；没有执行 Git 暂存、提交、拉取、合并或推送，工作区既有无关改动和未跟踪文件保持不动。

## 2026-08-30 第四章 DEV 节点重启地图加载修复

- 根因：DEV 面板应用第四章节点后会停止并重新启动 `chapter-four-temporal-maze`，Phaser 会复用同一个 Scene 实例，但上一轮的背景图片、前景、碰撞调试层、楼层面板与 NPC 等类字段仍指向已经销毁的 GameObject。`createBaseBackgrounds()` 又只检查楼层键是否存在，导致背景不重建；随后原子贴图事务对旧图片调用 `setTexture()`，稳定产生 `plate_group_background_commit_failed: Cannot read properties of undefined (reading 'sys')`，表现为地图偶发空白或一直停在旧加载状态。
- 生命周期修复：新增统一的 Scene 重启状态清理，在每次 `create()` 建图前以及 `shutdown` 收尾后释放所有跨轮次对象缓存、面板与输入锁、移动请求、楼层事务签名和调试错误；关闭中的楼层面板也会主动销毁。背景创建改为只复用仍然 `active` 的图片，发现已销毁缓存时先移除键再按当前楼层贴图重建。
- 防回归：第四章 warmup 验证器新增 Scene 重启合同，要求清理背景、电梯、前景、碰撞调试层、荣誉墙、NPC、移动请求和楼层面板，并保证清理发生在背景创建前、销毁背景缓存能够重新生成。该验证当前为 `88` 项断言通过。
- 自动验证：`npm run typecheck`、第四章 story/runtime/topology/effective-interactions/guard-presentation、`npm run build:single`、`verify:single`、生产构建和 Chromium browser smoke 均通过。`validate:critical` 为 `18/20`，刷新文本清单后的发布门为 `27/31`；剩余追逐音频、第四章 Task 14、第三章音频和 3.5 章语音备忘均仅因本机缺少 `ffprobe`。离线单文件为 `261026358` 字节，文本清单为 `6780` 条。
- 真实浏览器：真实 Chrome 在同一页面按 `opening → room204 → maintenance → final-minute → checkin → classrooms → opening` 连续执行 `7` 次 DEV 节点切换。每次均为单一 Phaser 画布、warmup ready、提交阶段与应用阶段一致、贴图签名有效、画面像素非空且 `contractFailures=[]`；原 `undefined.sys` 错误没有复现。
- 交付边界：临时 Chrome 用户目录、诊断脚本和开发服务器均已清理；没有执行 Git 暂存、提交、推送或再次合并，工作区原有未跟踪文件和无关改动保持不动。

## 2026-08-30 远端主分支拉取、第四章深色遮罩合并与 Windows 发布门复核

- 拉取与合并：按用户确认的 A 方案，先只暂存 `13` 个已跟踪改动，不包含未跟踪文件；随后将 `main` 从 `ea6b2e2` 快进到 `origin/main@0e548ae`，完整接收远端 `6` 个提交。恢复本地改动时所有代码自动合并，仅 `progress.md` 出现内容冲突；冲突两侧记录均保留，没有丢弃远端或本地进度。
- 本地改动保留：第四章深色观察继续使用单块 `960×540`、`0x071127 / 0.56` 的连续遮罩，扫描线和拼接边框没有被远端更新带回；其余合并前的本地改动也全部恢复为未暂存状态。原有 `1456` 个未跟踪项目全程未进入 stash、未移动、未删除。
- Windows 验证入口：新拉取的验证编排器在 Node `25.2.1` 下直接启动 `npm.cmd` 会返回 `EINVAL`，浏览器烟测直接启动 `vite.cmd` 也有同类问题。编排器现通过当前 Node 与 `npm_execpath` 调用 npm CLI；浏览器烟测通过当前 Node 启动 Vite JS，并补充 Windows Chrome、Edge 与 Playwright 缓存路径探测，不改变任何游戏逻辑。
- 自动验证：`validate:quick` 为 `4/4` 通过；修正后的 `validate:release` 为 `27/31` 通过。TypeScript、文本清单、任务提示、DEV 关卡、章节合同、第四章 story/topology/runtime、校园地图、生产构建、单文件构建、单文件结构与浏览器烟测均通过；离线文件为 `261023658` 字节，含 `2` 个内联脚本和 `1` 个内联样式。剩余 `4` 项（追逐音频、第四章 Task 14、第三章音频、3.5 章语音备忘）均因本机未安装 `ffprobe`，未擅自安装系统软件。
- 文本与浏览器复核：快检发现玩家文本清单过期后重新导出，当前共 `6780` 条、第四章 `1612` 条。真实 Chrome 烟测通过手机 `390×844`、剧院 RPG `1440×900` 和第四章 RPG `1440×900`；另在 `1280×720` 从第四章检查点切换到“深色观察”目检，遮罩连续均匀，中央横线和重复扫描纹均未出现，HUD、角色与交互点层级正常。
- 交付边界：本轮只执行拉取与快进合并，没有 Git 暂存、提交或推送；`demo/` 与 `dist/` 仅由正式验证命令生成，临时浏览器页面、截图、配置和开发服务器均已清理。

## 2026-08-30 “灿若星辰灯”像素转写实衔接视频脚本

- 方案确认：用户选择 C 方案。双签到后的衔接固定为三段各 `6s / 24 FPS / 16:9` 的视频：像素门厅确认与走向出口、玻璃门完全遮挡时的单次硬切、写实楼外与未点亮灯体；随后继续使用现有 `6200ms` 正式五层灯光序列，完整收束约 `24.2s`。
- 脚本交付：新增 `docs/plans/2026-08-30-chapter4-canruo-star-lamp-transition-video-design.md`，包含 `A01–A10` 锚点合同、`960×540` 坐标、逐秒分镜、中英文 H3 全参考提示词、全局负面提示词、对白/字幕/音效/配乐时钟、接缝、分段生成、返工和成片验收清单。
- 素材边界：A1 清晨母图、像素与高分辨率入口、主角纹理和正式五层灯素材继续作为身份与几何权威。灯体原始 `1024×1536` 画布与 `{x:361,y:27,width:307,height:1497}` 边界保持，视频只能展示未点亮灯体，正式发光仍由现有 consumer 完成。
- 状态边界：视频只负责演出，不拥有 `GameStore`、`SaveStore`、任务事实、道具、阶段或完成回执。未来接入时，视频结束只能进入现有灯光会话；只有正式灯光序列真实播放并通过一次性 proof 后才能完成第四章。
- 锚点完成：从真实 `c4-755-checkin` 运行时提取无 React HUD 的 `A01` 完成态与 `A02` 门槛态；基于正式主角纹理、签到装置和 `lamp_dark.png` 完成 `A03–A10`。`A08` 与正式灯体素材 SHA-256 完全一致，`A09` 使用正式灯体做 `960×540` 末段对齐，没有重画灯体结构。
- 运行时差异：第二次签到成功后控制器立即进入 `exterior_closure`，两台动态签到装置随阶段切换消失。因此真实 `A01` 保留了已完成的人员、前台和大厅状态，但不包含两台绿色完成装置；装置身份由 `A04` 独立保存。未来生成 C4V-01 前需要决定延后运行时清理或在视频锚点中合成完成装置，本轮没有改动运行时。
- C4V-02 单次预览：MiniMax CLI 的五参考 H3 路线当前不可调用，dry-run 确认首尾帧模式会使用 `MiniMax-Hailuo-02`。本轮只提交 `1` 个 A02→A07 的首尾帧任务，得到 `5.916667s / 142 帧 / 24 FPS / 1364×768 / 无音轨` 的本地预览；视频额度从区间 `0/5`、周 `2/35` 变为区间 `1/5`、周 `3/35`。
- 逐帧结论：三张联系表覆盖全部 `142` 帧。首尾身份、高光遮挡、唯一未点亮灯体、无文字和无音轨达标；双门错误旋开、主角在完全遮挡前连续拉伸、门框逐步形变、缺少 2 帧以及尾部没有 8 帧稳定段均不达标。该预览已标记为拒绝正式接入；用户确认按上传范围 A 排除预览、联系表、关键帧和审查报告，这些失败证据仅保存在本地，不进入 GitHub。
- 停止门槛：没有调用第二次视频生成，没有剪辑或接入预览，没有修改 React、Phaser、控制器、存档、正式灯光序列或 `demo/index.html`。下一步等待用户先审阅当前视频；未获明确批准前不继续消耗额度。

## 2026-08-30 第四章一楼面包坊柜台空气墙与工作人员

- 柜台空气墙：按用户在 A1 面包坊选定的完整柜台范围新增 `a1_air_wall_bakery_counter`，正式源像素边界为 `x=80 / y=292 / 397×90`，沿用 `a1-ann-018` 与 `a1_foreground_018` 的同源标注。碰撞下沿停在 `y=382`，与 12:25 排队绳上沿 `y=398` 之间保留 `16px` 通道，可容纳主角 `19.5×14.625` 的脚部碰撞体。
- 路线与交互：检修灯、传送带和时针站位从 `y=386` 统一微调到 `y=390`，两条正式面包坊路线同步经过 `(477,390) → (374,390) → (294,390)`。空气墙覆盖柜台正面，同时保留从东侧缺口进入队列通道的真实路径。
- 工作人员：复用现有 `counter_aunties_2frame.png` 的第 `6–7` 帧，在 `bakery_hour_hand` 与 `morning_checkin` 两个值守阶段显示。人物移动到糕点展示柜后方 `(260,340)`；运行时只绘制源帧上方 `80px`，深度置于柜台前景裁片上方，因此上半身清楚可见、腿部不会覆盖柜台正面。阶段离开或场景销毁时独立清理，不再依附 12:25 人群运行时。
- 可观察性：`rpgRuntime.player` 新增真实 `collisionBounds`，`chapterFour.bakeryStaff` 暴露可见性、位置、当前帧、动画与有效阶段，便于复核实体存在、碰撞停止点和生命周期。
- 自动验证：`npm run typecheck` 通过；`npm run chapter4:validate-runtime` 通过 `1095` 项断言；`npm run chapter4:validate-topology` 通过 `3009` 项断言；`npm run chapter4:validate-effective-interactions` 通过 `624` 项断言；`git diff --check` 通过。同步更新既有验证器，没有新增测试文件或测试依赖。
- 浏览器验证：`develop-web-game` 标准客户端分别载入 `c4-755-bakery-1225` 与 `c4-755-checkin`，两种阶段都返回 `bakeryStaff.visible=true`、正式位置 `(260,340)`、柜台碰撞边界和 `contractFailures=[]`；客户端固定画布截图在这两个横屏入口捕获为黑画面，因此追加 Chromium 页面级复核。补充画面确认工作人员完整站在展示柜后；真实方向键把主角脚部碰撞体推进到柜台前，从 `y=382.508` 向上后停在 `y=382`，随后 `Space` 正常写入 `bakery_conveyor_lamp_inspected` 并完成时针暴露。浏览器 console/page error 均为 `0`。
- 交付边界：继续遵守“先不打包单文件”，本轮没有运行 `build:single`、`verify:single`，也没有重新生成 `demo/index.html`；没有执行 Git 暂存、提交、合并、推送或上传，既有未提交工作全部保留。

## 2026-08-30 第四章一楼前台签到装置精修

- 签到板位置：按用户选择的 A 方案保留原 `152×100` 透明高清素材作为解码源，将运行时显示尺寸缩小到 `104×68`，中心从 `(750,600)` 调整到 `(751,582)`。同步把交互锚点改为 `x=699 / y=548 / 104×68`，签到板现在立在台面后缘，不再覆盖工作人员、前台正面或两台签到设备。
- 校园卡读卡器：将纯色矩形改为俯视像素装置，增加六边机壳、金属内框、浅色识别屏、青色扫描线、卡槽、状态灯、按钮与底部标识；可交互实体收紧并移动到 `x=768 / y=607 / 30×24`。
- 纸条签到槽：将纯色方块改为棕金色俯视纸槽，增加斜面机壳、伸出纸张、两行纸面痕迹、深色入口槽、金属饰条、状态灯和固定点；可交互实体调整到 `x=848 / y=606 / 38×25`。两台设备在成功后继续保留并切换为绿色状态灯和“已刷卡／已签到”标签。
- 合同一致性：签到板显示边界、上下文交互锚点、两台设备的可见外框、拖放边界、站位和调试状态继续使用同一组源像素数据；既有验证器的明确边界期望随正式布局同步更新，没有新增测试文件或测试依赖。
- 柜台可达性：真实键盘移动把主角推进到柜台碰撞边缘 `(783.2, 660.27)` 后，原 `56px` 投放距离仍会返回“人物距离不足”。两台签到设备的正式 `proximity` 因此统一调整为 `72px`，覆盖玩家能够实际到达的柜台边缘，同时仍要求命中设备的精确外框。
- 自动验证：`npm run typecheck` 通过；`npm run chapter4:validate-runtime` 通过 `1095` 项断言；`npm run chapter4:validate-topology` 通过 `2976` 项断言；`npm run chapter4:validate-effective-interactions` 通过 `624` 项断言；`git diff --check` 通过。
- 浏览器验证：`develop-web-game` 标准客户端在真实 Chromium 的 `c4-755-checkin` 检查点加载正式 A1 清晨状态。截图确认签到板完整落在台面后缘，工作人员无遮挡，读卡器和纸条槽分别位于左右操作位并具有可辨识细节；运行时返回更新后的精确边界与 `72px` 距离，`contractFailures=[]`，没有 console 或 page error 文件。另用真实方向键和鼠标拖拽把校园卡投入读卡器，得到 `checkinCardAccepted=true`、校园卡仍归玩家持有、纸条仍待签到；读卡器状态灯变绿并显示“已刷卡”，全程没有 console 或 page error。
- 交付边界：继续遵守“先不打包单文件”，本轮没有运行 `build:single`、`verify:single`，也没有重新生成 `demo/index.html`；没有执行 Git 暂存、提交、合并、推送或上传，既有未提交工作全部保留。

## 2026-08-30 第四章教学楼主入口玻璃门遮挡

- 根因：A1 教学楼主入口原本已经有与背景同源的玻璃门前景裁片和 `baselineY=881` 的脚点深度规则，但第四章运行时会把主角持续固定到 `depth=9900`，门前景仍停留在普通场景深度，因此主角经过门后时始终显示在玻璃门上方。
- 动态深度：仅为主入口前景 `floor_1_a1_foreground_017` 增加局部遮挡例外。主角脚点位于门槛后方时，玻璃门前景切换到 `depth=9901` 并遮挡主角；脚点越过门槛进入前场后，门前景恢复到原始 `depth=4881`，主角重新显示在门前。第四章其他区域继续保持人物最高图层规则。
- 可观察性：第四章运行时调试状态为该门前景增加 `occludesPlayer` 字段，可直接核对当前楼层、脚点与门槛关系，避免以后只看静态深度值而误判遮挡状态。
- 自动验证：`npm run typecheck` 通过；`npm run chapter4:validate-runtime` 通过 `1095` 项断言；`npm run chapter4:validate-topology` 通过 `2976` 项断言；`git diff --check` 通过。
- 浏览器验证：`develop-web-game` 标准客户端先确认 A1 主入口正常加载；随后真实 Chromium 驱动主角从室内走入玻璃门区域再越过门槛。门后状态为 `playerDepth=9900 / doorDepth=9901 / occludesPlayer=true`，画面中主角被玻璃门完整遮挡；门前状态恢复为 `doorDepth=4881 / occludesPlayer=false`，主角正常显示。全程没有 contract failure、console error 或 page error。
- 交付边界：本轮按用户要求暂不打包单文件，因此没有执行 `build:single`、`verify:single`，也没有重新生成 `demo/index.html`；没有新增测试文件或测试依赖，没有执行 Git 暂存、提交、合并、推送或上传。工作区中已有课外锻炼、剧院建模和相关文本改动均原样保留。

## 2026-08-30 课外锻炼虚拟定位跑道

- 交互替换：第一章“开始课外锻炼”不再直接写入完成状态。点击后进入手机框内的虚拟 GPS 跑道，玩家需要按顺序点亮十个定位点；每个点对应一分钟、`0.30 km` 和 `0.75` 圈，十个点完成时形成 `10:00 / 3.00 km / 7.50 圈 / 03'20\"/km` 的完整记录。
- 画面与文字：新增紫云田径场定位界面，包含跑道、内场、十个定位点、移动人物、左右迈腿动画、轨迹进度线、GPS 精度、用时、距离、配速、圈数、误点纠正、完成回执和参加者记录编号。首页按钮同步改为“开始虚拟定位 / 10 分钟跑完 3 km · 点击生成轨迹”。
- 状态边界：前九个点只保存在本次 React 会话中，不提前推进章节；只有第十个点完成后才调用 `ActOneBootstrapController.startExercise()`。中途按 `Esc` 或关闭页面不会留下半完成共享进度；控制器拒绝最终写入时回退到第九点，允许玩家重新结算。
- 输入与可观察性：十个定位点支持触屏、鼠标以及 `Tab + Enter / Space`，`Esc` 可退出；运行中的点位、用时、距离、配速、圈数和下一目标接入 `render_game_to_text()`，便于浏览器复核而不建立第二套进度权威。
- 自动验证：`npm run validate:quick` 四项全部通过，覆盖 TypeScript、文本清单、任务提示所有权和 `115` 个 DEV 检查点；`npm run build` 普通生产构建通过。`docs/game-text-by-chapter.md` 已重新导出并通过 `text:check`，当前总计 `6628` 条玩家文本。
- 浏览器验证：真实 Chromium 在 `658×739` 完成误点、第一个点键盘输入、中途五点、十点结算和返回首页；五点时 `exerciseStarted=false`，十点后为 `true`，console/page error 为 `0`。`390×844` 另行验证 `Esc` 不写进度、完成卡片完全可见、文档横纵溢出均为 `0`；Gecko 与 WebKit 也分别完成十点结算，均写入 `exerciseStarted=true`，完成卡片透明度为 `1`，溢出与 console/page error 均为 `0`。
- 交付边界：用户明确要求本轮先不打包单文件，因此没有执行 `build:single`、`verify:single`，也没有重新生成 `demo/index.html`；没有新增测试文件或测试依赖，没有执行 Git 暂存、提交、合并、推送或上传。既有剧院到达过场建模改动继续保留且未混入本功能逻辑。

## 2026-08-30 剧院到达过场左右翼楼建模升级

- 范围：用户指出第三章骑车抵达剧院的低多边形过场中，剧院入口与左右两侧建模过于粗糙。本轮继续使用 `selected-phaser-ui-20260721` 现有 `main` 工作区，只修改 Three.js 过场静态环境；追逐距离、133 帧收尾时间线、人物与自行车动作、剧情状态和单文件入口均未改动。
- 参考与结构：以仓库内 `campus_avenue_distance_755.png` 为视觉基准，将原来的单块米色长方体重建为中央凹入式深蓝玻璃门厅、双扇门、上下窗格、分层门楣、浅色高立柱，以及左右独立暖色翼楼。两侧翼楼分别加入三层窗格、横向楼板带、外侧高立柱和屋顶压边，中央与侧翼继续使用同一低多边形材质与光照体系。
- 前场融合：入口两侧补充对称绿篱花坛和黑色校园灯，扩大平坦入口铺装以承接完整立面。剧院停车镜头最靠近相机的一对道路树向外移，避免右翼楼被大面积遮挡；食堂出发场景的道路树位保持不变。入口仍保持无横向台阶的平坦可达设计。
- 源码组织：新增可复用的窗格、翼楼、中央入口、花坛和校园灯构建函数，并为中央门厅、左右翼楼、双扇门及灯具设置稳定对象名，后续可继续做镜头或材质微调，无需重写整座建筑。
- 自动验证：`npm run typecheck` 通过；`npm run verify:canteen-bike-transition` 为 `41/41` 通过；`npm run build:single` 与 `npm run verify:single` 通过；`git diff --check` 通过。最终离线单文件为 `258677345` 字节，含 `2` 个内联脚本和 `1` 个内联样式。
- 画面验证：临时 Chromium/SwiftShader 直接渲染收尾帧 `0 / 91 / 132`，帧 `0` 显示完整中央门厅和双侧翼楼，帧 `91` 显示停车构图且右翼楼不再被近景树完全遮住，帧 `132` 保持原进门遮挡收束；三帧 console/page error 为 `0`。随后用 `develop-web-game` 标准客户端分别冷启动 Vite 版与最终 `file://` 单文件，画布均正常渲染且没有错误文件。
- 交付边界：本轮没有新增测试文件或测试依赖，没有执行 Git 暂存、提交、合并、推送或 GitHub 上传；`demo/index.html` 仅由正式构建命令重新生成。
...
## 2026-08-30 第四章深色遮罩横线修复

- 根因：`ChapterFourTemporalMazeScene.createRealityModeVisuals()` 为第四章单独绘制了每隔 `16px` 的全屏水平扫描线，并叠加四边图形；这些屏幕空间线条在响应式缩放时会发生采样，正中线表现为明显横向接缝。第三章食堂、剧院与启真湖使用的则是不中断的单矩形压暗层。
- 视觉统一：第四章深色观察改为一块完整的 `960×540` 深蓝矩形遮罩（`0x071127 / 0.56`），删除 `darkPattern`、水平线和边缘拼接图形；原有深浅模式交叉淡入、青色观察点、角色层和共享 HUD 层级保持不变。浅色暖层继续使用完整矩形，不改玩法与控制器状态。
- 防回归：现有第四章 story 校验改为要求连续深色遮罩，并显式禁止该氛围块重新出现 `darkPattern`、`lineBetween()` 或 `strokeRect()`。
- 自动验证：`npm run chapter4:validate-story`、`npm run chapter4:validate-runtime`（`1095` 项）、`npm run typecheck`、`npm run build:single` 与 `npm run verify:single` 全部通过。首次单文件构建在 Node 默认约 `4GB` 堆上限处退出；按项目既有 `6144MB` 配置重跑成功，最终 `demo/index.html` 为 `258652905` 字节，包含 `2` 个内联脚本和 `1` 个内联样式。
- 浏览器验证：真实 Chrome 从 `c4-755-classrooms-1850` 检查点自动切换到深色观察，在 `1280×720` 与非 `16:9` 的 `1440×900` 两个窗口目检；遮罩覆盖连续且均匀，正中横线与重复扫描纹均已消失，模式按钮、观察点、角色、任务条和物品栏仍在正确层级。
- 交付边界：临时浏览器页面、用户目录、截图和开发服务器在检查后清理；没有执行 Git 暂存、提交、合并、推送或上传。
...
## 2026-08-30 第四章二十人校友荣誉墙与放大查阅

- 校友荣誉墙统一扩充为 `20` 位浙江大学历史人物。保留苏步青、竺可桢、路甬祥、陈建功、谈家桢、程开甲六位既有人物；A1 北墙十三个原画像框依次装入王淦昌、贝时璋、谷超豪、李政道、潘云鹤、韩祯祥、夏道行、潘镜芙、王元、陈宜张、林俊德、谭其骧、郑树森；A3 独立竖框装入杨卫。人物姓名、年代、身份、生平与来源链接统一收口在 `ChapterFourAlumniHonorWall.ts`。
- 新增十四张 `256×384` 透明 PNG。每张先使用对应人物的真实照片约束身份特征，再使用项目既有画像约束 RPG 画风；成图统一清除背景、收成透明边缘并缩放到同一尺寸。所有文件低于 `200 KB`，至少保留 `10%` 全透明像素，左上角透明，并由第四章 transport 阶段预热清单自动消费。
- A1 与 A3 画像按各自 `1672×941` 楼层底图源像素坐标落位。画像层级高于原校友墙前景裁片，保持人物、墙体和门的原遮挡关系；A3 三联小框继续绘制金色内框，底图已有框不重复绘制。校友画像不显示观察环、金色菱形或中心光点。
- 二十个框全部由同一交互循环创建无形点击区；鼠标和触屏可直接点击，人物靠近后也会显示 `Space · 查看××生平`。放大卡扩展为 `760×420`，画像显示区增至 `150×232`，同时呈现年代、身份、三条生平、资料来源和关闭操作。竺可桢原有“两问”控制器门禁保持不变，其余人物查阅不写章节进度。
- 自动验证通过：`npm run typecheck`、第四章资产校验（`alumniPortraits=20+transparent+referenced`）、第四章拓扑 `3009` 项、普通生产构建与 `git diff --check`。按用户当前要求没有执行 `build:single`、`verify:single`，也没有更新 `demo/index.html`。
- 浏览器实机复核覆盖 A1 与 A3：A1 王淦昌画像可直接点击并打开大图生平卡；A3 三联框与杨卫独立竖框正确嵌入底图，杨卫画像可点击并打开大图生平卡。两条路径均无 console/page error。临时截图在记录结论后删除；本轮没有执行 Git fetch、暂存、提交、合并、推送或上传。

## 2026-08-30 第四章跨楼层电梯档案与停靠链复核

- 将原三块楼层按钮重构为 `A 楼主电梯 / 18:50 运行复核` 面板。1F、2F、3F 分别显示楼层用途、当前层／已归档／可直达／楼梯绕行状态、该层可读取的运行档案和总收集进度；2F 在错位楼梯完成前明确提供绕行信息，不再反复显示笼统的“楼层未开放”。数字键和方向键选择楼层，`Enter` 执行直达或读取当前层记录，`Space` 在条件满足后打开停靠链推理。
- 新增三条可按任意顺序收集的深色观察档案：1F 门体开放八秒与乘客进入六秒，2F 下行外呼在门未开启时持续亮灯，3F 于 `18:50:12` 发出到站提示并开门。每条记录都要求玩家实际到达对应楼层并在深色模式读取；浅色模式只负责复核和提交，不再强制固定的楼层调查顺序。
- 三条档案收齐后，浅色模式开放停靠链复原：玩家分别判断“轿厢实际到达层”和“未被响应的外呼层”。错误组合只给出冲突反馈且不写进度；正确的 `3F / 2F` 组合写入控制器事实 `elevator_stop_chain_reconstructed`，并成为安装 204 教室定位片的前置条件。
- 任务栏新增“复核三层停靠记录，确认定位片的楼层基准”，并将 1F 起行轨、2F 外呼日志、3F 到站记录显示为三个并行调查节点。存档白名单、旧存档迁移、阶段闭包、DEV 检查点、运行时调试信息和章节文本导出同步接入；导出后 `docs/game-text-by-chapter.md` 共 `6779` 条文本，其中第四章 `1611` 条。
- 没有新增测试文件、测试框架或测试依赖，只扩充现有 validator。Fresh 自动验证通过：`validate:quick` `4/4`、`validate:critical` `20/20`、第四章故事、有效交互 `624` 项、拓扑 `3009` 项、运行时 `1107` 项、Task 14 `376` 项、普通生产构建、JSON 解析和 `git diff --check`。
- 真实浏览器完整链路覆盖 Blink、Gecko、WebKit `1280×720`：三内核都完成 2F 记录读取、深浅模式切换、错误答案零写入、正确答案提交，最终进度为 `3/3` 且停靠链事实成立；page/console error 均为 `0`。视觉复核后已删除临时截图和 QA 状态文件。
- 交付边界：继续使用既有 `selected-phaser-ui-20260721` 工作区，没有创建新工作区；按当前要求未执行 `build:single`、`verify:single`，未更新 `demo/index.html`；也没有执行 Git fetch、暂存、提交、合并、推送或上传。

## 2026-08-30 语音备忘录浅色界面恢复与播放暂停修正

- 根据用户澄清，将“红黑配色”限制在录音播放控件。删除语音备忘录页面对全局浅色证据界面的深色覆盖，页头、步骤说明、录音卡、事件标签、排序按钮、反馈和主操作全部恢复到原有浅灰纸面、青蓝信息层级；筛选页和 `2 / 2 排列顺序` 页保持相同视觉规则。
- 圆形播放控件采用局部红黑状态：未播放或暂停时为黑底红色播放三角，播放中为红底黑色暂停符号。按钮的可访问名称同步为“播放／暂停／继续播放”，不再以停止方块代替暂停。
- 播放逻辑新增真实暂停位置。暂停会停止当前音频并记录毫秒位置，继续播放时从该位置重新触发剩余片段；试听声音事件也可暂停后续播。切换录音、切换整理阶段、页面隐藏、剧情重置和退出页面仍会清理当前及暂停态，避免残留音频继续播放。
- Fresh 自动验证通过：`npm run typecheck`、3.5 章并行流程 `1957` 项断言／`48` 种完成顺序、语音素材合同 `7` 段录音／`4` 条正式路线／`3` 条干扰记录、普通生产构建和 `git diff --check`。按现有要求没有执行单文件构建，也没有更新 `demo/index.html`。
- 标准 web-game Playwright 客户端完成播放状态截图复核；补充浏览器链路覆盖 Blink、Gecko、WebKit，三内核均验证“播放 → 暂停 → 从约 `0.4s` 位置继续播放”，按钮颜色与可访问名称同步变化，page/console error 均为 `0`。另在 Chromium 中恢复四段排序草稿，确认排序页为浅色、四张卡完整显示，`CLIP D7` 可暂停并继续。
- 本轮继续使用既有工作区，没有创建新工作区，没有新增测试文件或测试依赖，也没有执行 Git fetch、暂存、提交、合并、推送或上传。临时浏览器截图与 QA 脚本在记录结论后删除。
## 2026-08-29 第三、四章画面内任务栏与共享遮罩

- 第三章移除 RPG 画面外的桌面任务栏入口及其遗留样式，与第四章统一由 `RpgGameHost` 在同一个 `rpg-shell` 内渲染 `rpg` 任务栏。章节只提供任务数据，不再分别决定任务栏的固定方式；打开抽屉后面板和按钮也沿用同一条交互路径。
- 复用方式收敛为一个共享 `.rpg-overlay-layer` 样式类，没有新增 React 包装组件。任务抽屉、第四章五区拓扑／配电、维修诊断、插入谜题和启真湖相机继续保留原组件结构，只各自声明透明度、内边距和层级；原来重复的定位、全覆盖、居中和遮罩规则被删除。
- 第四章深浅模式不再绘制包围整个画面的青绿色或金色边框；深色压暗、边缘暗角、扫描纹、模式按钮和对应谜题荧光仍保留。校史人物界面的画布遮罩由错误的 `y=40…540 / 960×500` 直接修正为 `y=0…540 / 960×540`，没有为这一行修复新增抽象文件。
- Blink、Gecko、WebKit 在 `1280×720`、`1180×800` 和 `390×844` 共 `9` 个组合中验证第三章任务抽屉与第四章插入谜题：遮罩边界均与 RPG 壳一致，面板位于壳内，页面无横纵溢出，console/page error 为 `0`。用户标注的 `836×898` 校史人物界面在三个引擎中均测得遮罩逻辑边界为 `0,0–960,540`，顶部不再漏出地图。
- 精简后本轮源码差异为新增 `47` 行、删除 `137` 行，净删除 `90` 行；没有保留 `RpgOverlayLayer.tsx` 或 `RpgCanvasOverlay.ts`。`npm run typecheck`、第四章 runtime `1095` 项、有效交互 `624` 项、现实模式顺序 `180` 项、任务提示所有权、启真湖日志和 `git diff --check` 均通过。
- `npm run build:single` 的 Vite 子进程在本机约 `3GB` 堆限制处退出；改用工作区自带 Node 显式提供 `6GB`，执行同一 `tsc --noEmit` 与 `vite build --mode demo` 后成功。`npm run verify:single` 通过，离线 `demo/index.html` 为 `258653130` 字节、两个内联脚本和一个内联样式。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；既有 `1456` 个未跟踪 Godot、启真湖素材、辅助脚本和第三章文案保持未动，专用浏览器、截图、下载的测试浏览器与 QA 脚本已清理，既有 Vite 服务继续运行。

## 2026-09-01 第二章任务回跳、室内纹理／门体与启真湖入口修复

- 第二章首次完成系统红圈对话时，在不提前离开手机的前提下将 RPG 现场预置为 `dorm_hub / dorm_spawn`；任务栏“返回任务现场”现在进入寝室拿校园卡，不再沿用初始 `campus_bootstrap` 而误入校园地图。对应控制器验证同时覆盖幂等返回、任务 ID、任务文案和目标界面。
- 修复图书馆同一 Phaser Scene 经 DEV 节点重启后的底图放大错位。原因是底图纹理后续加入书架、前台和书包裁剪帧后，无 frame 的 `add.image` 在重启时会取到裁剪帧；图书馆底图与共享门前景裁片现均显式绑定 `__BASE`，交互和碰撞坐标保持不变。
- BD 四位口令成功进入 `top_ten_reached` 后，任务栏立即前进到“提交恢复申请”。点击“前往相关界面”会通过控制器打开恢复申请状态，并直达浙大钉 `022座位恢复申请` 材料页，不再停留在“让帖子被看见”或浙大钉首页。
- 寝室、食堂和剧场三处共享动画门删除了运行时透明碰撞矩形及食堂专用门碰撞组；开门动画、前景遮挡、拒绝反馈和延时出场演出继续保留。室内门验证器改为禁止共享 blocker，并检查三个消费者均未重新声明空气墙。
- 启真湖入口可见性收口为控制器只读 selector：`location_search` 阶段只有桥边、倒影、湖三个关键词全部收集后才显示压暗入口，地点确认后才可进入；已经推进或迁移到后续阶段的存档仍保持入口可见可用。验证覆盖七种未集齐组合、全收集未确认和后续存档兼容。
- Fresh 验证：普通生产构建通过；`validate:quick` 为 `4/4`；第二章登录／任务回跳、启真湖雨天与入口、室内门三个定向验证均通过，室内门共 `48` 项断言。`validate:critical` 的代码与状态机项目为 `18/20` 通过；剩余两项仍是本机没有 `ffprobe`，分别在追逐音频探测报 `spawnSync ffprobe ENOENT`、在 Task 14 音频时长探测拿不到 stdout，与本轮改动无关。文本清单已刷新为 `6780` 条。
- Chromium `1440×900` 真实链路复核三条路径：`c2-seat-arrival → c2-occupancy-note` 同场景切换后底图、玩家和交互位置一致；系统红圈五句对话后打开任务栏并回到寝室；BD `3027` 成功后任务 ID 变为 `chapter_two_submit_recovery`，任务按钮直达三项材料页。三条路径均为一个稳定 canvas、文档横纵溢出 `0`、page／console error `0`。临时浏览器配置、脚本和截图已清理。
- 本轮未执行 Git fetch、暂存、提交、合并、推送或上传；既有未跟踪 Godot、启真湖素材、辅助脚本和第三章文案保持未动。

## 2026-09-02 第四章跨关卡因果衔接、环境线索与渐进辅助

- 第四章新增 `11` 条证据生产／消费合同和 `30` 个源像素环境细节，其中 `10` 条线索允许跨越多个阶段后再被消费。关卡推进改由已记录的方向、时间、空间、身份和供电事实驱动，阶段位置不再自动补齐缺失证据。
- Room 204 将十二个旧家具片收束为四组摆放，每组一次操作同时落下三个兼容片段；旧存档仍按十二片读取，新流程重复同一操作模板最多四次。维修盖板与润滑油继续共用一次有效操作，配电答案最多三次切换。
- 时间变化由四个独占转场覆盖层承接，场景移交由四个世界内接续点承接；转场期间字幕层暂停接收消息，避免任务栏、提示框、字幕和转场重复说明同一件事。A3 参照仅在深色观察记录，竺可桢两问保留到最终楼外闭幕，浅色配电只写入供电路线事实。
- 自适应辅助保持运行时状态，失败计数上限为四次：原始环境、局部视觉增强、局部闪动与位置声、同时强调两项待比较的原始事实。成功、阶段离开和场景销毁都会清空计数；辅助不写存档、不生成结论、不新增提示框。
- Fresh 自动验证通过：`typecheck`、第四章因果合同（`11` 条合同／`30` 个细节／`10` 条跨阶段复用／`4+4` 类转场）、第四章 story、runtime `1093` 项、topology `3009` 项、Task 14 `381` 项、`validate:critical` `21/21`、单文件构建及结构校验。文本导出已刷新为 `6816` 条，其中第四章 `1654` 条；`git diff --check` 通过。
- 标准 web-game Playwright 客户端完成第四章真实移动与状态导出；补充浏览器链路覆盖 Blink、Gecko、WebKit 的 `1280×720`、`1440×900` 和 `390×844` 共 `9` 个组合。Room 204 检查点在三内核均进入 `room204_restore`，RPG 画布保持 `16:9`，非 16:9 窗口使用 `1440×810` 画布，无横纵页面溢出、console/page error 为 `0`；开发通道关闭后键盘输入恢复，活跃目标为 `10` 个。临时截图和 QA 脚本在目视确认后删除。
- 预交付审查补齐最终门禁：A3 荣誉墙只保留人物生平，错位楼梯只读取三楼晨间教室参照；竺可桢两问移至楼外未点亮灯前，由一个提交动作原子保存两项答案，界面只确认 `回答已保存`，随后才创建正式点灯会话。对应 DEV 闭幕检查点补齐跨关卡保留的身份、纸条、Room 204 与供电事实，同时明确不预填两问。
- 最终问答链在生成的 `demo/index.html` 中通过 Chromium 桌面、Chromium `390×844` 与 Firefox：答题前只有一个问答层，提交后只有一条 `回答已保存`，约 `900ms` 后切换为唯一点灯层；桌面路径继续运行至 `phase=complete` 并写入正式收束事实。桌面与移动截图完成目视检查，问答无裁切、无文档溢出。WebKit 对 Vite 源码态完成同一答题到点灯链且无 console/page error；269MB 单文件在本机 WebKit 首次载入超过 `180s`，该组合未记为通过。

## 2026-09-02 第四章 A1 前台签到板落位修正

- 前台值班签到板先从 `(751, 582)` 移到 `(835, 610)`；根据第二次画面标注继续向右下移动到 `(872, 625)`。源像素交互框同步更新为 `x=820 / y=591 / 104×68`，保持 `52px` 交互距离；图片和热区完整落在前台桌面右侧范围内。
- 第二次右下移动后，`npm run typecheck`、第四章有效交互 `624` 项断言、`git diff --check` 与 `npm run build:single` 通过，单文件为 `269043742` 字节。标准 web-game 客户端确认运行时交互框为 `x=820 / y=591 / 104×68`；当前 in-app Browser 在 `710×739` 窗口目视确认签到板位于桌面右侧、没有越出桌沿，页面溢出和 console/page error 均为 `0`。
- 本轮没有执行 Git 暂存、提交、合并或推送；修改仍位于隔离交付 worktree，等待用户确定后续上传范围。

## 2026-09-02 第四章纸条拾取道具栏排版与视觉统一

- 第四章右下角道具栏新增独立视觉层：改为与章节任务栏一致的深蓝灰底、浅金标题、蓝灰边框与像素字体层级；共享道具组件、拖放命中、单击纸条查看和其他章节样式保持不变。
- 第四章道具卡从窄单行省略改为 `78×70px` 卡片和固定 `22px` 文字区，标签允许受控换行；`电子校园卡` 与 `签到记录纸` 的文字区均为 `clientWidth=scrollWidth=64`、`clientHeight=scrollHeight=22`，文字和卡片全部位于 RPG 壳内。
- Fresh 验证通过：`npm run typecheck`、第四章 runtime `1093` 项、有效交互 `624` 项、任务提示所有权、`npm run build:single`。单文件输出为 `269040.06 kB`。
- 标准 web-game Playwright 客户端在 `c4-755-hall-clock` 检查点确认纸条已进入道具栏，`ownedItems` 为 `campusCard + attendanceRecordPaper`，画面为 `1280×720`、一个 Phaser canvas、console/page error 为 `0`；in-app Browser 的 `710×739` 窗口同时完成边界复核，错误日志为 `0`。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改仍位于隔离交付 worktree。临时浏览器截图和状态文件在目视确认后删除，既有 Vite 服务继续运行。

## 2026-09-02 第四章电梯谜题 HUD 专注态与校准面板重排

- 将第四章电梯楼层、历史校准和停靠链三个 Phaser 面板统一纳入独占专注态。面板打开时，React 任务栏、返回／全屏按钮、现实模式切换、道具栏、字幕和场景底部交互提示同步退出；面板关闭后同一批常驻 HUD 自动恢复，Phaser 面板自身的鼠标和键盘输入继续可用。
- 三个电梯面板统一增加全画布压暗背景、`720×420` 双层直角框、金色章节标记和右上角可点击的 `× 返回`。返回按钮在 `710×739` 实际窗口中完成点击验证，关闭后任务栏、系统按钮、模式切换和道具栏数量均由 `0` 恢复为 `1`。
- “同步电梯历史”重排为四层信息：档案来源、校准目标、双轨时间图、读数与操作。轨道扩展至 `540×64`，门体和人物标签移到轨道外侧；三项读数左对齐；按钮改为 `提前 1 秒 / 重放并校验 / 延后 1 秒`，主操作使用金色强调，底部只保留一条键盘说明。
- `c4-755-elevator-history` 开发检查点在开发环境自动进入电梯面板，方便直接复核该 UI；DEV 检查点仍只使用 session state，正式游戏与单文件构建不自动打开面板。
- Fresh 自动验证通过：`npm run typecheck`、第四章 runtime `1093` 项、有效交互 `624` 项、topology `3009` 项、Task 14 `381` 项和 `npm run build:single`；单文件输出为 `269042.87 kB`。
- 标准 web-game 客户端完成检查点进入并由文本状态确认 `panelOpen=true / phase=selecting`；它的 Phaser 画布抓取错误选择了内部窄画布，因此补用同一 Playwright 运行时做全页截图。`1280×720` 与 in-app Browser `710×739` 都只有一个 Phaser canvas、页面横纵溢出为 `0`、console/page error 为 `0`，完整画面已目视确认。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改仍位于隔离交付 worktree。临时 QA 截图和状态文件在记录结论后清理，既有 Vite 服务继续运行。

## 2026-09-02 第四章一楼至三楼电梯按钮与跨关卡线索修复

- 修复“3F 显示可直达、点击前往后没有反应”的状态不一致。根因是楼层面板按电梯校准事实判定 3F 可达，控制器仍把未完成的一楼值班板当成跨层硬门槛；拒绝反馈又绘制在电梯面板下方，因此玩家看到的是静默点击。
- 一楼两处教室记录、电梯历史观察与校准完成后即可乘电梯前往三楼；一楼值班板保留为可跨关卡回收的原始线索，并在 Room 204 投影提交前由控制器再次校验。任务栏进入三楼时切换到三楼参照调查，完成三楼、错位楼梯、二楼三项现场记录及停靠链后，如值班板仍未完成，再引导回一楼补齐。
- 楼层面板新增面板内反馈行。二楼尚未接通等合法拒绝会直接在当前面板说明原因和下一步操作，不再依赖被遮住的场景反馈层；切换楼层会清除旧反馈。
- 现有 runtime validator 新增跨层回归：缺少值班板但具备教室与电梯事实时，A1→A3 请求必须被接受；缺少值班板时，Room 204 最终投影必须零写入拒绝。第四章 runtime 现为 `1099` 项断言；story、有效交互 `624` 项、topology `3009` 项、Task 14 `381` 项和 `npm run build:single` 均通过，单文件输出为 `269043.74 kB`。
- Chromium `710×739` 实际链路完成“历史校准 → 选择 3F → 点击前往 → 关门 → 到达 A3”，按钮点击后状态为 `panelOpen=false / phase=closing / targetFloor=3`，到站后控制器楼层与场景楼层均为 3、阶段回到 `idle`，console/page error 为 `0`。同一窗口确认锁定的 2F 会在面板底部显示红色绕行说明。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree，既有 Vite 服务继续运行。

## 2026-09-02 第四章讲台边界、配电状态同步与成功接续

- A1 104 教室讲台桌前沿新增源像素空气墙 `x=1278 / y=229 / 290×19`。共享人物脚部碰撞框在真实 Phaser 行走中停于 `body.top=248`，与标注线南沿 `y=248` 精确重合；人物仍可在黑板记录点的 `96px` 交互范围内完成调查。
- 配电箱全亮 `mask=31` 继续保留为未完成状态，并显示“总负载过高。核对已记录的必要路线，关闭旁路回路。”；界面没有提前写入答案。左侧实体箱的五个开关指示灯改为逐项读取当前 mask，与右侧五区按钮同步亮灭。
- 正确配电结果 `mask=13` 锁定后，由 Phaser 场景播放 `1320ms` 成功接续：三段必要路线依次亮起，实体箱显示“回路稳定”并播放确认音效；演出期间场景输入关闭、追逐守卫停驻，演出结束后关闭箱门、恢复输入并进入 `final_chase`。
- Blink `1280×720`、Gecko `1280×720`、WebKit `1180×800` 真实链路均验证两条分支：初始 `mask=14` 和全亮 `mask=31` 的左右五项状态逐一一致；正确解法进行中为 `storyPresentation=power_grid_success / chase=arming / input=false`，结束后为 `storyPresentation=idle / chase=running / input=true`。三个内核均保持一个 Phaser canvas、页面横纵溢出为 `0`，page／console error 为 `0`；Blink 额外确认演出期间 `audioBand=null`，结束后切换到 `audioBand=tracking`。
- Fresh 自动验证通过：`npm run typecheck`、第四章 causal-flow、story、runtime `1099` 项、topology `3043` 项、有效交互 `624` 项、Task 14 `384` 项、资产、守卫呈现、阶段预热与 `npm run build:single`；单文件输出为 `269050.56 kB`，gzip `192619.10 kB`，`git diff --check` 通过。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree。临时浏览器截图和 QA 状态文件在目视确认后删除。

## 2026-09-02 第四章“最后一分钟”黄铜分针组件统一

- 将 202 阶梯教室中原有的青色晶体状“最后一分钟”精灵替换为像素风黄铜分针组件。新图集为 `chapter4_story_items_v02.png`，其余七个图集单元与 v01 像素级一致；正式清单、来源哈希、运行时导入和资产说明同步切换到 v02，旧 v01 已从工作树移除并可通过 Git 恢复。
- 剧情仍使用“最后一分钟”描述时间目标，世界物件、交互提示、道具卡、像素图标和详情页统一显示“黄铜分针组件”。道具目录从纸张类修正为实体类；详情页显示来源为 `202 阶梯教室座椅间`、分类为 `钟表部件`，并指向大厅旧钟分针端点。
- 删除物件上方直接显示答案的青色 `07:54→07:55` 字样，改为贴近黄铜轴座的低透明磨损线；环境只保留可观察的材质细节和靠近后的操作提示。
- 修复 `c4-755-final-minute` 开发检查点缺少 `room202_route_reached` 的问题。此前检查点能显示物件，但控制器会拒绝拾取；现在该入口表示“已抵达 202、尚未拾取”，Task 14 增加对应回归断言。
- Blink、Gecko、WebKit `1280×720` 均完成 `Space` 拾取，三内核都从 `final_minute_recovery` 进入 `return_to_clock`，目标实体消失、合同失败为空、console/page error 为 `0`；`1180×800` 与 `390×844` 的三内核补充检查均保持 `16:9` 画布且页面横纵溢出为 `0`。Chromium 额外打开道具详情页，确认图标、分类、来源、说明和用途完整显示且无溢出；标准 web-game Playwright 客户端完成最终运行态导出和画面复核。
- Fresh 自动验证通过：`npm run typecheck`、第四章 causal-flow（`11` 条证据／`30` 个细节）、story、runtime `1099` 项、topology `3043` 项、有效交互 `624` 项、Task 14 `385` 项、assets、守卫呈现、阶段预热、`npm run build:single` 与 `git diff --check`。离线单文件输出为 `268728.07 kB`，gzip `192379.73 kB`。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree。临时 QA 脚本、截图和状态文件在记录结论后清理，既有 Vite 服务继续运行。

## 2026-09-02 第四章 202 教室脚部判定线校准

- 按浏览器标注将 202 教室黑板下沿源像素 `y=126` 设为脚部判定的北界。新增静态碰撞 `a2_air_wall_room202_blackboard_foot_limit`，范围为 `x=1166 / y=107 / 432×19`，南沿与标注线精确重合，并封住两侧房间墙之间的完整通行宽度。
- 本次只约束共享人物脚框的北沿。人物头部和上半身仍可进入黑板区域，保留俯视室内的正常视觉重叠；没有增加肩部判定、整身判定或黑板前景遮挡。
- 标准 web-game Playwright 客户端从 `c4-755-final-minute` 持续向北移动后确认 `player.y=101.625`、`collisionBounds.y=126`、合同失败为空；关闭调试碰撞层后的画面已目视检查。原地 `Space` 仍能拾取黄铜分针组件并进入 `return_to_clock`。
- Blink、Gecko、WebKit `1280×720` 实走均确认：首次抵线与继续按住向上时脚框北沿均保持 `126`；向下可立即脱离；重新进入检查点后均能拾取组件并进入 `return_to_clock`，console/page error 为 `0`。
- Fresh 自动验证通过：`npm run typecheck`、第四章 topology `3074` 项、runtime `1099` 项、有效交互 `624` 项、Task 14 `385` 项、causal-flow、story、assets、`npm run build:single`、`npm run verify:single` 与 `npm run verify:browser-smoke`。离线单文件为 `268728357` bytes。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree。临时 QA 脚本、截图和状态文件在记录结论后清理，既有 Vite 服务继续运行。

## 2026-09-02 第四章保洁推车动画连续性与尺寸统一

- 修正维修阶段保洁从待机动作切换到推车动作时的跳位与比例突变。原流程会销毁 `0.65` 倍独立推车，把 `96×128` 人物直接替换为 `192×128` 人车合成帧，动作结束后又改成轮廓更宽的 `cleaner_rest`，三个视觉包围盒无法连续。
- 推车现全程保留同一个 `cleaning_cart` 实例、同一位置基准和同一 `0.65` 倍缩放；推车动作改用 `cleaner_push_cart` 的人物区域，水平翻转后与位于左侧的原推车重合连接。八帧源图逐帧检查后保留 `0,0 / 96×128` 裁切，边缘中的把手部分会覆盖在原推车右缘，不再绘制第二个完整推车。
- 失败尝试改为人物动作层、待机人物层和推车层同步向左试推 `5px`，人物两种姿态在动作起止交叉淡化；轮轴修复后的正式推行改为三层同步向左移动 `66px / 900ms`，动作末端恢复同规格 `cleaner_idle`，不再切换到 `cleaner_rest`。
- 布局合同与三处既有校验规则已同步记录人物裁切、水平朝向和新位移方向。本轮依照用户要求未运行这些脚本，未执行 `typecheck`、单文件生成、浏览器自动化或跨内核检查；当前结论只覆盖源码修改和八帧素材逐帧检查。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree，既有 Vite 服务继续运行。

## 2026-09-02 第四章人物完整留在楼层地图内

- 项目规则新增第四章人物外边界合同：共享 `96×128` 人物帧的四条外沿必须全部留在当前 `1672×941` 楼层源图内。内部黑板、墙面和柜台仍使用脚部碰撞与前景深度遮挡，这条规则只处理楼层地图外沿。
- `RpgPlayerTextures.ts` 新增统一视觉余量计算，直接读取共享人物帧、`0.65` 显示倍率与固定脚框数据。当前得到左／右 `21.45px`、上 `65.975px`、下 `2.6px`；场景没有复制另一套人物宽高。
- 第四章在每次创建场景、乘电梯到站、楼梯换层和外部状态同步换层时，继续把 Phaser 物理世界与相机设置为完整楼层矩形，同时给玩家脚框设置对应楼层的专属 `customBoundsRectangle`。脚框到达专属上边界时，人物完整帧上沿正好停在地图 `y=0`。
- 运行时调试状态新增 `player.visualBounds` 与 `player.movementBounds`，后续可以直接比较人物视觉四边和楼层源图四边。Task 14 既有校验规则已同步登记共享计算与逐层安装要求。
- 依照用户上一条要求，本轮未执行 `typecheck`、第四章校验脚本、单文件生成、浏览器自动化或跨内核检查；当前只完成规则、源码和校验定义更新，实际移动结果仍待后续运行确认。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree，既有 Vite 服务未做操作。

## 2026-09-02 第四章错位楼梯篇幅与空间特效扩展

- 教学楼平面场景新增楼梯异常预告层：每个当前楼层落口绘制错位轮廓、离散梯线与 `18` 个漂浮碎片；A3→A2 正式入口使用 `30` 个碎片，完成三楼参照记录后提升至完整强度。该层只在 `room204_restore` 且错位楼梯尚未完成时显示，始终位于人物层下方，不参与碰撞、热区或通行判定。
- Three.js 两关分别新增 `30`／`46` 个硬边漂浮构件、`3`／`5` 组多边形能量环和 `96`／`144` 个离散尘点。所有对象集中在独立 `effectRoot`，没有注册为可走面、机关命中面或遮挡面，视觉密度不会改变导航图与投影接缝答案。
- 正式楼梯流程扩展为连续五段：入口空间偏移建立、第一关投影解谜、第一段结构分离、第二关垂直错层解谜、完整路径点亮与 A2 落点确认。第一关仍使用三件机关和一次投影连接，第二关仍使用四件机关、两次投影连接与一次错误重合，没有增加重复操作模板。
- 每关到达出口后先沿作者节点顺序播放青色路线和前导亮点，再开门、入门并进入下一段演出。第二关最终演出持续完成后才调用现有 `onComplete`，正式章节直接提交唯一 `complete_misaligned_stair` 意图；通用结果卡只保留给独立试玩入口。
- 场景 UI 新增 `01/02` 关卡进度、段落标题、扫描线、四角锁定、硬切帧移和成功脉冲；减弱动态偏好会把新增演出缩短为单帧并关闭 CSS 动画。设计文档、调试阶段联合类型与项目规则已同步更新。
- 依照用户当前要求，本轮未执行 `typecheck`、第四章校验脚本、单文件生成、浏览器自动化或跨内核检查；当前结论覆盖规则、源码、样式和设计文档修改，实际画面与完整可玩链仍待后续运行确认。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree，既有 Vite 服务未做操作。

## 2026-09-02 第四章路线板指令与返程目标语义修正

- 修正开放自习区路线板的空间逻辑。旧版把“开放自习区”当起点、把“202 出口”当终点，无法表达疏散目的，也无法支撑后续主楼梯追逐与返程；现改为从 `202 教室门口` 经 `东侧走廊`、`交通核心` 到 `主楼梯下行口` 的四段真实路线。
- 深色观察不再直接列出起点、中段和终点答案，改为呈现四处鞋印的朝向、连续纹路、外缘磨损和楼梯黄线内收束痕迹。浅色操作在卡片上方固定展示起点与终点，排序按钮从含混的左右箭头改为 `上移`／`下移`。
- 路线事实继续跨阶段使用：停电配电任务要求对照已记录的疏散线选择必要灯区；追向 202 时反向使用该路线；取回组件后按疏散方向返回主楼梯。相关任务文案不再只说“最后一分钟”，统一写明 `黄铜分针组件`、当前目的地和动作。
- 本轮依照用户要求未执行 `typecheck`、校验脚本、单文件生成、浏览器自动化或跨内核检查；当前只完成源码、内容数据、样式与项目规则修改，实际画面和交互结果仍待后续运行确认。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送或上传；修改继续保留在隔离交付 worktree，既有 Vite 服务未做操作。

## 2026-09-02 第四章黄铜分针组件旧钟拖放热区修复

- 根因确认：最终分针的运行时投放区沿用了端点装饰的 `16 × 16` 边界，坐标为 `{x:967,y:26,width:16,height:16}`；该区域贴近任务栏遮挡区，也没有覆盖玩家实际看到并尝试投放的旧钟表盘，因此合法状态下仍返回 `missed_target`。
- 运行时修复：将端点拆成 `visualHandleBounds` 与 `installationBounds`。端点光点继续使用实测的 `16 × 16` 视觉边界；拖放区改为旧钟完整可见表盘 `{x:940,y:30,width:122,height:120}`，共享交互合同、场景动态目标和楼层交互数据使用同一边界。
- 文案修复：任务提示、道具使用引导、道具详情和目标名称统一要求玩家靠近旧钟后拖到“可见表盘内”，不再要求瞄准画面中难以识别的小端点。
- 合同同步：更新已有第四章布局、素材与剧情校验定义，分别检查视觉端点和可接受表盘投放区，避免后续再次把装饰边界当成交互边界。
- 按用户当前要求，本轮没有生成 `demo/index.html` 单文件，也没有运行 typecheck、校验脚本、自动化浏览器操作或整套测试；当前记录仅确认源代码与合同已经同步修改。
- 本轮没有执行 Git 暂存、提交、合并、推送或分支操作。

## 2026-09-02 RPG 贴图加载恢复层与无损压缩策略

- 全局资源盘点确认 `src` 代码直接引用约 `230` 张图片、压缩文件合计约 `142 MiB`；`40` 张运行时图片超过 `1 MiB`，`21` 张超过 `2 MiB`。两张校园地图的文件体积各约 `22–23 MiB`，解码后各约 `57–59 MiB`；自行车 `273` 帧图集解码约 `63.3 MiB`。
- 新增 Phaser Scene Loader 的统一状态上报。所有主 RPG Scene 都会通过共享人物预载入口自动接入，进入场景前的加载开始、进度、最终失败项与完成状态由同一个注册表回调交给 React Host；Scene 已运行后的预测加载保持静默，不会打断操作。
- Phaser 图片加载并发从桌面默认最多 `32` 项收紧为 `4`，保留每项 `2` 次自动重试并设置 `15s` 超时。资源未完整载入时，React 层锁定场景输入、隐藏任务与系统按钮，并显示同画风进度卡；失败后可以原地重启当前 Scene 或返回手机，控制器状态和存档保持不变。
- 无损压缩使用临时副本实测并逐像素比较。无损 WebP 对北向校园图、横向校园图、启真湖直河道和第四章面包坊分别节省约 `29.5% / 20.0% / 21.8% / 52.2%`，对自行车大图集反而增大约 `2.2%`，因此禁止统一格式转换。正式策略要求像素完全一致且至少节省 `15%` 才允许替换运行时格式。
- 新增 `docs/plans/2026-09-02-rpg-texture-delivery-strategy.md`，固化 L0 当前必需、L1 当前场景后续、L2 相邻场景预测三层加载、选择性无损压缩、失败恢复和后续纹理释放顺序。
- 依照用户当前要求，本轮没有生成单文件，也没有运行 typecheck、校验脚本、自动化浏览器操作或跨内核检查；Vite 仅被动完成 HMR，控制台未输出编译错误。贴图二进制尚未批量替换，等待后续允许进行场景级目检后再执行首批转换。
- 本轮没有执行 Git 暂存、提交、合并、推送或分支操作；修改继续保留在隔离交付 worktree。

## 2026-09-02 第四章三层桌椅碰撞网页标注器

- 新增开发专用入口 `chapter4-collision-editor.html`，统一展示 A1、A2、A3 三张 `1672×941` 源像素楼层图；每层还可切换正式剧情使用的不同时间状态底图，切换底图不会改变或清空碰撞草稿。
- 标注规则直接参照食堂：一张桌子及其四周座椅使用一个独立矩形，桌间走道保持可通行。页面以红色显示现有结构碰撞，以黄色显示本轮草稿，当前选中框使用青色；已有结构碰撞只作为参照，不写入导出结果。
- 支持连续拖框新建、拖动、八方向缩放、数值精调、楼层独立草稿、跨楼层撤销、删除、清空本层、sessionStorage 防刷新丢失，以及复制／下载包含三层源像素坐标的 JSON。框选后继续保持框选模式，批量标完后再按 `V` 进入调整；新增 `window.render_collision_editor_to_text()` 供调试状态读取。
- `package.json` 新增 `npm run dev:chapter4-colliders` 稳定入口。依照用户当前范围，本轮没有改动正式 `staticCollisions`，没有生成单文件，也没有运行 typecheck、校验脚本、自动化测试或跨内核检查；只准备网页标注入口供用户实际框选。
- 本轮没有执行 Git 暂存、提交、合并、推送或分支操作；修改继续保留在隔离交付 worktree。

## 2026-09-02 第四章 A1 大厅南门接近动画

- 按浏览器框选将 A1 大厅南侧玻璃门门槛绑定为源像素接近触发。人物脚点进入 `{x:720,y:796,width:246,height:105}` 后，中央双扇门会自动开始打开；已打开时使用更大的 `{x:678,y:770,width:330,height:171}` 保持区，减少人物在边界附近移动时反复开合。
- 门体没有另造一套画风。运行时从当前 A1 状态图裁取左右门扇以及两块相邻玻璃作为门洞底层，使用双扇折叠动画打开；状态图在白天、夜间、停电或维修阶段切换时，门体也会重建为对应光照版本。
- 原有整块入口前景在门动起来后降回背景深度，固定横梁和两侧玻璃改由三块同源裁片继续承担人物遮挡，避免门扇打开后仍显示一张闭门图，也保留人物经过门框时的俯视遮挡关系。
- 运行时调试状态新增 `chapterFour.mainEntranceDoor`，可读取当前状态、进度、触发范围、保持范围、人物脚点、活动底图和是否请求开门；共享室内门运行时补充显式销毁入口，供状态底图切换时无残留重建。
- 依照用户当前范围，本轮没有生成 `demo/index.html` 单文件，也没有运行 typecheck、校验脚本、自动化浏览器操作或整套测试；当前结论只覆盖源码、源像素合同和状态图裁片设计。
- 本轮没有执行 Git 暂存、提交、合并、推送、上传或分支操作；修改继续保留在隔离交付 worktree，既有 Vite 服务未做操作。

## 2026-09-02 第四章 A1 用户框选家具碰撞导入

- 读取用户从三层网页标注器导出的 `chapter4-furniture-collision-draft/v1` 数据：A1 共 `32` 个草稿，A2、A3 均为空。本轮只落位用户实际提交的 A1 坐标，没有为尚未框选的 A2、A3 推测碰撞。
- A1 正式加入 `28` 个有效阻挡：104／105 两间教室共 `24` 组课桌阻挡、前台两侧 `2` 组座椅／盆栽阻挡、主楼梯两侧 `2` 条边缘阻挡。右侧楼梯边缘的下沿从导出 `y=229` 收回到 `y=227`，仅消除与既有必经出口的 `2px` 交叠。
- 排除 `a1_furniture_015`、`a1_furniture_017`、`a1_furniture_018`、`a1_furniture_037` 四个 `4×4` 单击误点。标注器新增单击防误建逻辑；旧会话中的 `4×4` 误点在读取和导出时都会过滤，窄而长的栏杆框仍可正常保留。
- 第四章碰撞数量契约同步为 A1 `44`、A2 `24`、A3 `26`。本轮仅做 JSON 解析、ID 唯一性、源图边界、既有结构碰撞重叠、必经区域重叠和 `git diff --check` 核对；结果为 `28` 个导入框全部位于 `1672×941` 源图内，无重复 ID、无 `4×4` 框、无既有碰撞重叠、无必经区域重叠。
- 依照用户当前要求，本轮没有生成单文件，没有运行 `typecheck`、项目校验脚本、浏览器自动化、跨内核检查或整套测试；标注器防误点和正式 Phaser 实走仍未做运行验证。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送、上传或分支操作；修改继续保留在隔离交付 worktree。

## 2026-09-02 第四章 A3 用户框选家具碰撞导入

- 第二批 `chapter4-furniture-collision-draft/v1` 导出包含 A1 `32` 个、A2 `0` 个、A3 `42` 个草稿。A1 与上一批坐标一致，本轮没有重复写入；A2 继续保持原碰撞；A3 新增 `37` 个有效阻挡。
- A3 有效阻挡包括 301 档案展区 `6` 组展柜／展台、主楼梯 `2` 条栏杆、304 报告厅 `2` 组座席和 `1` 个讲台、303 智慧教室 `24` 组课桌、校史人物门厅 `2` 个展柜。静态叠图确认各框与相应实体对齐，桌间走道和人物长廊保持开放。
- 排除 `a3_furniture_012`、`a3_furniture_013`、`a3_furniture_014`、`a3_furniture_027`、`a3_furniture_040` 五个 `4×4` 单击误点；其中 `a3_furniture_027` 位于三楼参照调查点附近，未让不可见小框进入正式碰撞。
- `a3_furniture_010` 从导出 `{x:1416,y:217,width:202,height:148}` 最小收缩为 `{x:1416,y:217,width:200,height:146}`，分别避开报告厅东墙和南侧门洞。主楼梯两条栏杆保持用户原坐标；原 `a3_walkable_003` 按真实空间拆成电梯平台、楼梯中央和栏杆下沿连接区三块，保留连续通路并排除栏杆实体。
- A1 与 A3 的家具碰撞补齐独立 `sourceAnnotationId`，顶层 `furnitureCollisionDraftContract` 记录两批原始数量、正式导入数量、九个误点、两项边界调整和楼梯通行区细分。第四章数量契约更新为 A1 `44/7`、A2 `24/5`、A3 `63/8` 个静态碰撞／必经区域。
- 本轮只做 JSON 解析、源像素静态叠图、边界、ID、来源字段、既有结构碰撞、必经区域和关键导航点的结构核对；A3 `37` 个导入框均位于 `1672×941` 范围内，无重复 ID、无 `4×4` 框、无既有碰撞重叠、无必经区域重叠、无导航点冲突。临时叠图在目视确认后删除。
- 依照用户当前要求，本轮没有生成单文件，没有运行 `typecheck`、项目校验脚本、浏览器自动化、跨内核检查或整套测试；正式 Phaser 人物实走仍待后续允许后验证。
- 本轮没有执行 Git fetch、暂存、提交、合并、推送、上传或分支操作；修改继续保留在隔离交付 worktree，现有 Vite 服务仍从该 worktree 的 `4173` 端口运行。

## 2026-09-03 第四章因果链、资源恢复与发布前总体验证

- 修复 A2 安全员动作资源的图集归属：`guard_check_watch` 统一进入交通人物预热组；运行时在创建辅助 NPC 前检查纹理是否存在，缺失时记录合同失败并跳过该实体，避免 Phaser 生成 `__MISSING` 占位图。当前 A2 实际画面中的人物比例与同层角色一致。
- 补齐资源失败后的原地恢复闭环。当前阶段高优先级资源预热完成时，Phaser 会向 React Host 上报 `rpg_chapter4_warmup_phase_ready`；Host 据此解除输入锁和失败卡。场景热重启重新绑定当前活动 Scene，同时三层底图及前景明确使用纹理的 `__BASE` 帧，避免帧注册后误取第一个门体裁片而只显示 `66×78` 局部图。
- Chromium `1280×720` 实际完成四段连续操作：同步电梯历史并前往 A3；按实体配电箱打开面板并锁定 `mask=13`，成功演出结束后进入追逐；在 202 拾取黄铜分针组件、经主楼梯返回 A1并把道具拖入旧钟可见表盘；提交两项楼外问题、收到唯一 `回答已保存` 确认并等待灿若星辰灯演出完成。最终状态为 `phase=complete`，全过程合同失败、console error 和 page error 均为 `0`。
- 资源故障注入实际中止 `a1_2245_opening.png` 首次请求，确认失败卡会锁住移动、现实模式、道具栏、任务栏和触控操作；点击原地重试后失败层只保留一份并自动消失，A1、A2、A3 三张活动背景均恢复为 `1672×941 / __BASE`。
- 浏览器矩阵覆盖 Chromium、Firefox、WebKit，每个内核检查 `1280×720`、`1180×820`、`390×844`、`844×390` 四种视口，共 `12` 个组合。所有组合均为一个 `16:9` Phaser canvas、文档横纵溢出 `0`、合同失败 `0`、console/page error `0`；桌面键盘与移动端触控均产生有效位移，细指针桌面不显示虚拟方向键，粗指针移动端显示五个触控按钮。
- Fresh 发布验证通过：`validate:critical` 为 `21/21`，`validate:release` 为 `32/32`；第四章 topology `4735` 项、runtime `1099` 项、有效交互 `624` 项、Task 14 `391` 项、阶段预热 `91` 项及其余发布校验均通过。文案清单已刷新为 `6907` 条，其中第四章 `1740` 条。
- `npm run build`、`npm run build:single`、`npm run verify:single` 与发布套件内的三条生产浏览器冒烟均通过。离线 `demo/index.html` 为 `268795524` 字节，包含两个内联脚本和一个内联样式；该文件由构建命令生成且未手工修改。临时 QA 截图和脚本已在目视检查后移入废纸篓，交付目录没有保留这些文件。
- 本节写入时尚未执行本轮 Git 暂存、提交、推送或合并；完整修改继续位于隔离交付 worktree，等待上传前远端差异复核。

## 2026-09-03 寝室书页翻转方向修正

- 修正寝室第二张书桌“翻看摊开的书”的动画轴向。桌面书本在源图中旋转了 `90°`，书脊沿画面水平方向；旧动画却让下半页围绕自身中心左右压缩并往返两次，因此没有表现出明确的起页、越过书脊和落页方向。
- 新动画以源图书脊坐标 `(786, 562)` 为固定轴，从画面上方的右手页起页，沿纵向越过书脊后落到画面下方的左手页。翻页纸增加同色页边与低对比文字纹理，底层继续使用原寝室贴图，没有替换书本资产。
- 同一动画正在播放时再次触发会先销毁旧翻页层，避免快速按键产生叠页；动画结束立即清除临时层，不写入剧情状态或存档。
- 使用标准 web-game Playwright 客户端从 `c2-dorm-exit` 实际走到第二张书桌并按 `Space`。起页帧位于书脊上方，后半帧位于书脊下方，`render_game_to_text()` 确认场景仍为 `dorm_hub`、人物停在交互范围内，console/page error 为 `0`。
- 依照用户当前范围，本轮没有生成单文件，也没有运行 typecheck、项目校验脚本、跨内核检查或整套测试；本轮没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 寝室吹风机拾取反馈与放大详情

- 根因确认：`hairDryer` 已存在于共享道具类型、状态和详情数据中，但 RPG 道具栏与手机物品栏的展示顺序均漏录该项。控制器写入 `items.hairDryer=true` 后，界面无法生成新槽位，通用入栏动画和详情入口也无法收到这次道具变化。
- 两套物品栏顺序均补入 `hairDryer`。拾取后，共享状态立即保存该道具；场景吹风机用 `680ms` 收缩轨迹移向当前画布右下角，道具栏同时播放既有像素飞入反馈，并给新增槽位加亮。重复点击在动画期间会被拦截。
- 桌面吹风机的显示画布从 `58×58` 放大到 `104×104` 源像素，并按素材实际非透明区域中心重新设置原点；交互框同步扩大为 `104×104`，玩家看到的吹风机位置仍落在个人书桌范围内。
- 共享详情弹窗为吹风机启用大图预览：RPG 版图标为 `128×128`，手机版为 `96×96`。RPG 道具栏双击或键盘 `Enter` 可打开详情，弹窗继续说明来源、用途和天气页操作方向。
- Chromium `1280×720` 从 `c3-qizhen-rescue-dorm` 实际走到书桌并按 `Space`。`render_game_to_text()` 显示 `items.hairDryer=true`、`ownedItems` 包含 `hairDryer`；页面中只生成一个吹风机槽位，拾取阶段槽位带 `is-new-item`，console/page error 为 `0`。双击后详情图实测 `128×128`，弹窗横纵溢出均为 `0`。
- 本轮依照用户范围没有生成单文件，也没有运行 typecheck、项目校验脚本、跨内核检查或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 天气页吹风机道具接入

- 湖区天气页取消原先自带吹风机图标、点击后直接启动校准的入口。待处理卡片改为空设备接口，玩家必须把已从寝室取得的 `hairDryer` 从手机道具栏拖入接口，控制器接受后才显示既有三层云带校准界面。
- 直接点击空接口只展开道具栏并在接口内显示下一步；拖入其他道具时，校准次数保持不变，接口内改为“请拖入寝室吹风机”。两类反馈均不再调用全局字幕，快速纠正后不会把旧提示叠在校准界面上。
- 手机道具栏补齐键盘等价路径：道具槽聚焦后按 `Space` 选中或取消，按 `Enter` 继续打开详情；选中吹风机后聚焦设备接口并按 `Enter`，会执行与指针拖放相同的控制器校验。
- Chromium `1063×739` 实机验证确认：直接点击后 `inventoryOpen=true / attempts=0 / calibration=0`；错误纸条拖入后仍为 `attempts=0 / calibration=0`；吹风机拖入后为 `inventoryOpen=false / attempts=1 / calibration=1`，校准标题内显示寝室吹风机且没有残留全局字幕。键盘路径确认 `Space` 后 `selectedItem=hairDryer`，`Enter` 后选择清空并进入校准。
- 同一真实页面完成三层云带稳定校准后，状态进入 `boarding_tutorial`，`rainSafetyCleared=true`，吹风机由控制器在成功结算时从道具栏消耗，校准组件退出并显示“湖区状态已更新”；全程 console/page error 为 `0`。标准 web-game 客户端另行确认初始天气页在 `1280×720` 正常渲染且无浏览器错误。
- 本轮依照用户范围没有生成单文件，也没有运行 typecheck、项目校验脚本、跨内核检查或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 启真湖节奏钓鱼三列竖向轨道

- 节奏界面从单条横向轨道改为 `A / S / D` 三列竖向节奏板，左、中、右三列分别固定对应「左收线／提竿／右收线」。移动音符沿各自列向下抵达同一条水平判定线，超过判定时刻后继续进入判定线下方。
- `A / S / D` 与动作名称移入独立固定键位层，显示深度高于移动音符。移动块仅保留动作图标，落入键位层时会被其遮住，字母与动作名称保持完整可读。
- 短按音符改为钢琴白键比例的短块；长按音符改为沿下落方向延伸的白色长条。长条头端命中后固定在判定线上，右侧窄进度由下向上填充，尾端越过判定线后再松开。谱面判定时间、good 窗口、长按时长、张力和失败规则均未改变。
- 触屏钓鱼按钮移除会分散键位识别的道具图标，改为大号 `A / S / D` 加动作名称；`844×390` 粗指针横屏下三键宽约 `62.7px`、高 `72px`。小屏横屏专用总宽收紧到 `198px`，三键停在节奏板左侧，未再覆盖游戏内 A 列。
- Chromium `1280×720` 实际启动「网框」谱面，检查长条下落和按住中两种画面；在 `elapsed=3.73s` 附近按下 A，保持到 `4.39s` 后松开，运行时记录 `judged=2/4`、`tension=40`。`844×390` 粗指针环境通过真实 Pointer Event 完成同一次长按，结果同为 `judged=2/4`、`tension=40`；两种视口横纵溢出均为 `0`，console/page error 为 `0`。
- 本轮依照用户范围没有生成 `demo/index.html` 单文件，也没有运行 typecheck、项目校验脚本、跨内核矩阵或整套测试；只执行与本次节奏布局和长按输入直接相关的浏览器运行检查。没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 启真湖钓鱼首块预告与 D 列安全边距

- 根因确认：小鲤鱼谱面只有一枚第 `7` 拍的 S 判定，正式提前量为 `1.25s`，因此会话开始后约 `3.125s` 内节奏板没有生成任何移动块；该谱面总长仅 `5s`，玩家看到方块后的反应窗口很短，实际体验接近“没有出块就失败”。
- 保留原判定时刻、音乐、张力和失败规则。每段空档会提前创建下一枚待判定方块：正式下落前以 `0.78` 透明度停在轨道顶端，并在块内显示距离判定的整数秒；进入原 `spawnSec` 后恢复满透明度，继续按原两拍提前量下落。
- 三列中心从 `362 / 480 / 598` 收进为 `370 / 480 / 590`，移动块宽度从 `88` 收为 `80`，左右留出对称安全边距。触屏三键网格改为 `minmax(0, 1fr)`，按钮显式允许收缩并受父层可用宽度限制，避免最右侧 D 键在窄横屏被裁切。
- Chromium 以用户当前 `1063×739` 视口从 `c3-qizhen-rhythm-fish` 实际按住并松开 `Space` 启动会话；在 `elapsed=0.981s` 时已存在 `1` 枚可见 S 预告块并显示 `4s`。同尺寸从 `c3-qizhen-rhythm-net` 运行到 `elapsed=5.461s`，D 方块、D 轨道和固定 D 键完整位于节奏板内。两次页面错误均为 `0`；自动化浏览器仅记录一次未由真实用户手势解锁 Web Audio 的预期警告。
- 本轮没有生成单文件，没有运行 typecheck、项目校验脚本、跨内核检查或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 学生剧抢票网络反馈调整

- 学生剧手机票务卡移除“需要移动数据”和“打开控制中心切换网络”两项直接指令；任务提示、第一波回帖、手机主页通知和离线入口提示同步改写，不再提前公布成功所需的网络状态。
- 校园网下第一波按钮保持可用。点击后控制器进入 `first_wave_failed`，页面显示“未抢到：当前网速过慢”，随后开始第二波倒计时。
- 第二波开放条件只依赖倒计时结束，不再依赖当前网络。校园网下可以点击“参加第二波抢票”，结果仍为“未抢到：当前网速过慢”，阶段继续停留在 `first_wave_failed`，玩家可以再次尝试。
- 隐藏的成功条件继续由控制器判定。通过手机状态栏自行切换网络后，同一第二波按钮可正常结算为 `delivered`，并写入 `cc98TicketClaimedWave=2`；票务卡本身不提供解法按钮或点名提示。
- Chromium `577×739` 使用 `c3-theater-ticket-accepted` 与 `c3-theater-ticket-first-wave` 两个开发检查点完成实际点击：校园网第一波可点击，第二波在 `5s` 倒计时后由禁用变为可用，两次校园网提交均显示网速过慢且没有误判成功；切换网络后第二波正常成功。console error 为 `0`。
- 玩家文案清单已通过 `npm run text:export` 刷新为 `6901` 条。本轮没有生成 `demo/index.html` 单文件，没有运行 typecheck、项目校验脚本、跨内核检查或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 启真湖四图空气墙调试页

- 新增独立入口 `qizhen-lake-collision-editor.html` 与 `npm run dev:qizhen-colliders`。页面使用四张正式 `1672×941` 源图，可在小码头、开阔水面、直河道和天鹅湾之间切换；小码头另可切换到拆除提示牌后的底图。
- 每张地图分别提供岸上人物与皮划艇两套碰撞层。现有正式空气墙会复制进当前浏览器标签页，支持选择、拖动、八方向缩放、源像素数值编辑、方向键微调、删除、新增、撤销、当前层重置和四图重置；四条 `world_*` 世界边界保持锁定。
- 调试画布可叠加水域、前景遮挡、出生点、跨区进入点、交互目标与目标站位。这些内容只作可达性参照，不会进入碰撞导出结果。
- 导出格式为 `qizhen-lake-collision-draft/v1`，同时包含四图两层的完整碰撞数组与相对当前源码的新增、修改、删除清单；页面会话使用 `sessionStorage`，不会修改正式游戏数据。
- 在内置浏览器实际切换四图，皮划艇层分别读取小码头 `13`、开阔水面 `12`、直河道 `28`、天鹅湾 `17` 个空气墙。实际完成一次拖框新增、源像素坐标修改、改动计数更新和当前层重置，最终回到全局改动 `0`；浏览器 console/page error 为 `0`。
- 使用标准 web-game Playwright 客户端对调试页运行一次画布与文本状态检查，确认小码头底图已加载、`render_game_to_text()` 可读取编辑状态且没有 console/page error。临时检查截图已在目视确认后删除。
- 依照用户当前范围，本轮没有生成 `demo/index.html` 单文件，也没有运行 typecheck、项目校验脚本、跨内核矩阵或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 第四章 A1 主入口开门呈现修正

- 根因确认：A1 主入口运行时虽然已经进入 `open / passable=true`，中央门洞仍被左右固定玻璃窗裁片填满，门扇又采用水平压缩的折叠动作，因此开启后仍像一整面关闭的玻璃门；门框补裁片还漏写 `__BASE` 帧，注册门扇小帧后会取得 `66×78` 小帧并产生空裁剪，角色经过门口时没有收到预期前景遮挡。
- 主入口改为双扇侧滑。中央门洞取消两块错误玻璃填充，改用同一活动底图门外地面的 `132×60` 原尺寸裁片补足下部，上部保留门洞阴影；门框横梁和两侧固定玻璃继续由原底图裁片覆盖，滑开的门扇收进两侧固定玻璃之后。
- 新增与 `passableProgress=0.38` 同步的透明门槛阻挡。关闭和开门早期保持阻挡，角色脚点在源坐标 `y=802` 停住；达到可通行进度后解除，返回并离开感应范围后随关门重新启用。运行时调试状态增加 `barrierActive`，可直接核对门动画和物理通行是否一致。
- Chromium `1280×720` 实际核对关闭、开门早期、完全开启和往返关门四段：`0ms` 为 `closed / barrierActive=true`；`300ms` 角色脚点停在 `y=802`，状态为 `opening / progress≈0.12 / barrierActive=true`；`900ms` 为 `open / progress=1 / barrierActive=false`，角色已通过中央净空；返回大厅并等待后恢复 `closed / barrierActive=true`。三段合同失败、console error 和 page error 均为 `0`。
- 另在与用户截图接近的 `1000×1280` 视口核对开启态：中央门洞、侧滑门扇、门外地面和角色层级完整，文档横纵溢出均为 `0`。标准 web-game Playwright 客户端再次完成真实向下移动，`render_game_to_text()` 确认门为 `open / passable=true / barrierActive=false`；现有内置浏览器 `4173` 服务也以同一视口和路径复核通过。
- 本轮依照用户范围没有生成 `demo/index.html` 单文件，也没有运行 typecheck、项目校验脚本、跨内核矩阵或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 第四章 104 教室粉笔板书补充

- 104 黑板新增与源像素坐标绑定的粉笔板书层：保留既有 `18:44` 原始擦痕，在周围加入“传递过程”、`Q = A·v`、三节点流程简图、短推导线与低透明度擦除痕迹。板书只补充课堂环境细节，不新增谜底字幕、任务结论或进度事实。
- 修复环境细节的底图识别：`room204_restore` 的投影列表只包含 A2/A3 阶段图层，实际显示的 A1 `a1_base` 由回退映射应用，旧筛选因此把 104 黑板、105 终端和电梯时间轨等 A1 原始细节全部排除。运行时现用“阶段图层 + 当前三层实际应用底图”判定可见细节，并把该组合写入重建签名。
- 真实 Chromium 从 `c4-755-classrooms-1850` 进入 A1 104 教室，分别目检浅色操作和深色观察。浅色下板书能辨认为课堂内容；深色下 `18:44` 仍保持较高识别度，周边标题、公式、流程图和擦痕没有遮住交互点。运行时显示板书文本各一份，环境细节对象由 `8` 个恢复为 `17` 个，合同失败、console error 和 page error 均为 `0`。
- 标准 web-game Playwright 客户端再次运行同一检查点，确认 `room204_restore / A1 / a1_base` 已正常应用、104 黑板目标仍可用且 `contractFailures=[]`。本轮依照用户范围没有生成单文件，也没有运行 typecheck、项目验证脚本、跨内核矩阵或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 第四章多时段旧钟调节主循环

- 第四章的时段推进改为重复的玩家操作：开场检查旧钟后选择 `12:25`；在面包坊取得并装回旧时针后，剧情阶段先停留在 `12:25`，玩家返回旧钟确认 `18:50`；完成二楼定位片链并装回定位片后，阶段先停留在 `18:50`，玩家再次返回旧钟确认 `22:45`。后续既有 `22:45 → 07:54 → 07:55` 手动操作继续保留。
- 新增统一时间控制模型。下一刻度只由控制器已经记录的 `hall_clock_inspected`、`hour_hand_installed`、`positioning_plate_installed` 三项事实依次解锁；钟面每次只显示当前刻度与本轮新稳定刻度，不能自行选择尚未获得线索的未来时段。
- 新增大厅旧钟面板，使用第四章现有深蓝、米白、黄铜色视觉体系，包含模拟钟面、两个可选刻度和键盘／指针确认。面板只把新刻度标为“刻痕清晰”；时间转场只展示光线、照明、机械声、轮印和油光等原始环境变化，不在字幕中提前说明下一房间或结论。
- 阶段和时间允许保存两个受限的中间状态：`room204_restore + 12:25 + 已装时针`、`maintenance_repair + 18:50 + 已装定位片`。中间状态下投影仍使用旧时段底图，除大厅旧钟外的剧情目标全部锁定；确认新刻度后才启用该时段的房间、NPC、机关和巡逻状态。
- 修复旧钟与一楼主楼梯入口范围重叠的问题。只要新刻度已经解锁，旧钟交互优先于楼梯通行；浏览器中将人物脚点置于两个范围同时命中的区域，运行时仍打开旧钟面板，没有误发上楼请求。
- 开发者通道增加“旧钟第二次调时”和“旧钟第三次调时”两个检查点，便于分别查看 `12:25 → 18:50`、`18:50 → 22:45` 的等待状态与转场。
- Chromium `1280×720` 定向验证三段调时：首段真实键盘打开面板并确认后进入 `bakery_hour_hand / 1225_bakery`；第二段确认后保持 `room204_restore` 并切到 `1850_evening`；第三段确认前 `guardMode=absent` 且维修实体未创建，确认后进入 `2245_maintenance / guardMode=patrol` 并开放维修目标。三段转场期间输入锁定，点击“继续行动”后恢复；当前导航产生的 console/page error 为 `0`。
- 标准 web-game Playwright 客户端另行完成同一首段检查点的运行与 `render_game_to_text()` 读取，确认资源预热 `status=ready`、`failedUrls=[]` 且未生成错误记录。依照用户当前范围，本轮没有生成 `demo/index.html` 单文件，也没有运行 typecheck、项目验证脚本、跨内核矩阵或整套测试；没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 面包坊传送带、食堂感应门与启真湖岸线修正

- A1 面包坊补入完整传送带设施：按源图坐标覆盖玻璃展柜内的 `166×27` 皮带、前后导轨、两端滚轮、三组面包托盘、方向标记和状态灯。旧实现只有目标边缘内一条 `3px` 往返闪光，现已移除，避免新旧动画争用同一对象。
- 传送带随 A1 场景持续存在。`12:25 / bakery_hour_hand` 且时针尚未露出时，皮带纹理、托盘和扫光同步向东运动；停机演出先减速后停止；切到 `18:50` 及后续 A1 阶段时保留完整停机设施。开发调试快照增加 `bakeryConveyor`，记录可见性、运行状态、源坐标和当前时段。
- 食堂东南门加入两段接近范围。角色进入 `approachBounds` 后自动播放同源门扇的双扇开启动画，离开更宽的 `holdOpenBounds` 后自动关闭；门框、门扇、出口锚点和通行口统一使用源图 `1305..1393 × 795..900` 坐标，`Space` 仍负责正式离开食堂。
- 启真湖小码头岸上碰撞新增 `pier_west_water` 与 `south_willow_water`，分别封住码头西侧误判为岸地的浅水方块，以及南侧柳树、花坛和邻接水面。登船站位从水面边缘移到铺装岸地 `{x:650,y:690}`。
- 内置浏览器完成定向运行检查：食堂门实际完成一次“靠近开启—远离关闭—再次靠近开启”；面包坊运行时在 `12:25` 两次读取间皮带纹理与扫光坐标均发生变化，在 `18:50` 返回 `visible=true / motion=stopped`；启真湖人物脚部碰撞体向上停在 `pier_west_water.bottom=650`，向右停在 `south_willow_water.left=790`。
- 依照用户当前范围，本轮只执行布局 JSON 解析、定向 `git diff --check` 和上述内置浏览器检查；没有生成 `demo/index.html` 单文件，没有运行 typecheck、项目验证脚本、跨内核矩阵或整套测试。没有执行 Git 暂存、提交、推送、合并或上传。

## 2026-09-03 全量修改发布前门禁

- 用户确认将当前隔离交付 worktree 中全部修改上传并直接合并到 `main`。本次范围共 `57` 个文件，覆盖第三章启真湖与学生剧反馈、第四章三层碰撞和时段循环、开门与传送带呈现、道具交互、人物侧面帧、调试页面、内容数据和既有验证规则；被忽略的 `demo/` 继续作为可重建 Release 产物，不进入普通 Git 历史。
- 提交前执行 `git diff --check`，结果通过；所有新增或修改 JSON 均通过 `jq empty`；对当前差异和未跟踪文本文件执行常见凭据格式扫描，没有发现 GitHub、OpenAI、AWS、Google、Slack 或私钥格式的匹配项。
- Fresh `npm run validate:critical` 通过 `21/21`。Fresh `npm run validate:release` 通过 `32/32`，其中第四章 topology 为 `4735` 项、runtime 为 `1118` 项、Task 14 为 `407` 项，开发者通道为 `532` 项断言、`24` 个关卡和 `117` 个检查点。
- 文案清单已通过 `npm run text:export` 刷新，`npm run text:check` 统计为 `7145` 条，其中第四章 `1765` 条。生产构建、单文件生成、单文件结构校验及三条生产浏览器冒烟全部通过；冒烟覆盖 `390×844` 手机、`1440×900` 剧场 RPG 和 `1440×900` 第四章 RPG。
- 最终 `demo/index.html` 为 `268864910` 字节，含两个内联脚本和一个内联样式，SHA-256 为 `ce7734e3071e112d38b9363ad48863bcb1c8431446f0e99d8576760807825041`。该文件由发布套件生成，没有手工修改。
- 当前验证覆盖发布套件、既有定向真实浏览器操作与生产浏览器冒烟；本轮没有重新进行从第一章开始的人工连续通关，因此交付结论不包含“完整人工通关已复验”。

## 2026-09-03 main 合并与 20260903 Release

- 全量修改提交为 `672f3f6b80213007f20b323a7308d300830bc6db`。PR [#42](https://github.com/zhu607705-coder/7-55/pull/42) 的 Web CI [33742422443](https://github.com/zhu607705-coder/7-55/actions/runs/33742422443) 成功后，于 `2026-09-03 18:09:05 +08:00` 以合并提交 `75fec8a3ef12c77bb164dfad1fe5cc5f16038a9e` 进入 `main`；该提交的主分支 Web CI [33742760526](https://github.com/zhu607705-coder/7-55/actions/runs/33742760526) 随后成功。
- [7:55 Demo 20260903](https://github.com/zhu607705-coder/7-55/releases/tag/demo-20260903) 于 `2026-09-03 18:19:03 +08:00` 发布并设为最新版本，目标精确绑定上述 `main` 合并提交。
- GitHub 端确认四项资产均为 `uploaded`：离线 HTML `268864910` 字节，SHA-256 `ce7734e3071e112d38b9363ad48863bcb1c8431446f0e99d8576760807825041`；实现归档 `569681321` 字节，SHA-256 `9db27dc7dc7fa226334df02d8167351ca2dc8cc3cae38d9fa8b64279859c891e`；两个校验文件分别为 `90` 与 `99` 字节。
- 实现归档直接由 `origin/main` 的 `75fec8a` 生成，包含 `1229/1229` 个 Git 跟踪文件；`unzip -tq` 未发现压缩数据错误，本地两项 SHA-256 复核通过，GitHub 服务端对两个大文件给出的摘要与本地一致。
- `README.md` 的当前离线版本和 `ASSETS.md` 的发布清单同步切换到 `20260903`。原始开发 checkout 保持未动，全部交付操作继续在 `/private/tmp/7-55-chapter4-delivery-tZmEdm` 隔离 worktree 中完成。
## 2026-09-03 校园大地图动态水面与手工遮罩

- 普通校园底图新增浏览器原生动态水层：使用归档 Godot 水纹图中三张 `128×128` 正式帧，每帧持续 `500ms`，不再叠加横向位移。运行时由 Phaser Canvas 分块绘制，不加载 Godot 场景、导出或兼容运行时，也不改变 `4516×3420` 世界、碰撞、入口和摄像机坐标。
- 水域覆盖生成物收口为 `2048×1536` RGBA Alpha 图集和紧凑分块清单。自然岸线支持灰度过渡，道路与桥面在最后按道路 Alpha 硬裁切；建筑层只额外接纳用户确认的中上部 `north_mid_river`，未把蓝屋顶、球场或其他装饰池塘纳入水面。
- `campus_water_mask_override.png` 作为与底图严格对齐的 `4516×3420` 完整手工遮罩。用户本轮直接调整 `17081` 个像素，覆盖月牙楼、启真湖右岸与中上部河道附近；重建结果保留全部 `255` 个非零 Alpha 级别。白色补水、黑色排除、灰色控制覆盖率，透明区域仍可退回自动识别。
- 真实浏览器定位到不规则旧水块的另一来源：同源建筑人物遮挡裁片位于动态水面上方，原先会把裁片多边形内的底图旧水重新盖回去。建筑前景现复用最终水域 Alpha，以 `destination-out` 只扣掉旧水像素；月牙楼与基础图书馆的建筑本体、道路和人物前后遮挡保持不变。
- Chromium `1280×720` 实测月牙楼、启真湖右岸和中上部河道三处：不规则深蓝旧水块消失，道路交界为硬边，文档横纵溢出均为 `0`。运行时创建 `188` 个水面分块；按最终反馈移除全部横向位移，只保留三帧水纹以 `500ms/帧` 循环，岸线固定且没有位移抖动。无 JavaScript 运行时异常，仅开发服务器缺少 `favicon.ico` 产生既有 `404` 资源日志。
- `npm run map:zijingang`、水层专用校验、玩家帧校验、`npm run typecheck` 与 `git diff --check` 通过。`validate:quick` 为 `3/4`：唯一失败是工作树中既有游戏文本改动尚未执行 `text:export`，与本轮水面资源和运行时代码无关；为避免覆盖用户的其他未提交内容，本轮未自动刷新该生成文档。没有执行 Git fetch、暂存、提交、合并、推送或上传。

## 2026-09-03 左右行走完整重绘与独立站立帧

- 左右动画不再使用腰部横切、上下半身拼接或局部腿部平移。通过内置 ImageGen 重新生成统一角色模型的完整侧身人物帧，正式源图收口为 `player_side_walk_8frame_generated_v4.png` 与 `player_side_model_9pose_generated_v4.png`；构建脚本只提取完整连通人物、去除品红背景，并对整个人做等比尺寸归一、头部锚点和脚底线对齐。
- 运行时左右走路改为 `8` 帧、`880ms` 完整周期：`0 / 4` 为相反腿接触，`1 / 3 / 5 / 7` 为紧凑收步／恢复，`2 / 6` 为前后腿相反且双脚几乎并拢的通过帧。九张运行图均为独立完整人物，没有复用裁切肢体；轮廓高度统一为 `104px`，头部／上身 Alpha 重合度最低保持在 `0.82` 以上。
- 新增单独的 `player_side_idle.png`。Phaser 动画器只有在速度为零时使用该图；移动中的左右转向仍使用行走帧，不再闪入站立姿势。Three.js 楼梯人物同样只在 `walking=false` 时使用独立站立纹理。左右共用同一套完整右向原画并由运行时镜像，碰撞尺寸、显示比例和脚点未改。
- 按用户要求，上下方向动画与 `16` 张上下帧保持原样；构建器检测到解码像素未变化时不再重新编码文件，本轮最终工作树中上下帧没有差异。旧 `player_side_8.png` 至 `player_side_11.png` 及十二帧引用已退出运行时。
- Chromium `1280×720` 校园地图 Demo 真实键盘验证左右各完整行走一轮：两侧都覆盖接触帧 `0 / 4` 与并腿帧 `2 / 6`，移动采样中没有 `side-idle`，松键后均切换为 `side-idle`；左右通过帧截图目视确认镜像、落脚基线和人物位置正常，应用运行时异常为 `0`。
- `npm run typecheck`、`npm run verify:rpg-player`、`npm run verify:rpg-character-sprites` 与浏览器定向验证通过。`validate:quick` 为 `3/4`，唯一失败仍是工作树中既有游戏文本导出过期；本轮没有改动玩家文案，也没有覆盖该未提交生成文档。没有执行 Git fetch、暂存、提交、合并、推送或上传。

## 2026-09-03 校园水面与左右人物动画合并验证

- 从 `origin/main` 获取并以 fast-forward 合入远端新增的 `5` 个提交，基线由 `74b245d` 更新到 `76fcdf7`。恢复本地修改时逐项合并人物运行时、构建器、素材说明和进度记录；用户确认的提交范围只包含校园动态水面与左右人物动画，室内门、启真湖实验资源、Godot 归档和第三章文案继续隔离在提交之外。
...
- 按最新反馈统一移除手机拖拽候选/悬停框，剧场热点框、剧场/图书馆/启真湖投放框、食堂饮品投放框、寝室矩形闪框和签到鼠标悬停描边。保留实际目标数据、距离/模式/物品校验与全部操作事件。
- RPG 道具栏仅有标题、图标、名称，使用结果交给短时场景字幕；手机物品栏移除拖动和距离说明。详情、合成、拾取动画及键盘焦点仍可使用。
- 已按新要求更新 AGENTS 展示约定，完整实现/验证边界见 canonical report 顶部。第二章水滴、三个浏览器内核的剧场纸巾到票根、离线同链、第四章两问到 complete 均已实际运行；另检查第一章签到页、图书馆、食堂、湖区和第四章大厅截图。标准客户端和发布校验已执行，临时 QA 不进入交付；未提交或上传 GitHub。

## 2026-09-05 实机预告片交付

- 通过 Computer Use 操作实际游戏录屏，完成 `/Users/zhuhangcheng/Movies/7-55-trailer-20260905/final/7-55_实机预告片_4分钟_1080p.mp4`：4:00、1080p、30 fps、H.264/AAC 双声道 48 kHz，约 32.21 MB。
- 内容含闹钟/签到、体艺闪退、食堂纸条、湖区划船及钓鱼开局、黑天鹅、教学楼调时、两问消散和真实点灯。游戏音乐/音效后期混音；不修改玩法、不伪造钓获成功、不采集麦克风。
- 31 段镜头与 6 个章节。全片完整解码无错误，62 个分镜抽帧检查，修正字幕遮挡和按键裁切，最终音频峰值 -1.5 dBFS。SHA-256 `534519211357369b58d29447c406eb617d8ecf60e2b3f42068a0741e372e18c9`。
- 可重剪素材、时间线、音频和脚本在媒体目录保留。未提交 Git、未上传、未改动原始开发 checkout。详细边界见 canonical report 顶部。

## 2026-09-05 启动、地图资源与运行时负载优化

- 当前用户请求：尝试优化整个游戏的性能，使其降低负载与快速加载。完整记录和测量口径见 `project-development-report.md` 顶部性能节。
- 修正新游戏手机阶段无条件预热校园大地图的问题，使用既有 `fullCampusMap` 权限后才预热；CC98、录音、时间线三个手机页面改为按需加载，并在手机壳内使用局部 Suspense。
- 七个活跃 RPG 场景把大型诊断快照改为按读取计算，普通游戏帧只登记提供函数。执行验证覆盖非急切计算、即时读取、提供者切换和场景清理；没有改变游戏时钟、剧情事实、碰撞或输入速率。
- 23 张大地图由原 PNG 生成 `-lossless -exact` WebP，解码 RGBA 23/23 逐字节相等，总体积由 `98,680,130 B` 减到 `72,494,984 B`。生产与离线构建使用 SHA-256 校验后的派生图；源图和源像素坐标保留，开发服务器继续使用 PNG。
- 本机 Chrome 单次 1280×720 冷上下文测量：启动 13 秒窗口内资源条目体积 `26,736,068 → 1,098,654 B`，条目 `30 → 3`；终章 5 秒 ScriptDuration `0.6252 → 0.5192 s`，追逐 `0.4836 → 0.4104 s`。这是单组本机样本，不代表所有设备；资源条目体积可含重复加载，不能视作真实传输总量。
- 完整 `validate:release` 33/33 通过。旧故事校验固定插件数组，已明确验证新地图插件与原 H3 嵌入插件共存，保留视频独立 256 KiB 分块要求。最新 `text:check`、`typecheck` 与 `git diff --check` 通过。
- 标准 web-game 客户端完成 opening 画面与运行态检查；Chromium 1280×720、Firefox 1440×900、WebKit 390×844 分别实测 CC98/录音/时间线加载、两问到终章 complete、湖区移动和返回手机，console/page error 均为 0。Chromium 另用 file:// 单文件走完同一组页面和交互，全部完成。截图检查后清理，不纳入交付。
- 生成单文件为 `234,255,410 B`，较原 `269,164,832 B` 减少约 `34.91 MB`；SHA-256 `9a88286b8806a740033bfa23af898b6f6527869a6f25f53d819da87715c280fb`。本地生产预览端口 4180；原 4178 开发页保持运行。
- 后续候选：低端真机的长期内存测量、纹理生命周期与慢网入口时延。无损压缩不减少同尺寸纹理解码内存，本轮未降低分辨率和帧率，也未执行 Git 提交或上传。

## 2026-09-06 分支整合上传

基于 e07a72e 整合当前特技骑行、剧情钓鱼、剧場与第四章玩法，以及“修复第四章剧情衔接”中的预约 UI、书架和取票提示细节。保留远端电量、雨天恢复、资源加载和画布兼容功能。用户要求停止继续验证并直接上传；具体已完成检查与未完成项见 docs/releases/20260906-integration.md。宣传录制分支单独保留，未打包单文件。

## 2026-09-06 保安追逐体验修复（本地）

- 被抓后原地停留 5.2 秒，显示保安对白：“你在这里干什么？已经这么晚了，快点回去，要清楼了。”对白结束后才提交原有失败/恢复事件。维修巡逻、主楼追逐和楼梯间采用相同呈现。
- 按用户修正，起步宽限为 2 秒；保安速度 174，玩家速度保持 208，楼梯间和二楼接续保留领先距离。
- 二楼显示“关门 [Space]”，支持点击和 Space/Enter 的短按；显式关门请求通过原控制器与空间校验后关闭门并结束追逐。场景重入使用独立交互编号，避免与一楼旧请求重复。
- 保安寻路使用当前 Arcade 碰撞矩形进行路径规划，原有实体碰撞仍为唯一碰撞权威；跨层位移用 Body.reset 同步上一帧位置，动画换向限频、保留脚点并跳过未加载动画。已检查全部保安图集，源文件和帧均存在。
- 验证：实际捕获时对白出现后 1.8 秒人物坐标和尝试次数均未变，之后才复位；原生键盘走通一楼、楼梯间、二楼及关门，进入 final_minute_recovery，页面错误为零。导航绕障与 A1/A2 连通检查通过。
- 不包含 docs/video 的其他修改；尚未为本次修复提交或推送 GitHub。

## 2026-09-06 四次剧情钓鱼递进（本地）

钥匙、网框、小鲤鱼、纸条分别使用 80/96/112/128 BPM，四套不同长短拍。判定容错逐步收紧，鱼影运动逐步加快；声音、底部 UI 与起鱼窗口共用时值映射，奖励按对应谱面重放验证。四种捕获在普通与辅助难度的操作、八次起鱼及输入重放均通过；保留现有控制和道具链，不涉及无尽模式。

## 2026-09-06 第四章恢复交付关键验证修复

- 上传前的完整关键套件发现旧第四章验证器仍把已经退役的 1.2 秒／196 速度、八段转场和旧维护物品链当作目标。当前正式合同是 2 秒／174 追逐、九段转场，其中五次时间覆盖、四次场景内交接；验证器已改为检查这一合同以及恢复后的手动调钟、六组调查、诊断、存档和楼梯间重试。
- 恢复追逐音乐强度事件：从实时守卫间距确定追赶、跟踪、贴近三个档位，跨楼层时重置为追赶档；每次尝试重建和销毁都会清空音乐、贴近台词与换层台词状态。场景重启验证改为检查重置调用位于背景创建之前，而不是误将其限制为 `create()` 的第一条语句。
- `npm run validate:critical` 通过 24/24：包含 TypeScript、第二章、第三章、启真湖、追逐音频、跨章节合同、第四章预热、有效交互、星灯、故事、拓扑、恢复运行时、因果链与任务合同。`docs/video/` 仍不在本次交付范围。
- GitHub release CI 首轮发现生成文本索引过期，以及活跃精灵目录遗留未注册的 `chapter4_story_items_v01.png`。已重新生成 `docs/game-text-by-chapter.md`，删除无运行时消费者的 v01 并将活跃资源说明指向 v02；本地 `npm run text:check`、`npm run chapter4:validate-assets` 与完整 `npm run validate:release` 均通过。

## 2026-09-06 第四章遮挡、空气墙与校友墙合并审计（本地）

- 对比 9 月 5 日已验证版本与当前 `main`：A1/A2/A3 的静态碰撞矩形、可通行区域、前景遮挡 ID 和所有几何数值（含 `baselineY`）逐项一致；空气墙数据没有被覆盖回旧版。
- 确认运行时回退：场景仍保留主入口门、墙面遮挡和校友墙字段，却曾缺少门帧注册／门障碍／前景深度同步／`playerRevealAlpha` 遮挡，以及按人物楼层、深度、帧和点击热区创建校友墙的代码。结果是人物在墙后不再正确被覆盖，A1 校友被错误投放到 A3，校友墙会在资源刷新后重复创建。
- 已恢复上述运行时路径并补充 2 项场景回归检查。Chrome 开发检查点确认 A1 碰撞和遮挡调试层正常加载、A3 肖像按三楼墙体坐标显示且控制台为零错误；`typecheck`、恢复场景 28/28、三层拓扑、资产、楼梯间、保安呈现及完整关键套件 24/24 通过。当前修复尚未提交或上传。

## 2026-09-06 扩展墙面与最终两问复查（本地）

- 上一节仅有检查点截图，不能证明三处扩展空间完整恢复。本轮进一步补回共享人物深度、完整人物边缘约束与完整地图 `__BASE` 裁片，在 Chrome／Firefox 1063×739、WebKit 390×844 用原生键盘从大厅走完三面墙的前后路线，脚部底边分别停在 264／248／584；墙后局部身体淡化、绕到前侧恢复均实际检查。
- “灿若星辰”两问重新使用 `ChapterFourExteriorQuestions` 和原逐题浮现／消散样式，修复手机容器选项裁切；Chrome／Firefox／WebKit 都已实际选答、确认原子保存、点灯、进入结局。旧逐题组件确实曾被整合后的挂载链绕过。
- 同时恢复遗漏的维修推车分层、签到读卡器／纸槽图形、1320 ms 配电完成演出，保留 2 秒／174 追逐及当前四关楼梯。配电演出期间不消耗追逐宽限。
- 恢复上次被大幅缩减的三份第四章验证器：运行时主体 1179 条、任务与身份矩阵 408 条通过；完整 `validate:release` 35/35 通过，已重新生成本地离线单文件和文本索引。完整范围、证据和未覆盖项见 `docs/reviews/20260906-chapter4-merge-reaudit.md`。本轮没有提交、推送或合并。
- 按追加要求直接对比 `demo-20260905` 完整版本后，再补回六处差异：仅已加载楼层快照、分批帧注册检查、停用旧维修拾取提示、校友资料卡布局、HUD 细描边、维修保安呈现状态初始化。新增探针复现了 A2 尚未加载时旧代码的 `background_missing:2`，修复后呈现执行探针 9/9；Chrome 原生移动、Space 打开程开甲资料及 Esc 关闭通过。精确比较结果见同一复查记录的“直接对比 9 月 5 日完整版本”节。

## 2026-09-06 面包坊传送带可见性及回钟链修复（本地）

- 用户标出的柜台区域复现了真正的渲染遮挡：传送带正在运行，但深度 4348、检查灯 4342、时针 4347 均低于不透明柜台裁片 4387。新增共享柜台表面深度，带面、托盘、灯、时针与提示均在柜台裁片之上、人物层之下；几何、碰撞和交互范围没有移动。新增实际方法执行探针在修复前失败、修复后通过，呈现探针 10/10。
- 实际完整流程又发现故事位置仍为 `a1_bakery` 时，大厅旧钟插槽被错误禁用。为同一连续 A1 地图的插槽增加该位置别名，继续严格验证楼层、阶段、物品、模式及实际距离。新增控制器流程测试不改写房间：检查灯→停带→拾取→远处投放拒绝且零写入→靠近安装成功→读档保持 12:25 待调钟。
- Chrome／Firefox 1063×739、WebKit 390×844 原生键盘与实际鼠标拖拽完整走通：深色模式可见运行带面→浅色检查→停带与时针露出→拾取消失并入包→步行回大厅→拖入插槽→手动调到18:50。WebKit 发现道具栏遮挡模式按钮，已将第四章手机端道具栏放在模式按钮下方后重走成功。没有通过修改坐标或强制剧情状态来通过浏览器路径。
- `chapter4:validate-runtime` 完整主体1179条及补充检查通过；修复逻辑的 `validate:release` 35/35通过。手机CSS调整另用WebKit完整流程复验；同步重建本地单文件。本轮未提交或推送。

## 2026-09-07 启真湖四件材料最终组装（本地）

- 用户澄清四条支线没有先后关系：钓鱼竿、尼龙绳、破旧网框、天鹅磁铁都必须独立保留在物品栏，集齐后才最终合成磁性钓鱼竿。投喂黑天鹅现只消耗小鲤鱼并获得 `swanMagnet`，阶段回到 `tool_chain`；不会提前消耗钓鱼竿或写入 `magneticRodCombined`。
- 恢复物品栏“四件材料已齐／组合四件材料”入口，提交时调用现有 `combineItems` 原子事务，四件材料一起消耗并生成 `magneticFishingRod`。`c3-qizhen-paper` 检查点重新提供四件材料等待组装；存档归一化不再把 `swanFed` 自动改造成磁性钓鱼竿。
- 黑天鹅在 `tool_chain` 可独立检查饲料盒并获得磁铁；保留旧存档 `swan_exchange` 的小鲤鱼投喂兼容入口。两个入口都只获得磁铁，不能提前合成或阻断其他材料支线。
- 发布前补查发现存档把黑天鹅进度错误推导为渔网已组合，导致读档丢失绳和网框。已让独立支线阶段保留真实装配事实，六种支线顺序每一步均保存/重载后继续验证，204 项断言通过。
- `scripts/verify-qizhen-swan-material.mjs` 验证投喂只产生独立磁铁；`scripts/verify-qizhen-inventory-combine-browser.mjs` 用 Chrome／Firefox／WebKit（含 390×844）检查四件材料按钮、最终消耗/产出和投喂状态。20260907 提交范围为湖区规则、存档、物品栏、任务文案、开发检查点及验证文件，排除 `docs/video/`。
