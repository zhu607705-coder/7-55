...
## 2026-08-30 A 范围协作者整合、冷启动发布门与 GitHub Release

- 范围与工作区：按用户确认的 A 范围，继续使用 `selected-phaser-ui-20260721` 现有工作区和 `main`，没有新建工作树。先将已批准的本地修订保护为 `2fbd96c`，再语义整合合作者渲染更新为 `b0561d4`，发布门修正为 `03793bc` 与 `f41e78b`。
- 合作者对比：提交和推送前分别核对工作树、本地超前提交与远端缺失提交。最后一次 `git fetch origin` 后，工作树无杂项，本地仅超前 `f41e78b`，远端没有本地缺失的提交。
- 发布门根因：首次 Web CI `33285941564` 在干净 GitHub runner 上为 `258 MB` 离线页面内联全部媒体时超过 Node 默认堆限额。`build:single:unchecked` 现在自带 `6144 MB` 堆预算，不再依赖调用方的 `NODE_OPTIONS`；发布套件顺序也收口为生产构建、单文件构建、单文件校验和真实 Chromium 烟测。
- 本地验证：在显式移除 `NODE_OPTIONS` 的条件下运行 `npm run validate:release`，`31/31` 项全部通过，总耗时约 `63.8s`。Chromium 烟测通过手机 `390×844`、剧院 RPG `1440×900` 和第四章 RPG `1440×900` 三个入口；单文件为 `258674356` 字节，含 `2` 个内联脚本和 `1` 个内联样式。
- 远端验证：`f41e78b4c9d34f7b1559e2fadc53a7a76d17bec5` 已推送到 `origin/main`，Web CI [33286229966](https://github.com/zhu607705-coder/7-55/actions/runs/33286229966) 在干净 Node.js 22 runner 上于 `2m53s` 内成功完成发布套件。
- Code review：本轮审查的新发布变更没有留下 CRITICAL 或 HIGH 级问题。已修复一个 HIGH 级的 CI 冷启动顺序与默认堆上限问题，以及 Chromium 烟测序列化整个内联文档导致的性能问题。
- `20260830` 附件：发布范围固定为 `7-55-demo-20260830.html`、它的 SHA-256 文件、`7-55-implementation-20260830.zip` 及它的 SHA-256 文件。单文件 SHA-256 为 `938084a731b44b3f2e0641499899071d3c0a13158e31ef33a177527cdfc00001`；实现归档固定从已通过 CI 的 `f41e78b` 产生，并通过 `unzip -t` 与 SHA-256 反向校验。
