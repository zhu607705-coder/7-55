...
function libraryQuest(state: GameState): QuestViewModel {
  ...
  const sources: StepSource[] = [
    ...
    { id: "bd_briefing", label: "听完 bd 机制说明", done: state.ui.libraryFinalsPhase !== "bd_briefing" && puzzle.cc98UploadedEvidenceIds.length === 4 },
  ...
  const next: Record<string, NextAction> = {
    ...
    bd_briefing: phone("听系统说明为什么还需要 bd", "cc98", ["证据完整度已经到 4/4。", "系统会解释为什么上传不等于被看见。", "回到 CC98 原帖，读完 bd 机制说明。"]),
