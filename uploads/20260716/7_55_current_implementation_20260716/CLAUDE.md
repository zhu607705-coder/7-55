...
- Campus exploration uses one strict `90-degree` top-down pixel-art world with north facing up. The player moves in four directions on authored roads, squares, bridges, lawns, and interior paths; Phaser owns the player, collisions, depth, labels, hotspots, quest markers, camera, and state transitions.
- The official campus plate is both the spatial source of truth and the primary walkable background. Buildings, roads, water, bridges, vegetation, collision geometry, entrances, labels, and story triggers must share the same `2400px × 1920px` world coordinate system.
...
- Before artwork generation, the campus model must record normalized official-map coordinates, footprint width/height, scale class, adjacent road or water edge, entrance direction, and an explicit silhouette signature for every named landmark. Runtime collision and trigger coordinates derive from this model.
- Use one global world scale for the complete plate. Landmark footprints preserve the ratios measured from the official map; a local visual exception must be documented in the model and cannot silently rescale one building. Repeated residential or teaching blocks share one authored size family.
...
- Landmark acceptance checks the recognizable top-down footprint and roof signature: Crescent Building keeps its continuous arc and twin cylindrical ends; Basic Library keeps its ring crown, connected wings, and glass atrium; the stadium keeps its wide elliptical shell and central roof fold; South Gate keeps its long horizontal nine-bay footprint and east-side Qiushi Eagle marker.
- Detailed campus artwork uses GPT Image image-to-image with the official full-campus plate and landmark crops as references. MiniMax remains audio-only and must not generate campus artwork.
- The playable road network follows the official plate continuously. Buildings cannot overlap roads or water; bridge sprites and bridge collision openings must span the complete water width and connect walkable land on both ends.
- The campus world preserves official road, water, bridge, landmark, district, size, and adjacency relationships. Local landmark exaggeration is allowed only when recorded in the spatial model and must not change its center, orientation, entrance, or neighboring features.
